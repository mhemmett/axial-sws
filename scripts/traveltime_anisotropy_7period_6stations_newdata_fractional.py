#!/usr/bin/env python3
"""
traveltime_anisotropy_7period_6stations_newdata_fractional.py

Extends traveltime_anisotropy_7period_5stations_newdata.py (kept as-is, not overwritten)
with AXAS2, whose 2022-2026 new-mfast max_dt=0.2s rerun completed 2026-07-23 (2015-2021
already existed). Tier 4 QC (quality>=0.5, dt<T_dom/2, phi_err<20deg, dt_err<0.05s).
6-station 7-period travel-time-weighted percent-anisotropy tomography:
  ...
  AXAS2: mfast_maxdt_pipeline_transfer/splitting_results_AXAS2_{2015_2021,2022_2026}_all_batches.csv

All 6 stations are now included and traced (yellow markers) -- no excluded/reference-only
stations remain.

Method: FRACTIONAL shear-wave anisotropy 'a', not the naive dt*100/T_travel ratio --
solved from the standard two-velocity (fast/slow) splitting relation
    Vs_fast = Vmean*(1+a/2), Vs_slow = Vmean*(1-a/2), dt = d/Vs_slow - d/Vs_fast
which rearranges to a quadratic in a:  a^2 + (4d/(dt*Vmean))*a - 4 = 0. Using the
travel-time-consistent mean velocity along the ray, Vmean = d/T_travel (so d and Vmean cancel
and only T_travel/dt matters), the quadratic simplifies to:
    x     = 2 * T_travel / dt
    a     = -x + sqrt(x^2 + 4)          (the other root, -x - sqrt(x^2+4), is always negative)
    A_ray = a * 100                     (percent) -- kept ONLY if 0 <= a <= 1, else the ray is
                                         dropped (non-physical solution) rather than clipped
...
Produces: traveltime_anisotropy_7period_6stations_newdata_fractional.pdf (1 page)

Run with:
    python3 traveltime_anisotropy_7period_6stations_newdata_fractional.py
"""
...
OUT_PDF = os.path.join(HERE, 'traveltime_anisotropy_7period_6stations_newdata_fractional.pdf')
...
INCLUDED_STATIONS = ['AXAS1', 'AXAS2', 'AXCC1', 'AXEC1', 'AXEC2', 'AXEC3']   # all 6 -> traced + yellow markers
EXCLUDED_STATIONS = []   # none -- AXAS2's 2022-2026 rerun completed 2026-07-23
...
def make_page(results, title):
    ...
    if last_h:
        ...
        cb.set_label("Travel-time-weighted mean fractional anisotropy 'a' per voxel (%)", fontsize=9)
...
def main():
    ...
    title = (f'AXAS1/AXAS2/AXCC1/AXEC1/AXEC2/AXEC3 (new mfast max_dt=0.2s data) — travel-time-weighted mean '
             f"fractional shear anisotropy 'a' per voxel (%, 7-period)\n"
