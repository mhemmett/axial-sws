"""
Delay time (dt) mapped spatially by earthquake SOURCE voxel, AXEC2 mfast try_filters data only,
with the dt<=T_dom/2 cycle-skip-risk cut applied (same filter as
rose_axec2_mfast_dt_vs_half_dom_period_filter.png): events with dt greater than half their own
chosen-filter dominant period are excluded, on top of the standard QC gate (Q_w>=0.5,
phi_error<20 deg, dt_error<0.04 s, success, dt>0).

Same 3 depth ranges (0.0-0.75, 0.75-1.5, 1.5-2.25 km) x pre/syn/post-eruption period grid as
dt_by_source_voxel_per_station.py, restricted to the single AXEC2 (mfast) page.

Produces: dt_by_source_voxel_axec2_domperiod_filtered.pdf

Run with:
    python3 dt_by_source_voxel_axec2_domperiod_filtered.py
"""
...
df = df.dropna(subset=['quality', 'phi_error', 'dt_error', 'chosen_filter_dom_period_samples'])
...
df = df[df['dt'] <= df['dom_period_s'] / 2.0].copy()
n_after = len(df)
...
df['x'] = x
df['y'] = y
print(f'Events with location: {len(df)}')

# ── Station coordinates + bathymetry ─────────────────────────────────────────
...
def voxelize(sub):
    ix = np.floor((sub['x'].values - X_START) / VXY).astype(int)
    iy = np.floor((sub['y'].values - Y_START) / VXY).astype(int)
    iz = np.floor(sub['depth'].values / VZ).astype(int)
    valid = (ix >= 0) & (ix < NX) & (iy >= 0) & (iy < NY) & (iz >= 0) & (iz < NZ)
    ix, iy, iz, dt = ix[valid], iy[valid], iz[valid], sub['dt'].values[valid]
...
for pi, label in enumerate(PERIOD_LABELS):
    ...
    print(f'  {label}: N={n:,}, covered voxels={np.isfinite(a3d).sum():,}')

vmax = float(np.nanpercentile(np.concatenate(
    [r['a'][np.isfinite(r['a'])] for r in results if np.isfinite(r['a']).any()]), 99))
if not np.isfinite(vmax) or vmax <= 0:
    vmax = 0.1
cmap = plt.colormaps['Blues']
levels = np.linspace(0., vmax, 15)

fig = plt.figure(figsize=(3.0 * 3 + 0.6, 3.2 * 3 + 0.5))
gs = GridSpec(3, 4, width_ratios=[1, 1, 1, 0.05], hspace=0.06, wspace=0.06)
last_h = None
...
legend_handles = [
    mlines.Line2D([], [], marker='^', color='w', mfc='#FFD700', mec='k', ms=7, label='AXEC2'),
...
fig.suptitle(f'AXEC2 (mfast, dt<=T_dom/2 filtered): Delay time by earthquake SOURCE voxel\n'
             f'N={n_total:,}, Q_w>={QW_MIN}, phi_err<{PHI_ERR_MAX:.0f}deg, dt_err<{DT_ERR_MAX}s, '
             f'dt<=T_dom/2, >={COUNT_MIN} events/voxel required',
             fontsize=11, fontweight='bold', y=1.04)
fig.subplots_adjust(bottom=0.12, top=0.86)

OUT_PATH = 'dt_by_source_voxel_axec2_domperiod_filtered.pdf'
