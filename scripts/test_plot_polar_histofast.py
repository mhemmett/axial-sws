...
NCat=Cat.select(lambda_select=['rms','min'],obs_stime=time_inters[1])
#NCat=Cat.select(obs_stime=time_inters[0])
#NCat=Cat.select()
...
plt.close('all')
ax_polar=NCat.plot_polar_histofast(ax=None,facecolor=None,width_bin_deg=10,lw=1,edgecolor='k',
                             label_mode='azimuth')
