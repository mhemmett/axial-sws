"""
Multipanel summary figure: AXAS2 shear-wave splitting measurements over time
...
(panel sizing, fonts, labels, legends), with AXAS2 in place of AXEC2. Reuses the
...
production AXEC rose and temporal-histogram plots:
  ...
  - Data: lqt_pykonal_combined_results/splitting_results_AXAS2_{2015_2021,
...
Panels:
  A. Single-row, 7-period (pre/syn/5 post-eruption bins) fast-direction rose
     ...
     restricted to AXAS2 only (one row instead of 6).
...
Run with: python3 figure_as2_multipanel_temporal.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_as2_multipanel_temporal.pdf')
...
def load_axas2_enriched():
    d1 = pd.read_csv(os.path.join(DATA_DIR, 'splitting_results_AXAS2_2015_2021_all_batches_enriched.csv'))
    d2 = pd.read_csv(os.path.join(DATA_DIR, 'splitting_results_AXAS2_2022_2026_all_batches_enriched.csv'))
...
def main():
    print('Loading AXAS2 enriched LQT + PyKonal-FMM results (2015-2026)...')
    raw = load_axas2_enriched()
    ...
    fig.suptitle('AS2: Shear-wave Splitting Measurements Over Time', fontsize=17 * FONT_SCALE,
