...
7-period fast-direction rose plots (5 rows: AXCC1, AXEC1, AXEC2, AXEC3, AXAS1 x 7 cols:
eruption-relative periods) built from the NEW mfast try_filters + max_t_shift_s=0.2s splitting
results. Extends rose_7period_axcc1_axec1_axec2_newdata.py (kept as-is, not overwritten) with the
two stations whose transfer-pipeline reruns completed 2026-07-23:
  ...
  AXEC3: both periods from mfast_maxdt_pipeline_transfer/
         splitting_results_AXEC3_{2015_2021,2022_2026}_all_batches.csv (new, complete)
  AXAS1: both periods from mfast_maxdt_pipeline_transfer/
         splitting_results_AXAS1_{2015_2021,2022_2026}_all_batches.csv (new, complete)
...
Produces: rose_7period_5stations_newdata.pdf  (5 pages)

Run with:
    python3 rose_7period_5stations_newdata.py
"""
...
OUT_PDF = os.path.join(HERE, 'rose_7period_5stations_newdata.pdf')
    ...
    with PdfPages(OUT_PDF) as pdf:
        for tier_idx in range(len(TIERS)):
            ...
            fig = make_rose_figure(
                dfs, time_periods,
                title=(f'AXCC1 / AXEC1 / AXEC2 / AXEC3 / AXAS1 fast direction rose '
