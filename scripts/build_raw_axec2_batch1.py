"""
Build organized_waveforms for AXEC2 raw batch 1 (the first 250 catalog events, pre-QC
entirely), replicating axial_splitting_mldd_AXEC2_batched.ipynb's cells 3-32 exactly, from:
  - data/mldd_catalog_2015_2021.csv (full catalog)
  - scripts/axial_mldd_2015_2021_axec2_batch_1.mseed (raw, unfiltered retrieved waveforms)

This intentionally does NOT apply the old P-wave-incidence QC cut (<30 deg) - that cut is
being replaced. SNR>=2.0 and rectilinearity>=0.7 are still applied, matching the original
pipeline. QC metrics computed fresh: SNR, rectilinearity, P-Jurkevics incidence (kept for
reference), Eigenvalue-S incidence, and PyKonal-FMM incidence.

Run with:
    python3 build_raw_axec2_batch1.py
"""
...
BATCH1_MSEED = os.path.join(HERE, 'axial_mldd_2015_2021_axec2_batch_1.mseed')
BATHY_FILE = os.path.join(HERE, '..', 'data', 'AXIAL_MODEL_3P_VELOCITY.S.mod.buf')
OUT_DIR = os.path.join(HERE, 'raw_axec2_batch1_data')
OUT_WAVEFORMS_DIR = os.path.join(OUT_DIR, 'waveforms')
OUT_METADATA_CSV = os.path.join(OUT_DIR, 'raw_axec2_batch1_metadata.csv')
...
def main():
    ...
    axec2_catalog = catalog[catalog['station'] == STATION].copy().reset_index(drop=True)
    axec2_catalog['datetime'] = pd.to_datetime(axec2_catalog['event_datetime'], format='ISO8601')
    axec2_catalog['datetime'] = axec2_catalog['datetime'].apply(lambda x: UTCDateTime(x))
    axec2_catalog['p_time'] = axec2_catalog.apply(lambda row: UTCDateTime(row['datetime']) + row['arrival_time_p'], axis=1)
    axec2_catalog['s_time'] = axec2_catalog.apply(lambda row: UTCDateTime(row['datetime']) + row['arrival_time_s'], axis=1)
    axec2_catalog['event_id'] = axec2_catalog.index
    print(f"Total AXEC2 events in raw catalog: {len(axec2_catalog)}")

    batch1_catalog = axec2_catalog.iloc[:BATCH_SIZE].copy()
    ...
    # Remove duplicate traces / events without exactly 3 traces (cells 31-32)
    events_to_remove = []
    ...
    # SNR + rectilinearity QC only (NOT incidence - that's the point of this rebuild)
    ...
    # Save as metadata CSV + one .mseed per event, matching the established convention
    # (see passing_waveforms_data_mldd_2015_2021_axec2/) - not a pickle.
