...
#
# # Master Shear-Wave Splitting Workflow for Axial Seamount — AXCC1
#
# Same pipeline as AXEC3, adapted for station AXCC1:
#   - Channels: HHE, HHN, HHZ  (AXEC3 uses EHE/EHN/EHZ)
#   - Events from 2015-03-01 through 2015-04-28 are excluded: the
#     instrument was unleveled during that period and those picks are
#     unreliable.
...
# Exclude the unleveled period (instrument was unleveled Mar 1 – Apr 28 2015 inclusive).
# UNLEVEL_END is set to Apr 29 00:00 UTC so that all Apr 28 events are excluded and
# data resumes cleanly on Apr 29.
UNLEVEL_START = pd.Timestamp('2015-03-01', tz='UTC')
UNLEVEL_END   = pd.Timestamp('2015-04-29', tz='UTC')
unlevel_mask = (axcc1_catalog['datetime'] >= UNLEVEL_START) & (axcc1_catalog['datetime'] < UNLEVEL_END)
n_excluded = unlevel_mask.sum()
axcc1_catalog = axcc1_catalog[~unlevel_mask].reset_index(drop=True)

print(f"Total AXCC1 events in catalog: {len(axcc1_catalog) + n_excluded}")
print(f"Excluded (unleveled instrument Mar 1 – Apr 28 2015): {n_excluded}")
print(f"Events remaining for analysis: {len(axcc1_catalog)}")
...
def get_station_traces_batch_axcc1(df, filename, starttime, endtime, station_id, batch_size=250):
    ...
    Mirrors the AXEC3 version but requests HH* instead of EH* channels.
    """
    from obspy.clients.fdsn import Client
    from obspy.core.utcdatetime import UTCDateTime
...
for i in range(total_batches):
    ...
    batch_filename = f'../data/axial_mldd_2015_2021_axcc1_batch_{i+1}'
...
passing_waveforms_dir = 'passing_waveforms_data_mldd_2015_2021_axcc1'
...
if os.path.exists(passing_waveforms_metadata):
    ...
    for i in range(total_batches):
        batch_filename = f'../data/axial_mldd_2015_2021_axcc1_batch_{i+1}.mseed'
...
passing_waveforms_reloaded = load_passing_waveforms(output_dir='passing_waveforms_data_mldd_2015_2021_axcc1')
...
for i, batch in enumerate(batched_waveforms):
    base_name = f'splitting_results_mldd_2015_2021_axcc1_batch_{i+1}.csv'
...
save_results_csv(final_results,
                 file_name=os.path.join(results_dir, 'splitting_results_mldd_2015_2021_axcc1_all_batches.csv'))
