...
Event: AXEC2, event_id 303_75564 (batch 303), origin 2015-04-23T14:30:16.873Z.

This is the SINGLE HIGHEST-dt OUTLIER in the strictest cumulative-filtered AXEC2 mfast dataset
(Q_w>=0.5, dt<=T_dom/2, phi_error<20 deg, dt_error<0.04 s -- N=12,550, see
rose_axec2_mfast_final_cumulative_filters.pdf): dt=0.22 s, more than 5x the population median
of 0.03 s. All other events in that filtered set top out at 0.12 s, so this is a genuine,
isolated outlier, not part of a continuum.

Production metadata (raw_axec2_all_batches_mfast_filters_data metadata + splitting results):
    quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    phi_error = 9 deg, dt_error = 0.005 s (both well inside phi_error<20 deg / dt_error<0.04 s)
    dt = 0.22 s
    depth = 0.548 km, SNR = 3.35 (just above the 2.0 production minimum), rectilinearity = 0.988
    chosen filter band (try_filters): 5-10 Hz, dominant period T_dom = 144.0 samples @ 200 Hz
        = 0.720 s (dt/T_dom = 0.306, i.e. dt is just under half T_dom -- the T_dom/2 = 0.360 s
        cycle-skip-risk threshold this event barely passes)
...
run_production_axec2_mfast_filters_all_batches.py), so for THIS event's T_dom=0.720 s the grid-
search window spans from -1.00*T_dom (=-0.720 s before S-pick) to +1.25*T_dom (=+0.900 s after
S-pick), a total length of 2.25*T_dom = 1.620 s. The single best-solution window is one specific
(start,end) pair drawn from the n_win x n_win grid inside that range and DBSCAN-clustered
(smallest-variance cluster, min combined dt+phi error observation) -- SWSPy does not retain
which individual (start_idx,end_idx) pair that observation came from, only its clustering
information (see plot_clustering_page), so the title below reports the full searched window
range rather than a fabricated single sub-window.

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch303_event_...mseed)
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_75564_outlier_dt_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch303_event_2015-04-23T14-30-16.873000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-04-23T14:30:16.873000Z')
S_ARRIVAL_TIME = 0.907   # seconds from origin
P_ARRIVAL_TIME = 0.447   # seconds from origin
BACK_AZIMUTH = 291.972782
INCIDENCE_PYKONAL_S = 21.190599   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 3.346294
RECTILINEARITY = 0.988063
DEPTH_KM = 0.548
...
def main():
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            best_win_info, title_suffix='0.22s Outlier',
        )
