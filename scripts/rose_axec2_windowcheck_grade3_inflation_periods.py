#!/usr/bin/env python3
"""
rose_axec2_windowcheck_grade3_inflation_periods.py

AXEC2-only rose plot, same data/filter as rose_7period_regions_windowcheck_grade3.py (windowcheck
...
AMOUNT, from the Central Caldera BOTPT de-tided seafloor-depth record
(bpr_inflation_periods.py's build_inflation_based_periods(), which bins on the 30-day rolling
mean of "m of uplift" referenced to zero at the 2015-05-17 post-eruption low point -- see that
module's docstring and the companion plot_bpr_detided_depth_and_inflation.py figure).

This directly compares the AXEC2 fast-direction rose across periods of equal PHYSICAL INFLATION
...
Produces: rose_axec2_windowcheck_grade3_inflation_periods.pdf (1 page, 1 row x 7 columns) -- a
NEW file, does not touch rose_7period_regions_windowcheck_grade3.pdf or any other existing
output.

Run with:
    python3 rose_axec2_windowcheck_grade3_inflation_periods.py
"""
...
OUT_PDF = os.path.join(HERE, 'rose_axec2_windowcheck_grade3_inflation_periods.pdf')
