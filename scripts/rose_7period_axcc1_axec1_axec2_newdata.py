#!/usr/bin/env python3
"""
rose_7period_axcc1_axec1_axec2_newdata.py

7-period fast-direction rose plots (3 rows: AXCC1, AXEC1, AXEC2 x 7 cols: eruption-relative
periods) built from the NEW mfast try_filters + max_t_shift_s=0.2s splitting results:

  AXCC1: both periods from mfast_maxdt_pipeline_transfer/
         splitting_results_AXCC1_{2015_2021,2022_2026}_all_batches.csv (new, complete)
  AXEC1: both periods from mfast_maxdt_pipeline_transfer/
         splitting_results_AXEC1_{2015_2021,2022_2026}_all_batches.csv (new, complete)
...
All three stations' new files already carry `dominant_period` (s) directly; the in-progress
AXEC2 2015-2021 batches only have `chosen_filter_dom_period_samples`, so dominant_period is
computed as chosen_filter_dom_period_samples/200.0 for that piece only (200 Hz sampling,
same convention used elsewhere in this repo, e.g. rose_axec2_batches1_153_original_vs_mfast_
vs_mfast_maxdt02.py).
...
Produces: rose_7period_axcc1_axec1_axec2_newdata.pdf  (5 pages)

Run with:
    python3 rose_7period_axcc1_axec1_axec2_newdata.py
"""
...
OUT_PDF = os.path.join(HERE, 'rose_7period_axcc1_axec1_axec2_newdata.pdf')
...
STATION_ORDER = ['AXCC1', 'AXEC1', 'AXEC2']
...
def make_rose_figure(dfs, time_periods, title):
    """3-row (station) x 7-col (time period) grid, count-weighted."""
...
def main():
    print('Loading new mfast max_dt=0.2s splitting results (AXCC1, AXEC1: complete; '
          'AXEC2 2015-2021: partial, in-progress re-run)...')
    raw = {}
    ...
    with PdfPages(OUT_PDF) as pdf:
        for tier_idx in range(len(TIERS)):
            ...
            fig = make_rose_figure(
                dfs, time_periods,
                title=(f'AXCC1 / AXEC1 / AXEC2 fast direction rose (new mfast max_dt=0.2s data) '
                       f'— {TIERS[tier_idx]}'))
