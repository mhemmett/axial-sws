#!/usr/bin/env python3
"""
traveltime_anisotropy_7period_6stations_newdata_phi10_diff_fractional.py

DIFFERENCE-FROM-PRE-ERUPTION variant of traveltime_anisotropy_7period_6stations_newdata_phi10.py:
identical data sources (6 stations: AXAS1, AXAS2, AXCC1, AXEC1, AXEC2, AXEC3), Tier 4 + phi<10deg
QC (quality>=0.5, dt<T_dom/2, phi_err<10deg, dt_err<0.05s), ray tracing, voxel grid, and
travel-time-weighted FRACTIONAL-anisotropy aggregation -- but using the FRACTIONAL shear-wave
anisotropy 'a' (solved from dt, T_travel via a^2 + 2x*a - 4 = 0 with x = 2*T_travel/dt; kept
only if 0<=a<=1) instead of the naive dt*100/T_travel ratio -- see
traveltime_anisotropy_7period_6stations_newdata_fractional.py's docstring for the full
derivation, not repeated here. The only difference from that sibling is what gets PLOTTED:

  Column 1 (Pre-eruption): the absolute travel-time-weighted mean % anisotropy per voxel,
    exactly as in the sibling plot (Blues colormap, 0 to 99th-percentile).
  Columns 2-7 (Syn-eruption ... Jul 2025): PER-VOXEL DIFFERENCE from the pre-eruption
    value at that same voxel (diverging colormap, RdBu_r, centered at 0 -- blue = anisotropy
    DECREASED vs. pre-eruption, red = INCREASED). A voxel's difference is only defined where
    BOTH the pre-eruption period AND that column's period have a real (coverage-gated) value
    at that voxel; voxels missing coverage in either period are left blank (NaN), not
    silently zero-filled.

Produces: traveltime_anisotropy_7period_6stations_newdata_phi10_diff_fractional.pdf (1 page)

Run with:
    python3 traveltime_anisotropy_7period_6stations_newdata_phi10_diff_fractional.py
"""
...
OUT_PDF = os.path.join(HERE, 'traveltime_anisotropy_7period_6stations_newdata_phi10_diff_fractional.pdf')
...
INCLUDED_STATIONS = ['AXAS1', 'AXAS2', 'AXCC1', 'AXEC1', 'AXEC2', 'AXEC3']
EXCLUDED_STATIONS = []
            ...
            if not (ez < 0 or ez > Z_MAX):
                try:
                    ...
                    if len(vcols) > 0:
                        ...
                        if T_travel > 0 and float(row.dt) > 0:
                            x_frac = 2.0 * T_travel / float(row.dt)
                            a_frac = -x_frac + math.sqrt(x_frac**2 + 4.0)
                            if 0.0 <= a_frac <= 1.0:
                                a_ray = a_frac * 100.0
...
def make_diff_page(results, title):
    """Column 0 (pre-eruption): absolute values, Blues. Columns 1+: per-voxel
    difference from column 0's value at that voxel, diverging RdBu_r centered
    at 0. A difference is only defined where BOTH the pre-eruption voxel AND
    that column's voxel are finite (coverage-gated) values."""
    pre_a = results[0]['a']

    diffs = []
    for r in results[1:]:
        d = r['a'] - pre_a   # NaN propagates automatically if either side is NaN
        diffs.append(d)
    ...
    panel_w = 1.8
    # Column 0 = pre-eruption (absolute) colorbar, on the LEFT; columns 1..n_per
    # = the panels themselves; last column = the difference colorbar, on the right.
    ...
    if last_h_abs:
        ...
        cb.set_label("Pre-eruption: mean fractional anisotropy 'a' per voxel (%)", fontsize=8)
        cb.ax.tick_params(labelsize=6)
        # Move ticks/label to the left side of the colorbar (it now sits left of
        # the panels, so default right-side placement would put them awkwardly
        # between the colorbar and the plots).
...
def main():
    ...
    title = (f"AXAS1/AXAS2/AXCC1/AXEC1/AXEC2/AXEC3 (new mfast max_dt=0.2s data) — pre-eruption fractional "
             f"anisotropy 'a' (%) + difference from pre-eruption (7-period)\n"
             f'a solved from x=2·T_travel/δt via a=-x+sqrt(x²+4), kept if 0<=a<=1; '
             f'quality>={QW_MIN}, dt<T_dom/2, phi_err<{PHI_ERR_MAX:.0f}deg, '
