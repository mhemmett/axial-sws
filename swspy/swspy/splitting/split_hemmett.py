...
def _phi_dt_grid_search(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, n_t_steps, n_angle_steps, Nbeg, Nend, fs, rotate_step_deg, 
    ...
    for a in range(Nbeg):
        ...
        print("Starting window: ", start_win_idx)
        for b in range(Nend):
            end_win_idx = win_end_idxs[b]
            print("Ending window:", end_win_idx)

            grid_search_idx = int(Nbeg*a + Nend*b)
            print("Grid search index: ", grid_search_idx)

            # *** FIX: Initialize invalid windows with NaN ***
            #if (end_win_idx - start_win_idx <= 3 or 
            #    start_win_idx < 0 or 
            #    end_win_idx >= len(data_arr_Q)):
                
            #    # Mark this entire window as invalid
            #    grid_search_results_all_win_EV[grid_search_idx, :, :] = np.nan
            #    if len(grid_search_results_all_win_XC) > 0:
            #        grid_search_results_all_win_XC[grid_search_idx, :, :] = np.nan
            #    continue
            ...
            for j in prange(n_angle_steps):
                ...
                theta_rot = -angle_shift_rad_curr
                 ...
                 # *** Add bounds check ***
                #if start_win_idx < 0 or end_win_idx > len(data_arr_T):
                #    continue
                ...
                for i in range(n_t_steps):
                    ...
                    #if not np.isfinite(rolled_rot_Q_curr).all():
                    #    continue

                    #if not np.isfinite(rolled_rot_T_curr).all():
                    #    continue

                    std_Q = np.std(rolled_rot_Q_curr)
                    std_T = np.std(rolled_rot_T_curr)

                    #if std_Q == 0.0 or std_T == 0.0:
                    #    continue
                    ...
                    cov_mat = np.cov(xy_arr)

                    #if not np.isfinite(cov_mat).all():
                    #    continue

                    lambdas_unsort = np.linalg.eigvalsh(cov_mat)
...
@jit(nopython=True, parallel=True)
def _phi_dt_grid_search_chat(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, n_t_steps, n_angle_steps, 
                        Nbeg, Nend, fs, rotate_step_deg, 
                        grid_search_results_all_win_EV, grid_search_results_all_win_XC):
    ...
    for a in range(Nbeg):
        ...
        for b in range(Nend):
            ...
            # Skip too-short windows
            if end_win_idx - start_win_idx <= 3:
                continue

            grid_search_idx = int(Nend*a + b)

            for j in prange(n_angle_steps):
                angle_shift_rad_curr = ((float(j) * rotate_step_deg) - 90.) * np.pi / 180. 
                theta_rot = -angle_shift_rad_curr

                rot_T_curr = (data_arr_T[start_win_idx:end_win_idx] * np.cos(theta_rot) -
                              data_arr_Q[start_win_idx:end_win_idx] * np.sin(theta_rot))
                rot_Q_curr = (data_arr_T[start_win_idx:end_win_idx] * np.sin(theta_rot) +
                              data_arr_Q[start_win_idx:end_win_idx] * np.cos(theta_rot))

                # Skip windows containing NaNs or infs
                if np.any(np.isnan(rot_Q_curr)) or np.any(np.isnan(rot_T_curr)):
                    continue
                if np.any(np.isinf(rot_Q_curr)) or np.any(np.isinf(rot_T_curr)):
                    continue

                for i in range(n_t_steps):
                    t_samp_shift_curr = int(i)
                    rolled_rot_Q_curr = np.roll(rot_Q_curr, +int(t_samp_shift_curr/2.))
                    rolled_rot_T_curr = np.roll(rot_T_curr, -int(t_samp_shift_curr/2.))

                    # Skip if zero std (cannot normalize) or NaNs
                    std_Q = np.std(rolled_rot_Q_curr)
                    std_T = np.std(rolled_rot_T_curr)
                    if std_Q == 0.0 or std_T == 0.0:
                        continue
                    if np.isnan(std_Q) or np.isnan(std_T):
                        continue

                    # Eigenvalue method
                    xy_arr = np.vstack((rolled_rot_T_curr, rolled_rot_Q_curr))
                    cov_matrix = np.cov(xy_arr)
                    if np.any(np.isnan(cov_matrix)) or np.any(np.isinf(cov_matrix)):
                        continue
                    lambdas_unsort = np.linalg.eigvalsh(cov_matrix)
                    lambda2 = np.min(lambdas_unsort)
                    lambda1 = np.max(lambdas_unsort)
                    if not np.isfinite(lambda1) or lambda1 <= 0.0 or not np.isfinite(lambda2):
                        continue
                    ...
                    if len(grid_search_results_all_win_XC) > 0:
                        ...
                        if denom == 0 or not np.isfinite(denom):
                            continue
                        grid_search_results_all_win_XC[grid_search_idx, i, j] = np.sum(
                            np.abs(rolled_rot_Q_curr * rolled_rot_T_curr) / denom
                        ) / float(len(rolled_rot_T_curr))
...
# Class from SWSPy by T. Hudson, adapted by Michael Hemmett
class create_splitting_object:
    ...
    def __init__(self, st, nonlinloc_event_path=None, event_uid=None, stations_in=[], origin_event_times=[], P_phase_arrival_times =[], S_phase_arrival_times=[], back_azis_all_stations=[], receiver_inc_angles_all_stations=[]):
        ...
        origin_event_times : list of obspy UTCDateTime objects
            List of origin times as UTCDateTime objects corresponding to order of values in the list <stations_in>.

        P_phase_arrival_times : list of obspy UTCDateTime objects
            List of P arrival times as UTCDateTime objects corresponding to order of
            <station_in>.
        ...
        self.max_t_shift_s = 0.4
        #self.n_win = 10

        # New, MFAST-like parameters for splitting:
        # Number of window start times
        self.Nbeg = 5
        # Number of window end times
        self.Nend = 20

        # Error in s-pick time is one standard deviation at the 95% confidence level
        # for the Baillard Kurtosis picker, used in Baillard's old catalog.
        # This statistic is reported in Figure 7, b-c of Wang et al., 2024.
        # There are separate pick errors reported for E, N components - 
        # we use the larger of the two for the conservative estimate.
        self.t_err = 0.0509

        # Define initially - will be updated on a per-event basis
        # Time step size between window start times
        #self.dTbeg = 0.2
        origin_time = origin_event_times[0]
        ...
        p_time = P_phase_arrival_times[0]
        s_time = S_phase_arrival_times[0]

        self.s_time = s_time

        p_time_absolute = p_time - origin_time
        s_time_absolute = s_time - origin_time

        # Compute S-P in seconds
        SP_time = s_time_absolute - p_time_absolute

        # Time step between window start times
        dTbeg_reduce = (SP_time / 2) / self.Nbeg
        self.dTbeg = min(dTbeg_reduce, 0.1)

        # Latest start time (closest to S, but before it)
        Tbeg0_reduce = (SP_time / 2) / (self.Nbeg + 1)

        if Tbeg0_reduce < self.t_err:
            self.Tbeg0 = -self.t_err

        elif self.t_err < Tbeg0_reduce < 0.1:

            self.Tbeg0 = -Tbeg0_reduce

        else:
            self.Tbeg0 = -0.1

        # Earliest start time (farthest before S)
        self.Tbeg1 = self.Tbeg0 - (self.Nbeg * self.dTbeg)

        #dTbeg_reduce = ((s_time_absolute - p_time_absolute) / 2) / (self.Nbeg)
        
        #if dTbeg_reduce < 0.1:
        #    self.dTbeg = dTbeg_reduce
        #else:
        #    self.dTbeg = 0.1

        # Latest time to start window -  closest to S-arrival time, but still before it
        #self.Tbeg0 = 0.3

        #Tbeg0_reduce = ((s_time_absolute - p_time_absolute) / 2) / (self.Nbeg + 1)

        # The latest a window can begin is one standard deviation of uncertainty in the S-pick time
        #if Tbeg0_reduce < self.t_err:
        #    self.Tbeg0 = S_phase_arrival_times[0] - self.t_err

        # The smaller of 0.1 or our defined (S-P)/2 component should be used as Tbeg0
        #elif self.t_err < Tbeg0_reduce < 0.1:
        #    self.Tbeg0 = S_phase_arrival_times[0] - Tbeg0_reduce
        
        # If what we defined above is bigger than 0.1, then we set it as 0.1
        #else:
        #    self.Tbeg0 = S_phase_arrival_times[0] - 0.1

        # Earliest time to start window - farthest before S-arrival time
        #self.Tbeg1 = (self.Tbeg0 - (self.Nbeg * self.dTbeg))
        ...
        # Window end times (AFTER S)
        self.Tend0 = self.Tmid / 1.2 + 0.15
        self.Tend1 = self.Tmid * 2.5 + 0.15

        # Earliest time to end window
        #self.Tend0 = S_phase_arrival_times[0] + self.Tmid/1.2 + 0.15
        # Latest time to end window
        #self.Tend1 = S_phase_arrival_times[0] + self.Tmid*2.5 + 0.15

        # Time step size between window end times
        self.dTend = (self.Tend1 - self.Tend0) / self.Nend
    ...
    #    """
    #    start_idxs_range = int((self.Tbeg1 - self.Tbeg0) * self.fs) 
    #    win_start_idxs = np.linspace(0, start_idxs_range, num=self.Nbeg, dtype=int) #np.random.randint(start_idxs_range, size=self.n_win)
    #    end_idxs_range = int((self.Tend1 - self.Tend0) * self.fs) 
    #    win_end_idxs = np.linspace(self.Tend0 * self.fs, end_idxs_range, num=self.Nend, dtype=int) #np.random.randint(end_idxs_start, end_idxs_start + start_idxs_range, size=self.n_win)
    ...
    #
    #
    #    end_min = int(self.Tend0 * self.fs)
    #    end_max = int(self.Tend1 * self.fs)
    #
    #    win_end_idxs = np.linspace(end_min,
    #                            end_max,
    #                            num=self.Nend,
    #                            dtype=int)
    #    
    ...
    #    
    #    # Start window bounds (before S)
    #    start_idx_min = int(self.Tbeg0 * self.fs)
    #    start_idx_max = int(self.Tbeg1 * self.fs)
    #
    #    # End window bounds (after S)
    #    end_idx_min = int(self.Tend0 * self.fs)
    #    end_idx_max = int(self.Tend1 * self.fs)
    #
    #    # --- Create evenly spaced start windows ---
    #    win_start_idxs = np.linspace(
    #        start_idx_min,
    #        start_idx_max,
    #        num=self.Nbeg,
    #        dtype=int
    #    )
    #
    #    # --- Create evenly spaced end windows ---
    #    win_end_idxs = np.linspace(
    #        end_idx_min,
    #        end_idx_max,
    #        num=self.Nend,
    #        dtype=int
    #    )
    #
    #    # Print out each combination of window start and end times
    #    print("Window start and end times (s):")
    #    for start_idx in win_start_idxs:
    #        for end_idx in win_end_idxs:
    #            start_time_s = start_idx / self.fs
    #            end_time_s = end_idx / self.fs
    #            print(f"Start: {start_time_s:.2f} s, End: {end_time_s:.2f} s")
    #            print(f"Window length: {end_time_s - start_time_s:.2f} s")
    #
    #    return win_start_idxs, win_end_idxs

    def _select_windows_chat(self):

        fs = self.sampling_rate

        # S pick absolute time
        S_time = self.s_time
        start_time = self.start_time

        # S index in trace
        S_idx = int((S_time - start_time) * fs)

        # --- Convert relative times to indices ---
        start_idx_min = S_idx + int(self.Tbeg1 * fs)
        start_idx_max = S_idx + int(self.Tbeg0 * fs)

        end_idx_min = S_idx + int(self.Tend0 * fs)
        end_idx_max = S_idx + int(self.Tend1 * fs)
    ...
    def _select_windows(self):
        ...
        # Ensure S_idx is valid
        trace_length = len(self.st[0].data)
        if S_idx < 0 or S_idx >= trace_length:
            raise ValueError(f"S arrival at index {S_idx} is outside trace bounds [0, {trace_length}]")
        ...
        # *** CRITICAL FIX: Clip indices to valid trace bounds ***
        #start_idx_min = max(0, start_idx_min)
        #start_idx_max = max(0, start_idx_max)
        #end_idx_min = min(trace_length - 1, max(start_idx_max + 10, end_idx_min))  # Ensure min window length
        #end_idx_max = min(trace_length - 1, end_idx_max)
        
        # Validate window ranges
        if start_idx_max <= start_idx_min:
            raise ValueError(f"Invalid start window range: [{start_idx_min}, {start_idx_max}]")
        if end_idx_max <= end_idx_min:
            raise ValueError(f"Invalid end window range: [{end_idx_min}, {end_idx_max}]")
        
        # --- Create windows ---
        win_start_idxs = np.linspace(start_idx_min, start_idx_max, num=self.Nbeg, dtype=int)
        win_end_idxs = np.linspace(end_idx_min, end_idx_max, num=self.Nend, dtype=int)

        print("Window start index: ", win_start_idxs)
        print("Window end index: ", win_end_idxs)
        
        # Additional validation: ensure all windows have minimum length
        #min_window_length = int(0.05 * fs)  # e.g., 50ms minimum
        #for start_idx in win_start_idxs:
        #    for end_idx in win_end_idxs:
        #        if end_idx - start_idx < min_window_length:
        #            raise ValueError(f"Window [{start_idx}, {end_idx}] is too short (< {min_window_length} samples)")
    ...
    def _calc_splitting_eig_val_method(self, data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, sws_method="EV", n_layers=1, layer_1_dts_phis=None, layer_2_dts_phis=None, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        # The number of time steps is maximum time shift times sampling rate
        # With different time steps for start and end windows, this is calculated to be the sum of the two (?)
        ...
        lags_labels = np.arange(0., n_t_steps, 1) / fs 
        phis_labels = np.arange(-90, 90 + rotate_step_deg, rotate_step_deg)
        grid_search_results_all_win_EV = np.zeros((Nbeg*Nend, n_t_steps, n_angle_steps), dtype=float)
        grid_search_results_all_win_XC = np.zeros((Nbeg*Nend, n_t_steps, n_angle_steps), dtype=float)
        # if n_layers == 1:
        ...
        if n_layers == 1:
            try:
                grid_search_results_all_win_EV, grid_search_results_all_win_XC = _phi_dt_grid_search(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, n_t_steps, n_angle_steps, Nbeg, Nend, fs, rotate_step_deg, grid_search_results_all_win_EV, grid_search_results_all_win_XC)
            except:
                raise ValueError("_phi_dt_grid_search failed!") 
        elif n_layers == 2:
            ...
            grid_search_results_all_win_EV, grid_search_results_all_win_XC = _phi_dt_grid_search_direct_multi_layer(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, 
                                                                                                        n_t_steps, Nbeg, Nend, fs, rotate_step_deg, 
                                                                                                        grid_search_results_all_win_EV, grid_search_results_all_win_XC, 
    ...
    def _get_phi_and_lag_errors_single_win_orig(self, phi_dt_single_win, lags_labels, phis_labels, tr_for_dof, interp_fac=1):
        """
        ...
        print("Phi err: ", phi_err)
        print("Lag err: ", lag_err)
    ...
    def _get_phi_and_lag_errors_single_win(self, phi_dt_single_win, lags_labels, phis_labels, tr_for_dof, interp_fac=1):
        """
        Compute the error in phi (fast direction) and lag (dt) for a single grid-search window,
        following the Silver and Chan (1991) method using an F-test confidence interval.

        Parameters
        ----------
        phi_dt_single_win : 2D array
            Grid-search eigenvalue ratio (lambda2/lambda1) for this window [lags x phis].
        lags_labels : 1D array
            Lag (dt) values corresponding to grid-search.
        phis_labels : 1D array
            Fast direction phi values corresponding to grid-search.
        tr_for_dof : Trace
            Trace used to calculate degrees of freedom for F-test.
        interp_fac : int
            Factor to interpolate grid for finer resolution. Default = 1 (no interpolation).

        Returns
        -------
        phi_err : float
            Estimated error in phi (degrees).
        lag_err : float
            Estimated error in lag (seconds).
        """

        # --- 1. Interpolate the error surface if requested ---
        if interp_fac > 1:
            from scipy import interpolate
            x_tmp = lags_labels
            y_tmp = phis_labels
            interp_spline = interpolate.RectBivariateSpline(x_tmp, y_tmp, phi_dt_single_win)
            lags_labels = np.linspace(x_tmp[0], x_tmp[-1], interp_fac*len(x_tmp))
            phis_labels = np.linspace(y_tmp[0], y_tmp[-1], interp_fac*len(y_tmp))
            error_surf = interp_spline(lags_labels, phis_labels)
        else:
            error_surf = phi_dt_single_win
        ...
        # --- 3. Compute confidence bound (F-test, 3-sigma default) ---
        conf_bound = ftest(error_surf, dof, alpha=0.003, k=2)
        conf_mask = error_surf <= conf_bound

        # --- 4. Compute lag (dt) error ---
        lag_mask = conf_mask.any(axis=1)  # True if any phi in this lag passes
        true_idxs = np.where(lag_mask)[0]
        ...
        if len(true_idxs) == 0:
            ...
            lag_step_s = lags_labels[1] - lags_labels[0]
            lag_err = (true_idxs[-1] - true_idxs[0] + 1) * lag_step_s * 0.5  # half-width of box

        # --- 5. Compute phi error ---
        phi_mask = conf_mask.any(axis=0)  # True if any lag in this phi passes

        if np.any(phi_mask):
            # Handle cyclic nature of phi (-90 to 90)
            phi_mask_cyclic = np.concatenate([phi_mask, phi_mask, phi_mask])
            true_runs = np.where(phi_mask_cyclic)[0]
            # Maximum gap between consecutive True values
            max_false_len = np.diff(true_runs).max() - 1 if len(true_runs) > 1 else 0
            # Shortest segment covering all True values
            max_true_len = len(phi_mask) - max_false_len
            phi_step_deg = phis_labels[1] - phis_labels[0]
            phi_err = max_true_len * phi_step_deg * 0.5  # half-width
        else:
            phi_err = 0.0

        # --- 6. Optional debug prints ---
        # print(f"Phi err: {phi_err:.2f} deg, Lag err: {lag_err:.4f} s")

        return phi_err, lag_err

    def _get_phi_and_lag_errors_orig(self, grid_search_results_all_win, tr_for_dof, interp_fac=1):
    ...
    def _get_phi_and_lag_errors(self, grid_search_results_all_win, tr_for_dof, interp_fac=1):
        ...
        for i in range(grid_search_results_all_win.shape[0]):
            ...
            # *** FIX 1: Check if window has valid (non-zero, non-inf, non-nan) values ***
            if not np.any(np.isfinite(grid_search_result_curr_win)):
                lags[i] = np.nan
                phis[i] = np.nan
                lag_errs[i] = np.nan
                phi_errs[i] = np.nan
                min_eig_ratios[i] = np.nan
                print(f"Warning: Window {i} has no finite values")
                continue
            
            if np.all(grid_search_result_curr_win == 0):
                lags[i] = np.nan
                phis[i] = np.nan
                lag_errs[i] = np.nan
                phi_errs[i] = np.nan
                min_eig_ratios[i] = np.nan
                print(f"Warning: Window {i} is all zeros")
                continue
            
            # *** FIX 2: Use only finite values to find minimum ***
            finite_mask = np.isfinite(grid_search_result_curr_win)
            ...
            min_val = np.min(grid_search_result_curr_win[finite_mask])
            
            # *** FIX 3: Check that minimum is reasonable ***
            if not np.isfinite(min_val) or min_val == 0:
                ...
                phi_errs[i] = np.nan
                min_eig_ratios[i] = np.nan
                print(f"Warning: Window {i} has invalid minimum: {min_val}")
                continue
            
            min_idxs = np.where(grid_search_result_curr_win == min_val)
            
            # *** FIX 4: Check that we found valid indices ***
            if len(min_idxs[0]) == 0:
                lags[i] = np.nan
                phis[i] = np.nan
                lag_errs[i] = np.nan
                phi_errs[i] = np.nan
                min_eig_ratios[i] = np.nan
                print(f"Warning: Window {i} could not find minimum indices")
                continue
            ...
            # Get associated error (from f-test with 95% confidence interval):
            try:
                phi_errs[i], lag_errs[i] = self._get_phi_and_lag_errors_single_win(
                    grid_search_result_curr_win, self.lags_labels, self.phis_labels, 
                    tr_for_dof, interp_fac=interp_fac)
            except Exception as e:
                print(f"Warning: Error calculating errors for window {i}: {e}")
                phi_errs[i] = np.nan
                lag_errs[i] = np.nan
            
            # And get min_eig_ratio:
            min_eig_ratios[i] = min_val

        return phis, lags, phi_errs, lag_errs, min_eig_ratios

    # Original clustering function
    def _sws_win_clustering_orig(self, lags, phis, lag_errs, phi_errs, min_eig_ratios=None, method="dbscan", return_clusters_data=False):
        ...
        # min_samples=int(np.sqrt(len(lags))
        ...
        if n_clusters > 0:
            ...
            print("Cluster var 0s: ", cluster_var_0s)
            ...
            print("Smallest var cluster: ", smallest_var_cluster)
    ...
    def _sws_win_clustering(self, lags, phis, lag_errs, phi_errs, min_eig_ratios=None, method="dbscan", return_clusters_data=False):
        """
        Perform SWS clustering of phis and lags using normalized coordinates and DBSCAN.
        Handles cyclic phi nature and removes NaN windows before clustering.
        """
        import numpy as np
        from sklearn.cluster import DBSCAN

        # Remove NaN or infinite entries
        valid_mask = (~np.isnan(lags)) & (~np.isnan(phis)) & (~np.isnan(lag_errs)) & (~np.isnan(phi_errs))
        lags = lags[valid_mask]
        phis = phis[valid_mask]
        lag_errs = lag_errs[valid_mask]
        phi_errs = phi_errs[valid_mask]
        if min_eig_ratios is not None:
            min_eig_ratios = min_eig_ratios[valid_mask]

        if len(lags) == 0:
            print("Warning: no   removing NaNs.")
            if return_clusters_data:
                return (None, None, None, None, None, None, None) if min_eig_ratios is not None else (None, None, None, None, None, None)
            else:
                return (None, None, None, None, None) if min_eig_ratios is not None else (None, None, None, None)

        # Normalize lags and convert phis into cyclic DBSCAN coordinates
        norm_lags = lags / np.max(lags)

        x = norm_lags * np.cos(2 * np.deg2rad(phis))
        y = norm_lags * np.sin(2 * np.deg2rad(phis))
        samples_new_coords = np.column_stack((x, y))

        # Remove any rows with NaN (just in case)
        valid_samples_mask = ~np.any(np.isnan(samples_new_coords), axis=1)
        samples_new_coords = samples_new_coords[valid_samples_mask]
        lags = lags[valid_samples_mask]
        phis = phis[valid_samples_mask]
        lag_errs = lag_errs[valid_samples_mask]
        phi_errs = phi_errs[valid_samples_mask]
        if min_eig_ratios is not None:
            min_eig_ratios = min_eig_ratios[valid_samples_mask]

        # Now run DBSCAN safely
        if len(samples_new_coords) == 0:
            print("Warning: No valid windows after removing NaNs for clustering.")
            # return None's safely as before

        # DBSCAN clustering
        db = DBSCAN(eps=0.25, min_samples=5)
        clustering = db.fit(samples_new_coords)
        labels = clustering.labels_

        # Identify clusters (ignore noise, label = -1)
        unique_labels = [l for l in set(labels) if l != -1]
        n_clusters = len(unique_labels)

        if n_clusters == 0:
            print("Warning: Failed to cluster for current receiver. Skipping receiver.")
            if return_clusters_data:
                return (None, None, None, None, None, None, None) if min_eig_ratios is not None else (None, None, None, None, None, None)
            else:
                return (None, None, None, None, None) if min_eig_ratios is not None else (None, None, None, None)
        ...
        # Compute cluster variance in normalized DBSCAN coordinates
        for idx, lbl in enumerate(unique_labels):
            cluster_idxs = np.where(labels == lbl)[0]
            clusters_dict[str(idx)] = {
                'lags': lags[cluster_idxs],
                'lag_errs': lag_errs[cluster_idxs],
                'phis': phis[cluster_idxs],
                'phi_errs': phi_errs[cluster_idxs]
            }
            if min_eig_ratios is not None:
                clusters_dict[str(idx)]['min_eig_ratios'] = min_eig_ratios[cluster_idxs]

            # Variance in normalized x,y coords
            cluster_vars[idx] = np.mean((x[cluster_idxs] - np.mean(x[cluster_idxs]))**2 +
                                        (y[cluster_idxs] - np.mean(y[cluster_idxs]))**2)
            # Variance from measurement errors
            data_vars[idx] = (1 / np.sum(1 / (lag_errs[cluster_idxs]**2))) + (1 / np.sum(1 / (phi_errs[cluster_idxs]**2)))

        cluster_var_0s = np.maximum(cluster_vars, data_vars)
        min_var_idx = np.argmin(cluster_var_0s)
        smallest_var_cluster = clusters_dict[str(min_var_idx)]

        # Pick observation with smallest combined dt + phi variance
        cluster_vars_tmp = smallest_var_cluster['lag_errs']**2 + smallest_var_cluster['phi_errs']**2
        opt_obs_idx = np.argmin(cluster_vars_tmp)
    ...
    def _get_uncorr_and_corr_waveforms(self, station):
        ...
        st_ZNE_curr.trim(starttime=arrival_time_curr + self.Tbeg1,
                            endtime=arrival_time_curr + self.Tend1)
    ...
    def _get_uncorr_and_corr_waveforms_multi_layer(self, station):
        ...
        if self.nonlinloc_event_path:
            ...
            arrival_time_curr = self.S_phase_arrival_times[station_idx_tmp]
        st_ZNE_curr.trim(starttime=arrival_time_curr - self.Tbeg0,
                            endtime=arrival_time_curr + self.Tend1)
    ...
    def perform_sws_analysis(self, coord_system="ZNE", sws_method="EV", return_clusters_data=True, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            if self.nonlinloc_event_path:
                ...
                arrival_time_curr = self.S_phase_arrival_times[station_idx_tmp]
            st_LQT_curr.trim(starttime=arrival_time_curr + self.Tbeg1,
                                endtime=arrival_time_curr + self.Tend1)
            st_ZNE_curr.trim(starttime=arrival_time_curr + self.Tbeg1,
                                endtime=arrival_time_curr + self.Tend1)
            try:
            ...
            if self.sws_method == "EV":
                ...
                print("EV results shape:", grid_search_results_all_win_EV.shape)
                print("Any NaNs?", np.isnan(grid_search_results_all_win_EV).any())
            ...
            print('---------------')
            print('Grid search results: ', grid_search_results_all_win_EV)
            print()
            # After the grid search calculations, add this NaN filtering step:
            # Filter out windows with NaN values in grid search results
            valid_win_mask = ~np.isnan(grid_search_results_all_win_EV).any(axis=(1, 2))

            if np.sum(valid_win_mask) == 0:
                print("Warning: No valid windows after removing NaNs. Skipping station.")
                continue

            # Apply mask to filter results
            grid_search_results_all_win_EV = grid_search_results_all_win_EV[valid_win_mask]

            # If using cross-correlation method as well
            if self.sws_method == "EV_and_XC":
                grid_search_results_all_win_XC = grid_search_results_all_win_XC[valid_win_mask]

            print(f"Valid windows after NaN filtering: {np.sum(valid_win_mask)}/{len(valid_win_mask)}")
            ...
            try:
                print("Phis labels: ", phis_labels)
                print("Opt phi: ", opt_phi)

                print()
                print("Lags labels: ", lags_labels)
                print("Opt lag: ", opt_lag)
    ...
    def perform_sws_analysis_multi_layer(self, coord_system="ZNE", multi_layer_method="explicit", num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            if multi_layer_method == "explicit":
                ...
                partition_idx = round( (self.Tbeg0 + dt_app) * self.fs )
    ...
    def plot(self, outdir=None, suppress_direct_plotting=False):
        ...
        for station in self.stations_list:
            ...
            # Plot data:
            # Setup figure:
            fig = plt.figure(constrained_layout=True, figsize=(24,20))
            if suppress_direct_plotting:
                plt.ion()
            gs = fig.add_gridspec(6, 5, wspace=0.3, hspace=0.3) # no. rows, no. cols
            ...
            if self.clustering_info:
                ...
                # added by me
                phi_dt_scatter_ax = fig.add_subplot(gs[5, 0:2])
            ...
            if self.clustering_info:
                ...
                if "layer1" in self.clustering_info[station]:
                    ...
                    for layer_id, color, label in [('layer1', '#1E69A9', 'Layer 1'), 
                        ...
                        phi_dt_scatter_ax.errorbar(layer_dts, layer_phis, 
                                                ...
                                                label=label, color='black', capsize=2)
                        
                    # Plot optimal values for both layers
                    phi_dt_scatter_ax.plot(dt_layer1, phi_layer1, 's', markersize=2.5, 
                                        markeredgecolor='black', markeredgewidth=0.5, 
                                        label='Optimal Layer 1', zorder=10)
                    phi_dt_scatter_ax.plot(dt_layer2, phi_layer2, 's', markersize=2.5,
                                        markeredgecolor='black', markeredgewidth=0.5,
                    ...
                    # Plot optimal value
                    phi_dt_scatter_ax.plot(dt_curr, phi_curr, 's', markersize=2.5,
                                        markeredgecolor='black', markeredgewidth=0.5,
                ...
                phi_dt_scatter_ax.legend(fontsize='x-small', loc='best')
                phi_dt_scatter_ax.grid(True, alpha=0.3)
                phi_dt_scatter_ax.set_title('Window Splitting Distribution', fontsize='small')
