"""
Temporal moving-window density histogram of FRACTIONAL delay time (dt divided
by the S-wave raypath length, in s/km) for all 6 stations, one row per
station, analogous to figure_ec2_multipanel_temporal.py's panel D/E but:

  1. Plots dt/r_km (s/km) instead of raw dt (s) - normalizing each
     measurement by its own raypath length (traced through the Baillard Vs
     model from event hypocenter to station), so a change in raypath
     geometry/length between periods doesn't masquerade as a change in
     anisotropy the way raw dt can.
...
Produces: figure_all_stations_temporal_fractional_dt.pdf

Run with:
    python3 figure_all_stations_temporal_fractional_dt.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_all_stations_temporal_fractional_dt.pdf')
...
print('\nTracing rays to compute raypath length (r_km) per event...')
for sta in STATIONS:
    df = station_events[sta]
    x, y = ll2xy(df['lat'].values, df['lon'].values)
    r_km = np.full(len(df), np.nan)
    t0 = time.time()
    ...
    df['r_km'] = r_km
    n_valid = np.isfinite(r_km).sum()
    print(f'  {sta}: {n_valid:,}/{len(df):,} events traced successfully ({time.time()-t0:.0f}s)')
    df['frac_dt'] = df['dt'] / df['r_km']
    station_events[sta] = df.dropna(subset=['frac_dt'])
    station_events[sta] = station_events[sta][np.isfinite(station_events[sta]['frac_dt'])]

# ── Shared y-axis range across all stations ──────────────────────────────────

all_frac = np.concatenate([station_events[sta]['frac_dt'].values for sta in STATIONS])
Y_END = float(np.nanpercentile(all_frac, 99))
print(f'\nShared y-axis range: 0-{Y_END:.4f} s/km (99th pct across all stations)')
...
def draw_frac_panel(ax, df, x_start, x_end, x_width, x_over, title, show_legend=False):
    ...
    ax.set_ylabel(r'$\delta t / r$ [s/km]', fontsize=9, fontweight='bold')
...
def main():
    ...
    for i, sta in enumerate(STATIONS):
        ...
        im = draw_frac_panel(ax_full, df, t_min, t_max, x_width=3 * 24 * 3600, x_over=0.5,
        ...
        im_z = draw_frac_panel(ax_zoom, df, zoom_start, zoom_end, x_width=6 * 3600, x_over=0.85,
    ...
    fig.suptitle('Fractional delay time ($\\delta t$ / raypath length) over time, all stations\n'
