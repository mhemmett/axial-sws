...
figure of the production SWS pipeline -- companion to figure_axec1_event_23919_high_dt_diagnostics.py
...
Event: AXEC1, event_id 41085 (row index in the per-station-reset data/mldd_catalog_2015_2021.csv,
batch 165 of data/axial_mldd_2015_2021_axec1_batch_*.mseed), origin 2015-03-24T02:02:46.325Z.

Selected from lqt_pykonal_combined_results/splitting_results_AXEC1_2015_2021_all_batches_enriched.csv
(the AXEC1 LQT + PyKonal-FMM production run) as a high-quality, low-delay-time event:
    quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    phi_error = 9 deg, dt_error = 0.005 s (both well inside the production
        phi_error<20 deg / dt_error<0.04 s gate used throughout this repo, e.g.
        lqt_pykonal_percent_anisotropy_quality_pages.py)
    dt = 0.02 s (< 0.24 s -- the dt threshold used to separate "high dt" from "low dt"
...
against the high-dt companion event (23919) and against this event: re-running
create_splitting_analysis with n_win=7 reproduces the production dt exactly for both (0.29 s and
0.02 s respectively) and the production fast-direction phi to ~1 deg once phi_from_Q is rotated
into the geographic frame (phi_from_Q + back_azimuth, wrapped to +/-90 deg) -- for this event,
-6.6 deg vs. the stored -7.6 deg. Q_w comes out 1.000 here, matching the stored production value
exactly; small residual differences on the companion event likely come from small differences in
exactly how the production pipeline's organize_waveform_data() trimmed the raw batch waveform vs.
the ad hoc extraction below.
...
this trace's actual P/S timing (P at 1.5 s into the trace, S at 1.91 s), not the snr()/
...
Run with: python3 figure_axec1_event_41085_low_dt_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec1_event_41085_low_dt_diagnostics.pdf')
...
STATION = 'AXEC1'
EVENT_ID = 41085
BATCH_NUM = 165   # EVENT_ID // 250 + 1
WAVEFORM_BATCH_PATH = os.path.join(HERE, '..', 'data',
                                   f'axial_mldd_2015_2021_axec1_batch_{BATCH_NUM}.mseed')
ORIGIN_TIME = UTCDateTime('2015-03-24T02:02:46.325000Z')
P_ARRIVAL_TIME = 0.445   # seconds from origin (data/mldd_catalog_2015_2021.csv arrival_time_p)
S_ARRIVAL_TIME = 0.855   # seconds from origin (arrival_time_s)
BACK_AZIMUTH = 255.374909
INCIDENCE_PYKONAL_S = 5.628120   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 17.96   # computed fresh, see docstring
RECTILINEARITY = 0.968   # computed fresh, see docstring
