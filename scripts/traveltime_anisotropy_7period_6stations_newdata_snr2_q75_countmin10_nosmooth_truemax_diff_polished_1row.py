#!/usr/bin/env python3
"""
traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_nosmooth_truemax_diff_polished_1row.py

UNSMOOTHED variant of
traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff_polished_1row.pdf
(same data/filter/cached ray tracing, COUNT_MIN=10, single row, true-max colorbars, title,
legend, column-header/N= typography, and gap+colorbar layout -- see that script's docstring,
not repeated here), with TWO changes:

  1. NO Gaussian smoothing is applied before contouring: the raw per-voxel nanmean array is
     plotted directly (still masked to only the coverage-gated voxels), and the true-max
     colorbars are computed from that raw (unsmoothed) plotted field.
  2. The single row now spans the FULL 0.0-Z_MAX (4.0) km grid depth -- i.e. every
     observation that passes the filter/ray-trace-eligibility test, not just the
     0.0-2.25 km subset used in the earlier smoothed 1-row figure.

Produces:
    traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_nosmooth_truemax_diff_polished_1row.pdf
(1 page, 1 row) -- a NEW file, does not touch any existing output.

Run with:
    python3 traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_nosmooth_truemax_diff_polished_1row.py
"""
...
# 0.0-Z_MAX km grid depth (not just the 0.0-2.25 km range used in the 3-row/earlier 1-row
# figures), so every observation surviving the filter is represented, not a depth-truncated
# subset.
DEPTH_ROWS_ALL = [
...
def _masked_slices_nosmooth(results, depth_rows, count_min_display):
    """Same shape as the smoothed sibling's _smoothed_masked_slices, but skips
    gaussian_filter entirely -- the raw nanmean depth slice is what gets stored/plotted."""
    fields = {}
    for ri, (iz_weights, zlbl, z0) in enumerate(depth_rows):
        for pi, res in enumerate(results):
            ...
            if msk.sum() >= count_min_display:
                fields[(pi, ri)] = (sl, msk)   # NO gaussian_filter, NO nan_to_num
            else:
...
def make_diff_page(results, period_labels, title, depth_rows, fields):
    ...
    # needs to be reserved explicitly. Both reduced from the original draft to tighten the
...
def main():
    ...
    print(f'Computing UNSMOOTHED masked field (single row, all depths 0-{T.Z_MAX:.1f}km combined)...')
    fields = _masked_slices_nosmooth(results, DEPTH_ROWS_ALL, count_min_display=T.COUNT_MIN)
