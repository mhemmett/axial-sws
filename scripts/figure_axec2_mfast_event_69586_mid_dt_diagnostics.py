...
figure_axec2_mfast_event_9788_high_dt_diagnostics.py (high dt) and
figure_axec2_mfast_event_15727_low_dt_diagnostics.py (low dt). Structure replicated exactly
from figure_axec2_event_33223_diagnostics.py, the original fixed-5-40Hz-filter AXEC2 template,
...
Event: AXEC2, event_id 279_69586 (batch 279), origin 2015-04-15T05:44:24.260Z.

Selected from production_axec2_mfast_filters_lqt_pykonal_results/ (the new mfast try_filters
AXEC2 run) as a high-quality, MID-delay-time event (0.15 < dt < 0.24 s, the gap between the
low-dt and high-dt companion events) where the AD HOC RE-RUN (create_splitting_analysis +
perform_sws_analysis, same as this script's own pipeline) itself yields Q_w >= 0.7 -- not just
the stored production CSV's Q_w, which can differ from a re-run's Q_w due to small waveform-
trimming differences (see figure_axec1_event_23919 docstring). Found via
find_mfast_diagnostic_events.py, which re-runs each QC-passing candidate and keeps the first
whose RE-RUN also clears the gate:
    RE-RUN: quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    RE-RUN: phi_error = 8 deg, dt_error = 0.005 s (both well inside phi_error<20 deg / dt_error<0.04 s)
    RE-RUN: dt = 0.19 s (mid-range, between the low-dt and high-dt companion events)
    chosen filter band (try_filters): 5-15 Hz
    pre-eruption (< 2015-04-24 06:00 UTC)

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch279_event_...mseed)
was already detrended/tapered/bandpass-filtered to this event's own 5-15 Hz optimal band when
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_69586_mid_dt_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch279_event_2015-04-15T05-44-24.260000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-04-15T05:44:24.260000Z')
S_ARRIVAL_TIME = 1.04    # seconds from origin
P_ARRIVAL_TIME = 0.65    # seconds from origin
BACK_AZIMUTH = 176.79590632491443
INCIDENCE_PYKONAL_S = 16.30165935165371   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 6.760223315912614
RECTILINEARITY = 0.8837854189404837
CHOSEN_FILTER_BAND = (5.0, 15.0)   # try_filters optimal band for this event
