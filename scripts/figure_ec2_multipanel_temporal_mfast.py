"""
Multipanel summary figure: AXEC2 shear-wave splitting measurements over time
(2015-2026), for building a manuscript figure. One-off script, not part of the
pipeline. Reuses the exact filtering/colorscale/period conventions already
established for the production AXEC2 rose and temporal-histogram plots.

Variant of figure_ec2_multipanel_temporal.py that swaps the 2015-2021 portion
of AXEC2's data for the NEW mfast try_filters results (whatever batches have
completed so far) instead of the old fixed 5-40 Hz results. The 2022-2026
portion is unchanged (old results; no mfast pipeline exists for that catalog).

  - Data: production_axec2_mfast_filters_lqt_pykonal_results/ (2015-2021, new
    mfast try_filters batches) + lqt_pykonal_combined_results/
    splitting_results_AXEC2_2022_2026_all_batches_enriched.csv (old, unchanged)
...
Panels:
  A. Single-row, 7-period (pre/syn/5 post-eruption bins) fast-direction rose
     ...
     restricted to AXEC2 only (one row instead of 6).
...
Run with: python3 figure_ec2_multipanel_temporal.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_ec2_multipanel_temporal_mfast.pdf')
...
def load_axec2_enriched():
    import glob
    batch_files = glob.glob(os.path.join(
        AXEC2_MFAST_DIR, 'splitting_results_mldd_2015_2021_axec2_mfast_filters_batch_*.csv'))
    d1_parts = [pd.read_csv(f) for f in batch_files]
    d1_parts = [d for d in d1_parts if len(d) > 0]
    d1 = pd.concat(d1_parts, ignore_index=True)
    d2 = pd.read_csv(os.path.join(DATA_DIR, 'splitting_results_AXEC2_2022_2026_all_batches_enriched.csv'))
...
def main():
    print('Loading AXEC2 results (2015-2021: new mfast try_filters, 2022-2026: old)...')
    raw = load_axec2_enriched()
    ...
    fig.suptitle('EC2 (mfast try_filters): Shear-wave Splitting Measurements Over Time', fontsize=17 * FONT_SCALE,
