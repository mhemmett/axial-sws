...
class create_splitting_object:
    ...
    def __init__(self, st, nonlinloc_event_path=None, event_uid=None, stations_in=[], origin_times=[], P_phase_arrival_times =[], S_phase_arrival_times=[], back_azis_all_stations=[], receiver_inc_angles_all_stations=[]):
        ...
        origin_times : list of obspy UTCDateTime objects
        ...
        self.dTbeg = 0.2
        # Earliest time to start window
        self.Tbeg0 = 0.3
        # Latest time to start window
        ...
        # Total shift in start windows times
        self.max_start_win_shift = self.dTbeg * self.Nbeg
        # Total shift in end windows times
        self.max_end_win_shift = self.dTend * self.Nend
    ...
    def _calc_splitting_eig_val_method(self, data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, sws_method="EV", n_layers=1, layer_1_dts_phis=None, layer_2_dts_phis=None, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        # Setup parameters:
        n_t_steps_beg = int(self.max_start_win_shift * self.fs)
        n_t_steps_end = int(self.max_end_win_shift * self.fs)
        ...
        n_t_steps = n_t_steps_beg + n_t_steps_end
    ...
    def perform_sws_analysis(self, coord_system="ZNE", sws_method="EV", return_clusters_data=True, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            if self.nonlinloc_event_path:
                ...
                arrival_time_curr = self.S_phase_arrival_times[station_idx_tmp]
            st_LQT_curr.trim(starttime=arrival_time_curr - self.Tbeg0,
                                endtime=arrival_time_curr + self.Tend1)
            st_ZNE_curr.trim(starttime=arrival_time_curr - self.Tbeg0,
                                endtime=arrival_time_curr + self.Tend1)
            try:
