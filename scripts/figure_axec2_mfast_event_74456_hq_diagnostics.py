"""
Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected as
a high-quality "textbook" example: high rectilinearity, high SNR, low S-wave incidence angle,
and phi_error < 10 deg on re-run. Structure replicated exactly from
figure_axec2_mfast_event_89944_high_dt_diagnostics.py (itself replicated from
figure_axec2_event_33223_diagnostics.py, the original fixed-5-40Hz-filter AXEC2 template),
EXCEPT the waveform here was pre-filtered by try_filters (mfast_filters_functions.py) to its
own per-event optimal band, not a fixed 5-40 Hz.

Event: AXEC2, event_id 298_74456 (batch 298), origin 2015-04-21T18:23:42.668Z (pre-eruption).

Selection: searched production_axec2_mfast_filters_maxdt02_lqt_pykonal_results/ (the COMPLETED
maxdt02 AXEC2 pre-eruption rerun, all 497 batches), restricted to pre-eruption events (before
2015-04-24 06:00 UTC) passing quality>=0.5, phi_error<10 deg, dt_error<0.05s, then ranked by
rectilinearity_jurkevics (desc), snr_horizontal (desc), incidence_pykonal_s (asc). This event
tops that ranking among candidates with rectilinearity>0.9, snr>10, incidence<15 deg:
    Production: quality=1.000, phi_error=7 deg, dt_error=0.005s, snr_horizontal=23.28,
                rectilinearity_jurkevics=0.9992, incidence_pykonal_s=14.62 deg, dt=0.03s
    RE-RUN (create_splitting_analysis + perform_sws_analysis, same pipeline as this script):
                phi=14.0 deg, dt=0.030s, phi_error=6 deg, dt_error=0.005s, Q_w=1.000
                (re-run phi_error=6 deg confirms < 10 deg threshold; matches the production
                CSV's stored QC almost exactly, unlike some other diagnostic events in this
                session where re-run Q_w can drift from the stored value -- see
                figure_axec1_event_23919 docstring for why that can happen)
    chosen filter band (try_filters): 5-10 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch298_event_...mseed)
...
Pages:
  ...
  7. Single-page A-E composite summary -- ZNE traces (not LQT) + particle motion + phi-dt
     space + clustering
  8. Cluster stability (phi and dt vs. window number, with Silver & Chan 1991 error bars, for
     the winning DBSCAN cluster) + Q and T wavelets shown separately before/after splitting
     correction
  9. Fast and slow wavelets overlaid on one seismogram (fast = solid black, slow = dashed red),
     before splitting correction (as recorded, offset by dt) and after (re-aligned by the
     +dt/2 / -dt/2 shift remove_splitting() applies internally)

Run with: python3 figure_axec2_mfast_event_74456_hq_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_74456_hq_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch298_event_2015-04-21T18-23-42.668000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-04-21T18:23:42.668000Z')
S_ARRIVAL_TIME = 1.672   # seconds from origin
P_ARRIVAL_TIME = 0.882   # seconds from origin
BACK_AZIMUTH = 268.136273
INCIDENCE_PYKONAL_S = 14.62113   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 23.279138
RECTILINEARITY = 0.999172
DEPTH_KM = 1.328   # event depth, km below sea level
...
def plot_multipanel_summary_page(pdf, st_zne, splitting_obj, st_q_uncorr, st_t_uncorr, st_q_corr, st_t_corr,
    ...
    right_gs = outer_gs[1].subgridspec(3, 1, height_ratios=[1.0, 1.0, 0.85], hspace=0.35)

    left_bbox = outer_gs[0].get_position(fig)
    row0_bbox = right_gs[0].get_position(fig)
    row1_bbox = right_gs[1].get_position(fig)
    row2_bbox = right_gs[2].get_position(fig)
    ...
    cs = ax5.contourf(X, Y, Z, levels=20, cmap='Blues')
    ax5.contour(cs, levels=cs.levels[1::2], colors='k', alpha=0.25, linewidths=0.5)
    ax5.axvline(t_dom / 2, c='k', ls='--', lw=1.5, label=r'$T_{dom}/2$')
    ax5.errorbar(dt_curr, phi_curr, xerr=dt_err_curr, yerr=phi_err_curr,
                 c='r', capsize=5, marker=(6, 2, 0), markersize=14, markeredgecolor='r',
                 markeredgewidth=2, label='Optimal solution')
    ...
    for idx, clust_idx in enumerate(clust_idxs):
        ...
        ax6.errorbar(dts, phis, xerr=dt_errs, yerr=phi_errs, fmt='o', markersize=2,
                     ...
                     zorder=5, label='Cluster solutions' if idx == 0 else None)
    ax6.errorbar(dt_curr, phi_curr, xerr=dt_err_curr, yerr=phi_err_curr,
                 marker=(6, 2, 0), linestyle='none', markersize=14, color='r', markeredgecolor='r',
                 markeredgewidth=2, ecolor='r', elinewidth=0.7, capsize=4,
                 capthick=0.7, label='Optimal (best window)', zorder=10)
    ...
    # ── F, G: cluster stability (phi, dt vs. window number) for the winning cluster --
    # all data black, no optimal-solution line, no legend, per explicit user instruction.
    min_var_idx = clustering_info['min_var_idx']
    winning_cluster = clustering_info['clusters_dict'][str(min_var_idx)]
    order = np.argsort(winning_cluster['sample_idxs'])
    win_num = winning_cluster['sample_idxs'][order]
    stab_phis = winning_cluster['phis'][order]
    stab_phi_errs = winning_cluster['phi_errs'][order]
    stab_lags = winning_cluster['lags'][order]
    stab_lag_errs = winning_cluster['lag_errs'][order]

    # F, G span the FULL figure width (left column + right column combined), matching the
    # rows above (A|B and C,D|E each span the whole figure width) -- not just the right
    # column's narrower width.
    fg_x0 = left_bbox.x0
    fg_x1 = row2_bbox.x1
    fg_gap = 0.09
    fg_w = (fg_x1 - fg_x0 - fg_gap) / 2.0
    fg_top_margin = 0.05 * row2_bbox.height   # reserve room for the title, below the F/G label
    fg_height = row2_bbox.height - fg_top_margin
    ax_F = fig.add_axes([fg_x0, row2_bbox.y0, fg_w, fg_height])
    ax_G = fig.add_axes([fg_x0 + fg_w + fg_gap, row2_bbox.y0, fg_w, fg_height])

    ax_F.errorbar(win_num, stab_phis, yerr=stab_phi_errs, fmt='o', ms=4, color='k',
                  ecolor='k', elinewidth=0.8, capsize=3, zorder=5)
    ax_F.set_xlabel('Window number', fontsize=9)
    ax_F.set_ylabel(r'$\phi$ from Q ($^o$)', fontsize=9)
    ax_F.tick_params(labelsize=8)
    ax_F.set_title(f'Cluster stability: $\\phi$ per window ({len(win_num)} windows)', fontsize=10)
    ax_F.grid(alpha=0.3)

    ax_G.errorbar(win_num, stab_lags, yerr=stab_lag_errs, fmt='o', ms=4, color='k',
                  ecolor='k', elinewidth=0.8, capsize=3, zorder=5)
    ax_G.set_xlabel('Window number', fontsize=9)
    ax_G.set_ylabel(r'$\delta t$ (s)', fontsize=9)
    ax_G.tick_params(labelsize=8)
    ax_G.set_title(f'Cluster stability: $\\delta t$ per window ({len(win_num)} windows)', fontsize=10)
    ax_G.grid(alpha=0.3)

    label_gap = 0.02
    for ax, row_top, label in [
        (ax_Z, row0_bbox.y1, 'A'), (ax5, row0_bbox.y1, 'B'),
        (ax3, row1_bbox.y1, 'C'), (ax4, row1_bbox.y1, 'D'), (ax6, row1_bbox.y1, 'E'),
        (ax_F, row2_bbox.y1, 'F'), (ax_G, row2_bbox.y1, 'G'),
    ]:
        x0 = ax.get_position().x0
        fig.text(x0 - 0.02, row_top + label_gap, label, fontsize=15,
                 fontweight='bold', va='bottom', ha='left')

    fig.suptitle('AXEC2: High Quality Shear-Wave Splitting Event', fontsize=15, fontweight='bold')

    Q_w = float(row['Q_w'])
    fig.text(
        0.5, 0.06,
        f'Rectilinearity: {RECTILINEARITY:.3f}     SNR: {SNR_HORIZONTAL:.2f}     '
        f'Quality (Q_w): {Q_w:.3f}     Incidence angle: {INCIDENCE_PYKONAL_S:.2f}°     '
        f'Depth: {DEPTH_KM:.3f} km',
