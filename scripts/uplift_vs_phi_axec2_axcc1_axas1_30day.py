#!/usr/bin/env python3
"""
uplift_vs_phi_axec2_axcc1_axas1_30day.py
...
conventions -- see that script's docstring), EXCEPT the rolling window used to compute each
page's "avg fast direction" (pages 1-3 only) is shortened from WINDOW_DAYS=180 down to
30 DAYS (step=1 day, MIN_N lowered from 15 to 12 to match the smaller per-window event
counts at this resolution -- AXCC1 in particular only averages ~0.9 events/day
post-eruption) -- MATCHING the geodetic uplift x-axis, which has always used a 30-day
rolling mean (bpr_inflation_periods.py's ROLLING_DAYS=30, unchanged here). The original
uplift_vs_phi_axec2_axcc1_axas1.py mismatched the two axes' smoothing (180-day phi window
vs. 30-day uplift window); this version and the companion _1week.py/_2week.py (7-day and
14-day phi windows) exist per explicit user request as a comparison series, to see how much
of the apparent sigmoid shape in "avg fast direction vs. de-tided uplift" is a smoothing
artifact of a wide phi-averaging window versus a real signal that survives much finer time
resolution. Page 4 (AXEC2's eruption transition) is UNCHANGED -- it already uses a 12-hour
window there, unrelated to this comparison.

Produces (NEW file, 4 pages):
    uplift_vs_phi_axec2_axcc1_axas1_30day.pdf

Run with:
    python3 uplift_vs_phi_axec2_axcc1_axas1_30day.py
"""
...
OUT_PDF = os.path.join(HERE, 'uplift_vs_phi_axec2_axcc1_axas1_30day.pdf')
...
def main():
    ...
    for station in STATIONS:
        ...
        roll = rolling_phi_stats(df, baseline_phi, window_days=ROLL_WINDOW_DAYS,
                                 step_days=ROLL_STEP_DAYS, min_n=ROLL_MIN_N)
        n_valid = roll['mean_phi'].notna().sum()
        print(f'  {n_valid}/{len(roll)} {ROLL_WINDOW_DAYS}-day rolling windows have >= '
              f'{ROLL_MIN_N} events')
