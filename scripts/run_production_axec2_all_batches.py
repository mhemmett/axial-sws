"""
Production splitting run for AXEC2, all batches, LQT frame + PyKonal-FMM incidence
(35 deg cut, drives both the QC cut and the rotation, per today's established convention),
Q_w (Wustefeld 2010) kept as an output column.

Reads raw_axec2_all_batches_data/raw_axec2_all_batches_metadata.csv (built by
build_raw_axec2_all_batches.py), groups by the original raw-retrieval batch number, and runs
perform_splitting_on_organized_waveforms per batch. Resumable: skips batches whose results
CSV already exists.

Run with:
    python3 run_production_axec2_all_batches.py [--start N] [--end M]
"""
...
RAW_DATA_DIR = os.path.join(HERE, 'raw_axec2_all_batches_data')
RAW_METADATA_CSV = os.path.join(RAW_DATA_DIR, 'raw_axec2_all_batches_metadata.csv')
...
def main():
    ...
    print("Loading full raw-rebuilt metadata...")
    ...
    for batch_num in range(args.start, args.end + 1):
        out_csv = os.path.join(RESULTS_DIR, f'splitting_results_mldd_2015_2021_axec2_batch_{batch_num}.csv')
        if os.path.exists(out_csv):
            continue

        batch_df = meta[meta['batch'] == batch_num]
        if len(batch_df) == 0:
            # Write an empty marker so this batch isn't retried indefinitely if genuinely empty
            pd.DataFrame(columns=['event_id', 'phi', 'dt', 'phi_error', 'dt_error', 'quality', 'success']).to_csv(out_csv, index=False)
            continue
        ...
        for eid, r in results.items():
            res = r['result']
            rows.append({
                'event_id': eid, 'datetime': organized_all[eid]['datetime'],
