...
Windowcheck / grade-3 variant of figure_ec3_multipanel_temporal_mfast_tier5.py -- identical
panel layout, moving-window binning, ray-tracing/anisotropy definitions, and titles/labels,
per explicit user request, with two changes:

  1. Data: mfast_maxdt_windowcheck_pipeline_transfer/splitting_results_AXEC3_{2015_2021,
     2022_2026}_all_batches.csv (the window-check lag-shift-wraparound-fix rerun -- see
     mfast_maxdt_windowcheck_pipeline_transfer/README.md -- both periods complete for AXEC3,
     unlike AXEC2) instead of the older "newdata" (unfixed) mfast_maxdt_pipeline_transfer/ run.
  2. Filter: this session's "grade 3" QC (rose_7period_6stations_newdata_snr_grades.py's
     GRADES[3] / rose_axec2_windowcheck_vs_newdata_grade3_matched_batches.py's GRADE) --
     SNR>=2.0, quality>=0.75, dt_err<=0.05s, dt<=T_dom/2, phi_error<=20 deg -- replacing
     tier-5's quality>=0.5, dt<T_dom/2, phi_error<10 deg, dt_error<0.05s (no SNR cut).

Panels, ray-tracing, both anisotropy-page definitions, and all titles/labels are otherwise
UNCHANGED from figure_ec3_multipanel_temporal_mfast_tier5.py -- see that script's docstring for
...
Produces: figure_ec3_multipanel_temporal_mfast_windowcheck_grade3.pdf (2 pages) -- a NEW file,
does not touch figure_ec3_multipanel_temporal_mfast_tier5.pdf or any other existing output.

Run with: python3 figure_ec3_multipanel_temporal_mfast_windowcheck_grade3.py
"""
...
WC_TRANSFER_DIR = os.path.join(os.path.dirname(HERE), 'mfast_maxdt_windowcheck_pipeline_transfer')
OUT_PDF = os.path.join(HERE, 'figure_ec3_multipanel_temporal_mfast_windowcheck_grade3.pdf')
STATION_FILE = os.path.join(HERE, '..', 'data', 'stations_axial.llz')

STATION = 'AXEC3'

# ── Grade-3 QC (this session's SNR-gated filter -- see docstring) ──────────────────────────
SNR_MIN = 2.0
QW_MIN = 0.75
PHI_ERR_MAX = 20.0
DT_ERR_MAX = 0.05

# ── Anisotropy y-axes + ray tracing (verbatim from figure_ec3_multipanel_temporal_mfast_
#    tier5.py) ────────────────────────────────────────────────────────────────────────────
...
# ── Moving-window time binning (verbatim from figure_ec3_multipanel_temporal_mfast_tier5.py) ─
...
def apply_grade3_filter(df):
    return df[(df['snr_horizontal'] >= SNR_MIN) &
              (df['quality'] >= QW_MIN) &
...
def add_T_travel(df):
    ...
    method as figure_ec3_multipanel_temporal_mfast_tier5.py."""
...
def add_frac_aniso(df):
    ...
    x = 2*T_travel/dt (see figure_ec3_multipanel_temporal_mfast_tier5.py's docstring for the
    derivation); kept only if 0<=a<=1, else the event is dropped."""
    df = df.copy()
...
def main():
    print('Loading AXEC3 windowcheck results (both periods)...')
    raw = load_axec3_windowcheck()
    df = apply_grade3_filter(raw)
    print(f'  {len(raw):,} baseline (success, dt>0) -> {len(df):,} after grade-3 filter '
