"""
figure_axec2_mfast_event_5_1232_cycleskip_check_windowcheck_diagnostics.py

Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected to
test whether the swspy grid-search's cyclic np.roll time-shift (see split.py's
_phi_dt_grid_search) produces a visible wraparound artifact for a high-frequency (short
dominant-period) event, per explicit user request -- BEFORE making any changes to analysis code.
...
Run with: python3 figure_axec2_mfast_event_5_1232_cycleskip_check_windowcheck_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_5_1232_cycleskip_check_windowcheck_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch5_event_2015-01-28T10-28-34.941000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-01-28T10:28:34.941000Z')
S_ARRIVAL_TIME = 1.399   # seconds from origin
P_ARRIVAL_TIME = 0.699   # seconds from origin
BACK_AZIMUTH = 304.4398513460168
INCIDENCE_PYKONAL_S = 17.95315765991177   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 26.365504256862582
RECTILINEARITY = 0.9780769333352276
...
def main():
    ...
    splitting_obj = swc.create_splitting_object(
        ...
        # IMPORTANT: pass max_t_shift_s in at CONSTRUCTION time -- the window-check fix in
        # split_windowcheck.py's __init__ needs the correct value at the point the window
        # layout is computed, not a post-construction attribute override.
        max_t_shift_s=0.2,
    )
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='Cycle-Skip Check (Window-Check Fix)',
        )
