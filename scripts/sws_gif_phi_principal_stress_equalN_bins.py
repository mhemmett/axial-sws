#!/usr/bin/env python3
"""
sws_gif_phi_principal_stress_equalN_bins.py

GIF equivalent of sws_temporal_combined_phi_annual_principal_stress_kidiwela_source_filtered.pdf

For each page (station + all-stations) × depth-slice row, divides the data
into N_BINS=100 equal-earthquake-count bins (sorted by time) and produces one
animated GIF per (station, depth) combination — 21 GIFs total.

Each frame shows:
  - bathymetry background
  - φ+90° principal-stress stick vectors (LineCollection, hsv colormap)
  - Kidiwela source locations
  - station triangles
  - title: "STATION — σ₁ principal stress  z=X km\ndate range   N=n_eq"

Output: results/gifs/sws_gif_phi_ps_equalN_{STATION}_z{depth}.gif
Frame speed: 300 ms/frame, infinite loop.
"""
...
# ── Grid / binning parameters (same as originals) ────────────────────────────
...
# ── Spatial mesh ──────────────────────────────────────────────────────────────
def xyzd2mesh(x, y, z, phi_az, z0):
    mz = np.abs(z - z0) <= Z_HALF_WIDTH
    xs, ys, phi_s = x[mz], y[mz], phi_az[mz]
    ...
    for k, nb in enumerate(nbrs):
        ...
        PHI.ravel()[k] = float(np.degrees(
            np.arctan2(np.mean(np.sin(ang)), np.mean(np.cos(ang))) / 2) % 180)
    return Xg, Yg, PHI, CNT
    ...
    for i in range(0, ny, STICK_STEP):
        for j in range(0, nx, STICK_STEP):
            if CNT[i, j] < COUNT_MIN:
                continue
            phi_val = PHI[i, j]
            phi_rad = np.radians(phi_val + 90)   # φ+90° = principal stress direction
            x0, y0  = Xg[i, j], Yg[i, j]
...
def make_equal_n_bins(df, n_bins):
    ...
    n_ev   = len(events)
    splits = np.array_split(events.values, n_bins) if n_ev >= n_bins else [[ev] for ev in events]
    bins = []
    for chunk in splits:
        if len(chunk) == 0:
            continue
...
def render_frame(sub, z0, title, highlight, fig_size=(4.5, 4.5), dpi=100):
    """Render one principal-stress phi frame; return PIL Image (RGB)."""
    cmap = plt.colormaps['hsv']
    ...
    if len(sub) >= COUNT_MIN:
        ...
        has_vectors = _add_vectors(ax, Xg, Yg, PHI, CNT)
    ...
    sm = mcm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=0, vmax=180))
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, shrink=0.75, pad=0.02)
    cbar.set_ticks(np.arange(0, 181, 30))
    cbar.ax.yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
    cbar.set_label('φ [° from N]', fontsize=8)
...
targets = [(sta, dfs[sta]) for sta in STATIONS]
targets.append(('ALL', all_df))

for sta_name, df in targets:
    ...
    for z0 in DEPTH_SLICES:
        ...
        for i, (t_start, t_end, n_eq, sub) in enumerate(bins):
            ...
            title = (f'{sta_name}  σ₁ principal stress  z={z0} km\n'
                     f'{date_str}   N={n_eq:,}')
            img = render_frame(sub, z0, title, highlight)
            frames.append(img)
            if (i + 1) % 25 == 0:
                print(f'    z={z0}  frame {i+1}/{len(bins)}')

        out = os.path.join(GIF_DIR,
                           f'sws_gif_phi_ps_equalN_{sta_name}_z{z0}.gif')
