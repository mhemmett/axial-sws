"""
figure_axec2_mfast_event_73170_15hz_diagnostics_windowcheck.py

Diagnostic multi-page PDF for the SAME event as figure_axec2_mfast_event_73170_15hz_diagnostics.py
(AXEC2 293_73170, 15.15 Hz), but run through swspy.splitting.split_windowcheck instead of the
production swspy.splitting.split module. split_windowcheck.py is a byte-for-byte copy of
split.py's production create_splitting_object class, modified in __init__ only: rather than
masking/excluding undersized windows after the grid search (an earlier attempt at this fix, which
turned out to starve DBSCAN clustering of samples for this exact event -- 33/49 windows undersized,
leaving only 16, below the eps=0.15/min_samples=15 clustering threshold, so no solution was
produced at all), this version adjusts the END-WINDOW RANGE ITSELF at window-setup time, before any
grid search runs, so every window is well-formed from the start:
  - If the shortest candidate window (latest start window -> earliest end window) would be
    shorter than max_t_shift_s, the first end window is moved out to
    (max_t_shift_s - win_S_pick_tolerance) after the S-pick (making the shortest window exactly
    max_t_shift_s), and the last end window is kept exactly one dominant period (Tmid) after that
    -- preserving the end-window range's span (which, with this repo's standard first_window_end/
    last_window_end multipliers of 1.5/2.5, is already exactly one Tmid) rather than reusing the
    original multipliers themselves.
  - Start windows (win_S_pick_tolerance, overall_win_start_pre_fast_S_pick) are left untouched.
  - max_t_shift_s must now be passed to create_splitting_object() as a CONSTRUCTOR argument
    (new kwarg, default 0.30s matching swspy's original class default) rather than set as a
    post-construction attribute override -- this fix needs the correct value at the moment the
    window layout is computed, not after.

Purpose: compare page 5 (phi-dt parameter space) directly against the original
(un-patched, split.py-based) diagnostics PDF for this same event, to see whether widening the
shortest window up to max_t_shift_s removes the periodic/comb-like wraparound artifact observed
there, and whether/how the reported best-fit phi/dt/Q_w change as a result -- all n_win**2=49
windows should still produce a solution (unlike the masking approach).

Event: AXEC2, event_id 293_73170 (batch 293), origin 2015-04-20T03:52:30.634Z (pre-eruption).
Dominant period T_dom = 0.0660s (~15.15 Hz) -- selected specifically to sit below the ~9.35 Hz
/ 0.107s threshold at which an event's SHORTEST candidate analysis window becomes shorter than
max_t_shift_s=0.2s (this session's production override, NOT swspy's own default of 0.30s --
IMPORTANT: unlike the earlier _74456/_18279/_59346/_10205 diagnostics scripts this session,
which did NOT set max_t_shift_s and therefore silently used swspy's 0.30s default, this script
explicitly sets splitting_obj.max_t_shift_s = 0.2 to match the actual production pipeline being
investigated).

Selection: searched production_axec2_mfast_filters_maxdt02_lqt_pykonal_results/ for pre-eruption
events with dominant_period within +/-0.003s of 1/15 Hz (0.0667s), quality>=0.5, dt<T_dom/2,
phi_error<10 deg, dt_error<0.05s (tier 5), rectilinearity>0.9, snr>10, incidence<20 deg, ranked
by rectilinearity/snr descending. This event tops that ranking AND was independently re-verified
to hold up on re-run with max_t_shift_s=0.2 explicitly set (many nearby-ranked candidates were
also re-verified as a batch and ALL held up robustly under this setting -- unlike an earlier,
mistaken first pass using swspy's default 0.30s, where this same event's re-run quality
collapsed, Q_w dropping from 1.000 to 0.000 -- a reminder that max_t_shift_s materially changes
results and must be set explicitly to match production):
    Production: quality=1.000, phi_error=5 deg, dt_error=0.005s, snr_horizontal=19.12,
                rectilinearity_jurkevics=0.9888, incidence_pykonal_s=16.18 deg, dt=0.01s,
                dominant_period=0.066032s
    RE-RUN (max_t_shift_s=0.2 explicitly set, matching production):
                phi=?, dt=0.010s, phi_error=5 deg, dt_error=0.005s, Q_w=1.000, T_dom=0.0660s
    chosen filter band (try_filters): 10-20 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch293_event_...mseed)
was already detrended/tapered/bandpass-filtered to this event's own 10-20 Hz optimal band when
...
to both T_dom and max_t_shift_s, for direct comparison.

Run with: python3 figure_axec2_mfast_event_73170_15hz_diagnostics_windowcheck.py
"""
...
import swspy.splitting.split_windowcheck as swc  # noqa: E402

sys.path.insert(0, HERE)
from splitting_functions import create_splitting_analysis  # noqa: E402  (unused here; kept for parity)

OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_73170_15hz_diagnostics_windowcheck.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch293_event_2015-04-20T03-52-30.634000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-04-20T03:52:30.634000Z')
S_ARRIVAL_TIME = 0.686   # seconds from origin
P_ARRIVAL_TIME = 0.326   # seconds from origin
BACK_AZIMUTH = 180.51437480290883
INCIDENCE_PYKONAL_S = 16.18299475138002   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 19.12217455867814
RECTILINEARITY = 0.9887560375313807
...
def main():
    ...
    print('Creating SWSPy splitting object (LQT, PyKonal-S incidence) via split_windowcheck...')
    stream_filtered = st_zne.copy()
    event_time = UTCDateTime(event_data['datetime'])
    s_arrival_absolute = event_time + float(event_data['s_arrival_time'])
    splitting_obj = swc.create_splitting_object(
        stream_filtered,
        stations_in=[STATION],
        back_azis_all_stations=[BACK_AZIMUTH],
        receiver_inc_angles_all_stations=[INCIDENCE_PYKONAL_S],
        ...
        # IMPORTANT: pass max_t_shift_s in at CONSTRUCTION time (not as a post-construction
        # attribute override, as the original/production diagnostics scripts do) -- the
        # window-adjustment fix in split_windowcheck.py's __init__ needs the correct value
        # (0.2s, the production override) at the point the window layout itself is computed,
        # not swspy's own 0.30s class default.
        max_t_shift_s=0.2,
    )
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='15 Hz Wraparound Test (window-check fix)',
        )
