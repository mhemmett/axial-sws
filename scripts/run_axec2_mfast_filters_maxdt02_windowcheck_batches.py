"""
Splitting run for AXEC2, mfast-like per-event bandpass variant, using the WINDOW-CHECK fix
(swspy/swspy/splitting/split_windowcheck.py) instead of production swspy
(swspy/swspy/splitting/split.py).
...
Otherwise identical to run_axec2_mfast_filters_maxdt02_batches.py (same raw mfast-filtered
waveform data, same LQT frame + PyKonal-FMM incidence 35 deg cut, same window/n_win/
s_pick_uncertainty/max_t_shift_s=0.2). All output paths are new
(production_axec2_mfast_filters_maxdt02_windowcheck_lqt_pykonal_results/), so nothing in
either existing results directory is overwritten. Resumable: skips batches whose results CSV
already exists.

Run with:
    python3 run_axec2_mfast_filters_maxdt02_windowcheck_batches.py [--start N] [--end M]
"""
...
RESULTS_DIR = os.path.join(HERE, 'production_axec2_mfast_filters_maxdt02_windowcheck_lqt_pykonal_results')
...
def create_splitting_analysis_windowcheck(event_data, first_window_start, last_window_start, first_window_end, last_window_end, n_win, s_pick_uncertainty,
    ...
    print(f"  Creating SWSPy (split_windowcheck) splitting object...")
...
def main():
    ...
    for batch_num in range(args.start, args.end + 1):
        out_csv = os.path.join(
            RESULTS_DIR, f'splitting_results_mldd_2015_2021_axec2_mfast_filters_maxdt02_windowcheck_batch_{batch_num}.csv')
