...
Single-row (depth 1.25-1.75 km), 7-time-period percent shear-wave anisotropy
tomography, modeled on lqt_pykonal_tomography_traveltime_anisotropy_7period.pdf
but simplified per a specific figure request:
  ...
  - Fixed QC gate on every page: dt_err < 0.04 s, phi_err < 20 deg, dt < 0.24 s.
...
Produces 18 pages total (2 depth rows x 3 quality thresholds x 3 page sets):
  1. Percent shear anisotropy (travel-time-weighted mean, COUNT_MIN=8 rays/voxel),
     titled "Percent Shear-wave Anisotropy, Quality >= x".
  2. Delay times at each earthquake's OWN source voxel (mean dt of events
     located in that voxel -- NOT apportioned across every voxel the ray
     crosses, unlike set 1), titled "Delay Times at Earthquake Source
     Locations, Quality >= x".
  3. Same method as set 1, but with the per-voxel coverage gate raised to
     COUNT_MIN_HIGH=20 contributing rays.
...
Run with:
    python3 lqt_pykonal_percent_anisotropy_quality_pages.py
"""
...
def main():
    ...
    base_mask_unfiltered = ok & (dte < DT_ERR_MAX) & (phe < PHI_ERR_MAX)

    cbar_label = 'Travel-time-weighted mean % shear anisotropy per voxel'
    dt_cbar_label = 'Mean delay time at earthquake source voxel (s)'

    out_path = os.path.join(OUT_DIR, 'lqt_pykonal_percent_anisotropy_quality_pages.pdf')
    write_all_pages(out_path, base_mask_filtered, ray_ix, ray_iy, ray_iz, dt, q,
                    ray_period, periods, vox, vray, tt_seg, a_ray_tt,
                    cbar_label, dt_cbar_label)

    out_path_unfiltered = os.path.join(
        OUT_DIR, 'lqt_pykonal_percent_anisotropy_quality_pages_dt_unfiltered.pdf')
    write_all_pages(out_path_unfiltered, base_mask_unfiltered, ray_ix, ray_iy, ray_iz, dt, q,
                    ray_period, periods, vox, vray, tt_seg, a_ray_tt,
                    cbar_label, dt_cbar_label, title_suffix=', dt not filtered')
