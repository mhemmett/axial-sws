"""
Four-version splitting comparison for AXEC2 raw batch 1 (first 250 catalog events,
rebuilt from raw via build_raw_axec2_batch1.py to avoid the old cache's P-incidence<30
...
Run with:
    python3 build_raw_axec2_batch1.py   # once, to build the metadata CSV + per-event mseed this script loads
    python3 run_lqt_zne_incidence_comparison_axec2_batch1.py
"""
...
RAW_DATA_DIR = os.path.join(HERE, 'raw_axec2_batch1_data')
RAW_METADATA_CSV = os.path.join(RAW_DATA_DIR, 'raw_axec2_batch1_metadata.csv')
OUT_PDF = os.path.join(HERE, 'rose_plots_lqt_zne_incidence_comparison_axec2_batch1.pdf')

STATION = 'AXEC2'
INCIDENCE_CUT_DEG = 35.0

# SWSPy dynamic-window parameters, matching the production template notebook
# (axial_splitting_mldd_AXEC2_batched.ipynb) for MLdd-catalog runs.
...
def build_organized_waveforms(batch_df):
    ...
    from the raw-rebuilt AXEC2 metadata CSV + per-event mseed files (build_raw_axec2_batch1.py
...
def main():
    ...
    pd.DataFrame(rows).to_csv(os.path.join(HERE, 'lqt_zne_incidence_comparison_axec2_batch1_results.csv'), index=False)
