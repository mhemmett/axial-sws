#!/usr/bin/env python3
"""
sws_tomography_shallow_source_row.py

Single-row version of sws_tomography_dt_7period_phi20_dt004_fractional_anisotropy_CORRECTED.pdf:
same 7-period shear anisotropy (dVs/Vs) tomography, restricted to ONE depth
row spanning 1.25-1.75 km (instead of the standard 4-row 0-2.5 km stack).

Pipeline (ray tracing + damped/smoothed linearised inversion) is identical to
sws_tomography_fractional_anisotropy_ultra_strict.py; only the depth slice
and plotted periods differ.

Output:
  results/shear_wave_anisotropy_shallow_source.pdf
"""
...
out_path=os.path.join(BASE,'shear_wave_anisotropy_shallow_source.pdf')
print(f'\nWriting {os.path.basename(out_path)}...')
with PdfPages(out_path) as pdf:
    fig=make_single_row_page(results_7,'Shear Anisotropy at 1.25-1.75 km depth, Axial Seamount')
