#!/usr/bin/env python3
"""
run_synthetic_hudson_splitting_two_sills_v2.py

Two-sills driver combined with the NEW v2 geometry (see run_synthetic_hudson_splitting_v2.py
for the full derivation): sill 1 moves to 0.25 km WEST of AXEC1; all other dike waypoints
(P1-P4) stay exactly where they are in the base geometry; only the south-going segment
(P0->P1) is redrawn as a straight connector from the new sill 1 location to the unmoved P1.
SILL2 (Kidiwela S2 location) is UNCHANGED -- only sill 1 and the dike's south segment move.

Everything else (background stress, both fault-damage-zone crack sets, dike opening
magnitude, sill/dike-overprints-fault physics -- including SILL2's own overprint suppression
of the fault crack set and SILL2's deflation in Scenario C -- depth-aware fault gating,
linearly depth-decaying background crack density, event subsample size, QC, period-scenario
pairing) is identical to run_synthetic_hudson_splitting_two_sills.py -- imported directly, not
duplicated, except for the SILL/DIKE_WAYPOINTS geometry overrides applied before the SILL2
stress wrapper.

NOTE: per explicit user request, this script is NOT run automatically -- wait for
confirmation on the corresponding structure map (synthetic_hudson_structure_map_two_sills_v2.pdf)
before running this.

Produces:
    synthetic_hudson_pre_syn_phidt_two_sills_v2.csv
    synthetic_hudson_rose_plots_two_sills_v2.pdf

Run with:
    python3 run_synthetic_hudson_splitting_two_sills_v2.py
"""
...
print(f'SILL2 (Kidiwela S2 location, unchanged): x0={SILL2["x0"]:.2f}, y0={SILL2["y0"]:.2f}, '
...
if __name__ == '__main__':
    ...
    out_csv = os.path.join(base.HERE, 'synthetic_hudson_pre_syn_phidt_two_sills_v2.csv')
    df_all = base.pd.DataFrame(results)
    df_all.to_csv(out_csv, index=False)
    print(f'Saved {out_csv} ({len(df_all):,} rows)')

    out_pdf = os.path.join(base.HERE, 'synthetic_hudson_rose_plots_two_sills_v2.pdf')
    with base.PdfPages(out_pdf) as pdf:
        ...
        fig.suptitle('Synthetic Hudson-crack splitting, TWO SILLS + NEW v2 geometry (sill1 '
                     'moved 0.25km west of AXEC1, only south dike segment redrawn; sill2='
                     'Kidiwela S2, unchanged): Pre-eruption = Scenario B (2 sills), '
                     'Syn-eruption = Scenario C (dike + both sills deflated)\n'
