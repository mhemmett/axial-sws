#!/usr/bin/env python3
"""
uplift_vs_phi_axec2_axcc1_axas1_2week.py
...
page's "avg fast direction" (pages 1-3 only) is shortened from WINDOW_DAYS=180 down to a
2-WEEK window (step=1 day, MIN_N lowered from 15 to 8 to match the far smaller per-window
event counts at this resolution -- AXCC1 in particular only averages ~0.9 events/day
post-eruption). Per explicit user request, this (and the companion _1week.py) exists purely
as a comparison against the original 180-day version, to see how much of the apparent sigmoid
shape in "avg fast direction vs. de-tided uplift" is a smoothing artifact of the wide rolling
window versus a real signal that survives much finer time resolution. Page 4 (AXEC2's eruption
transition) is UNCHANGED -- it already uses a 12-hour window there, unrelated to this comparison.

Produces (NEW file, 4 pages):
    uplift_vs_phi_axec2_axcc1_axas1_2week.pdf

Run with:
    python3 uplift_vs_phi_axec2_axcc1_axas1_2week.py
"""
...
OUT_PDF = os.path.join(HERE, 'uplift_vs_phi_axec2_axcc1_axas1_2week.pdf')
