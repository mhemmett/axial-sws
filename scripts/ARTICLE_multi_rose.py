...
for k_sta,station_name in enumerate(stations):
    ...
    for hyp_dist in hypdist_inters: 
        ...
        for kk,NCat in enumerate(NCats):   
            ...
            (ax_main,ax_insets,ax_middle,ax_top)=NCat.plot_polar_histofast_multi(ax=ax[kk],title='',
            num_ax=6,label_mode='azimuth',edgecolor='k',facecolor=rgb_list[k_sta][0:3],lw=0.5,norm=False)
     
        fig.suptitle(station_name,fontweight='bold')
    
        figname_png='ARTICLE_multi_rose_%s_%s.png'%(station_name,z_suffix)
        figname_pdf='ARTICLE_multi_rose_%s_%s.pdf'%(station_name,z_suffix)
