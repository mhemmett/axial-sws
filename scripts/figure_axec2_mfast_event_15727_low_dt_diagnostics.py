...
figure_axec2_mfast_event_9788_high_dt_diagnostics.py for a high-delay-time vs. low-delay-time
...
Event: AXEC2, event_id 63_15727 (batch 63), origin 2015-02-24T17:42:29.722Z.

Selected from production_axec2_mfast_filters_lqt_pykonal_results/ (the new mfast try_filters
AXEC2 run) as a high-quality, low-delay-time event meeting the same QC gate used throughout
this repo (e.g. lqt_pykonal_percent_anisotropy_quality_pages.py):
    quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    phi_error = 10 deg, dt_error = 0.005 s (both well inside phi_error<20 deg / dt_error<0.04 s)
    dt = 0.02 s (near the bottom of the QC-passing distribution -- the "low dt" companion to
        the 0.29 s event in figure_axec2_mfast_event_9788_high_dt_diagnostics.py)
    chosen filter band (try_filters): 10-20 Hz (chosen_filter_dom_period_samples=14.93, i.e.
        dominant frequency ~13.4 Hz at 200 Hz sampling)
    pre-eruption (< 2015-04-24 06:00 UTC)

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch63_event_...mseed)
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_15727_low_dt_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch63_event_2015-02-24T17-42-29.722000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-02-24T17:42:29.722000Z')
S_ARRIVAL_TIME = 0.948   # seconds from origin
P_ARRIVAL_TIME = 0.508   # seconds from origin
BACK_AZIMUTH = 318.226851
INCIDENCE_PYKONAL_S = 17.555653   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 4.227985
RECTILINEARITY = 0.916620
