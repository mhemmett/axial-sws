...
Produces (7-period and annual × source-binned and receiver-location × filtered):
  sws_temporal_combined_phi_vector_field{suffix}.pdf
  sws_temporal_combined_phi_annual_vector_field{suffix}.pdf
  sws_temporal_combined_phi_receiver_locations_vector_field{suffix}.pdf
  sws_temporal_combined_phi_annual_receiver_locations_vector_field{suffix}.pdf
    ...
    for ri, z0 in enumerate(DEPTH_SLICES):
        for ci, ((lbl,_,__), sub, neq) in enumerate(zip(periods, subs, n_eqs)):
            ...
            if len(sub) >= COUNT_MIN:
                ...
                # dummy invisible mappable for colorbar
                last_h = ax.contourf(Xg,Yg,ma.array(PHI,mask=True),
    ...
    for periods_fn, period_tag in [
        ...
        for x_col, y_col, loc_tag in [
            ...
            out=os.path.join(OUT_DIR,
                f'sws_temporal_combined_phi{period_tag}{loc_tag}_vector_field_kidiwela_source{suffix}.pdf')
            with PdfPages(out) as pdf:
                for sta in STATIONS:
                    fig=make_page(dfs[sta],periods,highlight=sta,
                                  title=f'{sta} — φ vector field {label}',
                                  x_col=x_col,y_col=y_col)
                    pdf.savefig(fig,dpi=300,bbox_inches='tight'); plt.close(fig)
                fig=make_page(dfs,periods,highlight=None,
                              title=f'All stations — φ vector field {label}',
                              x_col=x_col,y_col=y_col)
                pdf.savefig(fig,dpi=300,bbox_inches='tight'); plt.close(fig)
            print(f'  Saved {out}')
