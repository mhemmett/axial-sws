#!/usr/bin/env python3
"""
sws_temporal_kidiwela_source.py

Generates new versions of all sws_temporal combined PDFs with the two
Kidiwela Mogi source locations plotted on each map panel as red dots.

S1 (deep):   x=7.57, y=4.55 km  (labelled 'S1')
S2 (shallow): x=7.53, y=6.60 km  (labelled 'S2')

Produces (7-period and annual, filtered and unfiltered):
  sws_temporal_combined_{phi,dt}_kidiwela_source.pdf
  sws_temporal_combined_{phi,dt}_annual_kidiwela_source.pdf
  sws_temporal_combined_{phi,dt}_kidiwela_source_filtered.pdf
  sws_temporal_combined_{phi,dt}_annual_kidiwela_source_filtered.pdf
"""
...
# ── Kidiwela source locations (km) ────────────────────────────────────────────
KIDIWELA_SOURCES = [
    dict(x=7.57, y=4.55, label='S1'),   # deep
    dict(x=7.53, y=6.60, label='S2'),   # shallow
]
...
def _add_kidiwela(ax):
    """Plot Kidiwela source locations as red dots, smaller than station triangles."""
...
def make_page(param, df_or_dict, periods, highlight, title, dt_vmax=None):
    ...
    for ri, z0 in enumerate(DEPTH_SLICES):
        for ci, ((lbl,_,__), sub, neq) in enumerate(zip(periods, subs, n_eqs)):
            ...
            _add_kidiwela(ax)           # ← Kidiwela sources on every panel
...
for dt_err_max, suffix in [(None, ''), (0.1, '_filtered')]:
    ...
    for periods_fn, period_label in [
        (lambda: build_7_periods(all_df),  ''),
        (lambda: build_annual_periods(),   '_annual'),
    ]:
        periods = periods_fn()
        print(f'  Generating {period_label or "7-period"} figures...')

        for param in ['phi','dt']:
            out = os.path.join(OUT_DIR,
                f'sws_temporal_combined_{param}{period_label}_kidiwela_source{suffix}.pdf')
...
print('\nDone.')
print('\nKidiwela source locations plotted:')
for s in KIDIWELA_SOURCES:
    print(f'  {s["label"]}: x={s["x"]:.2f} km, y={s["y"]:.2f} km')
