...
def _find_dom_freq(data_arr_Q, data_arr_T, fs):
    """
    Find dominant frequency of shear-wave following MFAST definition:
    harmonic mean of dominant frequencies of Q and T components.
    """

    # Periodograms
    f, Pxx_Q = signal.periodogram(data_arr_Q, fs)
    _, Pxx_T = signal.periodogram(data_arr_T, fs)

    # Remove DC component (important for stability)
    f = f[1:]
    Pxx_Q = Pxx_Q[1:]
    Pxx_T = Pxx_T[1:]

    # Dominant frequency per component
    f_Q = f[np.argmax(Pxx_Q)]
    f_T = f[np.argmax(Pxx_T)]

    # Guard against zero or invalid frequencies
    if f_Q <= 0 or f_T <= 0:
        print("Warning: zero dominant frequency detected.")
        dom_freq_Hz = 0.0
    else:
        # Harmonic mean (MFAST definition)
        dom_freq_Hz = 2.0 * f_Q * f_T / (f_Q + f_T)

    # Optional MFAST bounds
    dom_freq_Hz = np.clip(dom_freq_Hz, 0.3, 8.0)
...
def _phi_dt_grid_search(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, n_t_steps, n_angle_steps, n_win, fs, rotate_step_deg, 
    ...
    """..."""
    # Loop over flattened (start,end) pairs. n_win here represents the number of valid pairs.
    for grid_search_idx in prange(n_win):
        start_win_idx = win_start_idxs[grid_search_idx]
        end_win_idx = win_end_idxs[grid_search_idx]

        # Loop over angles:
        for j in range(n_angle_steps):
            angle_shift_rad_curr = ((float(j) * rotate_step_deg) - 90.) * np.pi / 180.

            # Rotate QT waveforms by angle (slice ONCE from full trace):
            theta_rot = -angle_shift_rad_curr
            # Slice from the FULL trace arrays:
    ...
    for grid_search_idx in prange(n_win):
        ...
        for j in range(n_angle_steps):
            ...
            for i in range(n_t_steps):
                ...
                # Rotate the ALREADY-WINDOWED data (no re-slicing):
                theta_rot = -angle_shift_rad_curr
                rot_T_curr = (rolled_rot_T_curr * np.cos(theta_rot)) - (rolled_rot_Q_curr * np.sin(theta_rot))
                rot_Q_curr = (rolled_rot_T_curr * np.sin(theta_rot)) + (rolled_rot_Q_curr * np.cos(theta_rot))
...
class create_splitting_object:
    ...
    def __init__(self, st, nonlinloc_event_path=None, event_uid=None, stations_in=[], S_phase_arrival_times=[], back_azis_all_stations=[], receiver_inc_angles_all_stations=[]):
        ...
        # Dominant frequency
        self.T_mid = None  # will be set after dominant frequency is estimated
       
        # Number of windows
        self.Nw_beg = 5                     # Nw,beg
        self.Nw_end = None                  # computed from T_mid

        # Time offsets
        self.toffbeg = 0.3                  # sec before S
        self.toffend = None                 # T_mid / 2 (computed later)
    ...
    def _setup_mfast_windowing(self, dom_freq_Hz):
        """
        Configure MFAST window geometry from dominant frequency.
        Must be called AFTER dominant frequency estimation.
        """
        if dom_freq_Hz <= 0:
            raise ValueError("Invalid dominant frequency.")

        self.T_mid = 1.0 / dom_freq_Hz

        # End window offset relative to S
        self.toffend = self.T_mid / 2.0

        # Number of end windows (Teanby/MFAST)
        self.Nw_end = int((1.5 * self.T_mid) / self.dTend)

        # Safety limits (from MFAST experience)
        self.Nw_end = np.clip(self.Nw_end, 10, 25)

    def select_windows(self, tS, trace_length):
        """
        MFAST-style window selection.
        Drop-in replacement for SWSPy's select_windows().
        """
        if self.T_mid is None:
            raise RuntimeError(
                "T_mid not defined. "
                "Call _setup_mfast_windowing() before select_windows()."
            )

        # Start times (relative to S)
        t_beg = (
            -self.toffbeg
            - np.arange(self.Nw_beg) * self.dTbeg
        )

        # End times (relative to S)
        t_end = (
            self.toffend
            + np.arange(self.Nw_end) * self.dTend
        )

        win_start_idxs = np.round((tS + t_beg) * self.fs).astype(int)
        win_end_idxs = np.round((tS + t_end) * self.fs).astype(int)

        starts, ends = np.meshgrid(win_start_idxs, win_end_idxs, indexing="ij")
        starts = starts.flatten()
        ends = ends.flatten()

        valid = (starts >= 0) & (ends > starts) & (ends <= trace_length)
    ...
    def _calc_splitting_eig_val_method(self, data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, sws_method="EV", n_layers=1, layer_1_dts_phis=None, layer_2_dts_phis=None, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        # Number of (start,end) pairs returned by select_windows
        n_pairs = int(win_start_idxs.size)
        fs = float(self.fs)
        ...
        phis_labels = np.arange(-90, 90 + rotate_step_deg, rotate_step_deg)
        if n_layers == 1:
            grid_search_results_all_win_EV = np.zeros((n_pairs, n_t_steps, n_angle_steps), dtype=float)
            grid_search_results_all_win_XC = np.zeros((n_pairs, n_t_steps, n_angle_steps), dtype=float)
        elif n_layers == 2:
            # For direct multi-layer inversion, keep higher dimensional storage
            grid_search_results_all_win_EV = np.zeros((n_pairs, n_t_steps, n_angle_steps, n_t_steps, n_angle_steps), dtype=float)
            grid_search_results_all_win_XC = np.zeros((n_pairs, n_t_steps, n_angle_steps, n_t_steps, n_angle_steps), dtype=float)
        else:
        ...
        # Perform grid search:
        if n_layers == 1:
            grid_search_results_all_win_EV, grid_search_results_all_win_XC = _phi_dt_grid_search(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, n_t_steps, n_angle_steps, n_pairs, fs, rotate_step_deg, grid_search_results_all_win_EV, grid_search_results_all_win_XC)
        elif n_layers == 2:
            ...
            grid_search_results_all_win_EV, grid_search_results_all_win_XC = _phi_dt_grid_search_direct_multi_layer(data_arr_Q, data_arr_T, win_start_idxs, win_end_idxs, 
                                                                                                        n_t_steps, n_angle_steps, n_pairs, fs, rotate_step_deg, 
                                                                                                        grid_search_results_all_win_EV, grid_search_results_all_win_XC, 
    ...
    def _sws_win_clustering(self, lags, phis, lag_errs, phi_errs, min_eig_ratios=None, method="dbscan", return_clusters_data=False):
        """
        Perform SWS window clustering following Teanby (2004) / MFAST logic.

        - Uses hierarchical (Ward) clustering in a circular-safe (phi, dt) space
        - Enforces minimum cluster size (Ncmin) and maximum number of clusters (Mmax)
        - Selects cluster with minimum representative variance
        - Selects best observation within that cluster
        """
        ...
        # MFAST parameters
        # -----------------------------
        Ncmin = 1     # minimum samples per cluster
        Mmax = 15     # maximum number of clusters

        # -----------------------------
        # Circular-safe coordinate transform
        # -----------------------------
        lag_norm = lags / np.max(lags)
        phi_rad = np.deg2rad(phis)

        samples_new_coords = np.column_stack(
            (
                lag_norm * np.cos(2.0 * phi_rad),
                lag_norm * np.sin(2.0 * phi_rad),
            )
        )
        ...
        # -----------------------------
        try:
            D = pdist(samples_new_coords, metric="euclidean")
            Z = linkage(D, method="ward")
            cluster_labels = fcluster(Z, t=Mmax, criterion="maxclust")
        except Exception:
            print("Warning: Hierarchical clustering failed.")
            return self._return_fail(min_eig_ratios, return_clusters_data)

        # -----------------------------
        # Build clusters and enforce Ncmin
        # -----------------------------
        clusters_dict = {}
        for label in np.unique(cluster_labels):
            ...
            clusters_dict[key] = {
                "lags": lags[idxs],
                "lag_errs": lag_errs[idxs],
                "phis": phis[idxs],
                "phi_errs": phi_errs[idxs],
            }

            if min_eig_ratios is not None:
                clusters_dict[key]["min_eig_ratios"] = min_eig_ratios[idxs]

        if len(clusters_dict) == 0:
            print("Warning: No clusters passed Ncmin.")
            return self._return_fail(min_eig_ratios, return_clusters_data)

        # -----------------------------
        # Compute Teanby variances
        # -----------------------------
        ...
        for i, key in enumerate(keys):
            cl = clusters_dict[key]

            # Eq. 13 – cluster variance
            cluster_vars[i] = (
                np.sum(
                    (cl["lags"] - np.mean(cl["lags"])) ** 2
                    + (cl["phis"] - np.mean(cl["phis"])) ** 2
                )
                / len(cl["lags"])
            )
            ...
            # Eq. 14 – data variance
            data_vars[i] = (
                1.0 / np.sum(1.0 / cl["lag_errs"] ** 2)
                + 1.0 / np.sum(1.0 / cl["phi_errs"] ** 2)
            )
        ...
        # -----------------------------
        # Select minimum-variance cluster
        # -----------------------------
        min_var_idx = np.argmin(cluster_var_0s)
        best_cluster = clusters_dict[keys[min_var_idx]]

        # -----------------------------
        # Select best observation in cluster
        # -----------------------------
        obs_vars = best_cluster["lag_errs"] ** 2 + best_cluster["phi_errs"] ** 2
        opt_obs_idx = np.argmin(obs_vars)

        opt_lag = best_cluster["lags"][opt_obs_idx]
        opt_phi = best_cluster["phis"][opt_obs_idx]
        opt_lag_err = best_cluster["lag_errs"][opt_obs_idx]
        opt_phi_err = best_cluster["phi_errs"][opt_obs_idx]

        if min_eig_ratios is not None:
            opt_eig_ratio = best_cluster["min_eig_ratios"][opt_obs_idx]
    ...
    def _return_fail(self, min_eig_ratios, return_clusters_data):
        if return_clusters_data:
            if min_eig_ratios is not None:
    ...
    def _get_uncorr_and_corr_waveforms(self, station):
        ...
        # Trim using MFAST-style offsets. Use `toffbeg` and `toffend` (fallback if toffend not set).
        end_offset = self.toffend if (self.toffend is not None) else self.toffbeg
        st_ZNE_curr.trim(starttime=arrival_time_curr - self.toffbeg,
                  endtime=arrival_time_curr + end_offset + self.max_t_shift_s)
    ...
    def perform_sws_analysis(self, coord_system="ZNE", sws_method="EV", return_clusters_data=True, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            # Define tr_Q, tr_T windows with set 3.0 sec window starting from toffbeg
            #tr_Q = st_LQT_curr.select(station=station, channel="??Q")[0]
            #tr_T = st_LQT_curr.select(station=station, channel="??T")[0]

            #tr_Q.trim(starttime=tr_Q.stats.starttime - self.toffbeg,
            #            endtime=tr_Q.stats.starttime - self.toffbeg + 3.0)
            #tr_T.trim(starttime=tr_T.stats.starttime - self.toffbeg,
            #            endtime=tr_T.stats.starttime - self.toffbeg + 3.0)
            #self.fs = tr_T.stats.sampling_rate

            # --- NEW: dominant frequency estimation (MFAST stage 1) ---
            #dom_freq = _find_dom_freq(
            #    tr_Q.data,
            #    tr_T.data,
            #    self.fs
            #)

            # --- NEW: configure MFAST window geometry ---
            # Allows calculation of toffend, which is needed for proper window trimming below
            #self._setup_mfast_windowing(dom_freq)
            ...
            if self.nonlinloc_event_path:
                ...
                arrival_time_curr = self.S_phase_arrival_times[station_idx_tmp]
            # Trim using MFAST-style offsets. Use `toffbeg` and `toffend` (fallback if toffend not yet set).
            end_offset = self.toffend #if (self.toffend is not None) else self.toffbeg

            st_LQT_curr.trim(starttime=arrival_time_curr - self.toffbeg,
                                endtime=arrival_time_curr + end_offset + self.max_t_shift_s)
            st_ZNE_curr.trim(starttime=arrival_time_curr - self.toffbeg,
                                endtime=arrival_time_curr + end_offset + self.max_t_shift_s)
            ...
            # 3. Get window indices (MFAST-style)
            ...
            # Time of S arrival in trimmed trace coordinates
            # (arrival is at self.toffbeg seconds in the trimmed traces)
            tS = self.toffbeg

            # --- REPLACEMENT: window selection ---
            trace_length = len(tr_Q.data)
            win_start_idxs, win_end_idxs = self.select_windows(tS, trace_length)
    ...
    def perform_sws_analysis_multi_layer(self, coord_system="ZNE", multi_layer_method="explicit", num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            if multi_layer_method == "explicit":
                ...
                # 1. Split data into two windows (using apparent delay time). Configure MFAST windows first.
                dom_freq = _find_dom_freq(tr_Q.data, tr_T.data, self.fs)
                self._setup_mfast_windowing(dom_freq)
                tS = self.toffbeg
                win_start_idxs, win_end_idxs = self.select_windows(tS)
                dt_app = self.sws_result_df.loc[self.sws_result_df['station'] == station]["dt"].values[0]
                partition_idx = round( (self.toffbeg + dt_app) * self.fs )
                ...
                # 1. Get window idxs: configure MFAST windows first.
                dom_freq = _find_dom_freq(tr_Q.data, tr_T.data, self.fs)
                self._setup_mfast_windowing(dom_freq)
                tS = self.toffbeg
                win_start_idxs, win_end_idxs = self.select_windows(tS)
    ...
    def plot(self, outdir=None, suppress_direct_plotting=False):
        ...
        for station in self.stations_list:
            ...
            if suppress_direct_plotting:
                plt.ion()
            gs = fig.add_gridspec(5, 5, wspace=0.3, hspace=0.3) # no. rows, no. cols
            ...
            if self.clustering_info:
                ...
                # added by me
                phi_dt_scatter_ax = fig.add_subplot(gs[5, 0])
                phi_dt_scatter_ax.get_xaxis().set_visible(False)
                phi_dt_scatter_ax.get_yaxis().set_visible(False)
            ...
            if self.sws_multi_layer_result_df is None:
                ...
                CS = phi_dt_ax.contourf(X, Y, Z, levels=20, cmap="jet")
            ...
            # Or plot for double layer:
                ...
                CS = phi_dt_ax_layer1.contourf(X, Y, Z1, levels=20, cmap="jet")
                ...
                CS = phi_dt_ax_layer2.contourf(X, Y, Z2, levels=20, cmap="jet")
            ...
            if self.clustering_info:
                ...
                if "layer1" in self.clustering_info[station]:
                    ...
                    # Plot optimal value
                    phi_dt_scatter_ax.plot(dt_curr, phi_curr, 'g*', markersize=12,
                                        markeredgecolor='k', markeredgewidth=0.5,
