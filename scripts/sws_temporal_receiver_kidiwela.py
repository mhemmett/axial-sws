#!/usr/bin/env python3
"""
sws_temporal_receiver_kidiwela.py

Receiver-location projected temporal maps WITH Kidiwela Mogi source dots.
Combines _receiver_locations and _kidiwela_source naming conventions.

Ray projection: 70% toward receiver.
Kidiwela sources: S1 (deep, x=7.57,y=4.55) and S2 (shallow, x=7.53,y=6.60)
plotted as red filled dots with black outline, smaller than station triangles.
CC1 label shifted left to avoid overlap with S2.

Produces (7-period and annual, filtered and unfiltered):
  sws_temporal_combined_{phi,dt}_receiver_locations_kidiwela_source.pdf
  sws_temporal_combined_{phi,dt}_annual_receiver_locations_kidiwela_source.pdf
  + _filtered versions of each
"""
...
def make_page(param,df_or_dict,periods,highlight,title,dt_vmax=None):
    ...
    if param=='dt' and dt_vmax: dp['vmax']=dt_vmax; dp['step']=round(dt_vmax/6,3)
    cmap=plt.colormaps[dp['cmap']]; vmin,vmax=dp['vmin'],dp['vmax']
    levels=np.linspace(vmin,vmax,15); n_per=len(periods); n_dep=len(DEPTH_SLICES)
    ...
    for ri,z0 in enumerate(DEPTH_SLICES):
        for ci,((lbl,_,__),sub,neq) in enumerate(zip(periods,subs,n_eqs)):
            ...
            if len(sub)>=COUNT_MIN:
                Xg,Yg,PHI,DT,CNT=xyzd2mesh(sub['x_proj'].values,sub['y_proj'].values,
...
for dt_err_max,suffix in [(None,''),(0.1,'_filtered')]:
    ...
    for periods_fn,period_tag in [
        ...
        periods=periods_fn()
        print(f'  Generating {period_tag or "7-period"} receiver+Kidiwela figures...')
        for param in ['phi','dt']:
            out=os.path.join(OUT_DIR,
                f'sws_temporal_combined_{param}{period_tag}_receiver_locations_kidiwela_source{suffix}.pdf')
