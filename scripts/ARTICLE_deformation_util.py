...
# -*- coding: utf-8 -*-
"""
Created on Wed May 29 14:22:55 2019
...
def read_disp_file(disp_file,flag_plot=False):
    ...
    if flag_plot:
        ...
        for k_ax,D in enumerate([Ux,Uy,Uz]):
            im=ax[k_ax].pcolormesh(X[:,:,0],Y[:,:,0],D[:,:,0],cmap=plt.cm.get_cmap('jet'))
...
def plot_sigma1_stations(X_slice,Y_slice,SIGMA1,period='syn',
    ...
    ax_norm.set_ylabel('$\sigma_{1}-\sigma_{2}$')
    ...
    ax_fast.set_xlabel('$\Phi_{obs}$ [°]')
    ax_fast.set_ylabel('$\Phi_{cal}$ [°]')

    return (ax_norm,ax_fast)


def plot_elev_stations(X_slice,Y_slice,Uz_slice,period='syn'):
    ...
    xticklabels.append('')
    ax.set_xticks(np.arange(0,k_sta+1))
