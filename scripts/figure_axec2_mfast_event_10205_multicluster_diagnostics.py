"""
figure_axec2_mfast_event_10205_multicluster_diagnostics.py

Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected as
a quality~0.5 event that PASSES tier-5 QC (quality>=0.5, dt<T_dom/2, phi_error<10 deg,
dt_error<0.05s) on re-run, but whose DBSCAN clustering finds TWO separate clusters of
per-window solutions in phi-dt space -- i.e. the windowed grid search is not unanimous, and
the "winning" (smallest-variance) cluster is a real choice made among competing candidate
solutions, not the only solution found.

Event: AXEC2, event_id 41_10205 (batch 41), origin 2015-02-13T06:04:50.252Z (pre-eruption).
...
production quality in [0.35, 0.70] (broadly "around 0.5") and dt < T_dom/2, then RE-RAN a
random sample of ~460 candidates (create_splitting_analysis + perform_sws_analysis, same
pipeline as this script) checking splitting_obj.clustering_info['AXEC2']['clusters_dict'] for
more than one cluster. 35/463 sampled candidates had >1 cluster; this event is the only one
among them that ALSO clears tier 5 (phi_error<10, dt_error<0.05) on re-run:
    Production: quality=0.483065, phi_error=7 deg, dt_error=0.010s, snr_horizontal=5.25,
                rectilinearity_jurkevics=0.878, incidence_pykonal_s=16.11 deg, dt=0.02s
    RE-RUN (create_splitting_analysis + perform_sws_analysis, same pipeline as this script,
            reproducible across repeated runs with full-precision inputs):
                phi=66.0 deg, dt=0.050s, phi_error=8 deg, dt_error=0.020s, Q_w=0.565,
                n_clusters=2, T_dom=0.1351s (T_dom/2=0.0676s, dt=0.05s clears the cycle-skip
                cut with some margin)
    chosen filter band (try_filters): 5-10 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch41_event_...mseed)
...
Pages:
  ...
  6. DBSCAN clustering result in phi-dt space -- TWO clusters expected here (see n_clusters=2
     above); the winning (smallest-variance) cluster's optimal solution is marked with a green
     star, but the OTHER cluster's windows are also plotted, showing the competing solution
  ...
  8. Cluster stability (phi and dt vs. window number, with Silver & Chan 1991 error bars, for
     the WINNING cluster only -- the other cluster's windows are excluded from this page, see
     page 6 for both) + Q and T wavelets, initial vs. corrected overlaid on shared axes (same
     y-scale for Q and T)
...
Run with: python3 figure_axec2_mfast_event_10205_multicluster_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_10205_multicluster_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch41_event_2015-02-13T06-04-50.252000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-02-13T06:04:50.252000Z')
S_ARRIVAL_TIME = 1.358   # seconds from origin
P_ARRIVAL_TIME = 0.648   # seconds from origin
BACK_AZIMUTH = 306.53473124119216
INCIDENCE_PYKONAL_S = 16.105159353629098   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 5.2543904537454935
RECTILINEARITY = 0.8781572173239225
...
def main():
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='Multi-Cluster, Quality~0.5',
        )
