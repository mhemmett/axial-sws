...
#
# # Master Shear-Wave Splitting Workflow for Axial Seamount — AXEC1 (Real-Time, 2022-2026)
#
# Same pipeline as AXEC3, adapted for station AXEC1:
...
# ## 2. Load catalog and filter for AXEC1
...
def get_station_traces_batch_axec1(df, filename, starttime, endtime, station_id, batch_size=250):
    """Batched bulk waveform retrieval for AXEC1 (EHE/EHN/EHZ channels)."""
    from obspy.clients.fdsn import Client
    from obspy.core.utcdatetime import UTCDateTime
...
for i in range(total_batches):
    ...
    batch_filename = f'../data/axial_mldd_2022_2026_axec1_batch_{i+1}'
    ...
    get_station_traces_batch_axec1(batch_df, filename=batch_filename,
...
passing_waveforms_dir = 'passing_waveforms_data_mldd_2022_2026_axec1'
...
if os.path.exists(passing_waveforms_metadata):
    ...
    for i in range(total_batches):
        batch_filename = f'../data/axial_mldd_2022_2026_axec1_batch_{i+1}.mseed'
...
passing_waveforms_reloaded = load_passing_waveforms(output_dir='passing_waveforms_data_mldd_2022_2026_axec1')
...
for i, batch in enumerate(batched_waveforms):
    ...
    base_name = f'splitting_results_mldd_2022_2026_axec1_batch_{batch_num}.csv'
...
save_results_csv(final_results,
                 file_name=os.path.join(results_dir, 'splitting_results_mldd_2022_2026_axec1_all_batches.csv'))
