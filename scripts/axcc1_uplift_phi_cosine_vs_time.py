#!/usr/bin/env python3
"""
axcc1_uplift_phi_cosine_vs_time.py

AXCC1 / Central Caldera BOTPT (RS03CCAL-MJ03F-05-BOTPTA301) counterpart of
...
x-range handling, etc.), not repeated in full here. Differences from the AXEC2 version:
  ...
  - Data: AXCC1 windowcheck grade-3 events + bpr_inflation_periods_ccal (Central Caldera BOTPT,
    not the Eastern Caldera one AXEC2's script uses).
  - No "seismometer potentially unleveled" window is assumed here -- that flag was specific to
    ...
    AXCC1 shows a comparable anomaly has NOT been established, so this script does not shade or
    flag any window. If a similar excursion is visible once plotted, it should be confirmed and
    added deliberately, not assumed by copying AXEC2's dates.
  - The cube-root fit's x-shift (page 2) is NOT hardcoded to 0.55 m (that value was specific to
    ...
    fit via scipy.optimize.curve_fit, same as page 3, so it's determined from AXCC1's own data.
...
Produces (NEW file):
    axcc1_uplift_phi_cosine_vs_time.pdf (3 pages)

Run with:
    python3 axcc1_uplift_phi_cosine_vs_time.py
"""
...
OUT_PDF = os.path.join(HERE, 'axcc1_uplift_phi_cosine_vs_time.pdf')
...
# AXCC1 has a genuine ~58-day raw-data gap (no events at all -- confirmed by inspecting the
...
def make_uplift_vs_cosine_page(roll, inflation_roll):
    ...
    since no user-dictated shift value exists for AXCC1 -- see module docstring)."""
...
def main():
    ...
    # Wider range than the AXEC2 script's -0.25/1.25 -- this station's re-inflation reaches
    # ~2.6 m (vs AXEC2's ~1.1 m), consistent with the caldera center inflating substantially
    # more than flank sites (see plot_bpr_detided_depth_and_inflation_ccal.py's docstring).
