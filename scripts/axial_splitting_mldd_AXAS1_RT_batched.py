...
#
# # Master Shear-Wave Splitting Workflow for Axial Seamount — AXAS1 (Real-Time, 2022-2026)
#
# Same pipeline as AXEC3, adapted for station AXAS1:
...
# ## 2. Load catalog and filter for AXAS1
...
axas1_catalog = catalog[catalog['station'] == 'AXAS1'].copy()
axas1_catalog = axas1_catalog.reset_index(drop=True)
axas1_catalog['datetime'] = pd.to_datetime(axas1_catalog['event_datetime'], format='ISO8601')

print(f"Total AXAS1 events in catalog: {len(axas1_catalog)}")
print(f"\nDate range:")
print(f"  Start: {axas1_catalog['datetime'].min()}")
print(f"  End:   {axas1_catalog['datetime'].max()}")

test_catalog = axas1_catalog.copy()
...
def get_station_traces_batch_axas1(df, filename, starttime, endtime, station_id, batch_size=250):
    """Batched bulk waveform retrieval for AXAS1 (EHE/EHN/EHZ channels)."""
    from obspy.clients.fdsn import Client
    from obspy.core.utcdatetime import UTCDateTime
    ...
    for batch_idx in range(total_batches):
        ...
        for _, row in batch_df.iterrows():
            ...
            if current_station == 'AXAS1':
                batch_bulk_list.append(('OO', 'AXAS1', '', 'EHE', t_start, t_final))
                batch_bulk_list.append(('OO', 'AXAS1', '', 'EHN', t_start, t_final))
                batch_bulk_list.append(('OO', 'AXAS1', '', 'EHZ', t_start, t_final))
...
for i in range(total_batches):
    ...
    batch_filename = f'../data/axial_mldd_2022_2026_axas1_batch_{i+1}'
    ...
    get_station_traces_batch_axas1(batch_df, filename=batch_filename,
...
passing_waveforms_dir = 'passing_waveforms_data_mldd_2022_2026_axas1'
...
if os.path.exists(passing_waveforms_metadata):
    ...
    for i in range(total_batches):
        batch_filename = f'../data/axial_mldd_2022_2026_axas1_batch_{i+1}.mseed'
...
passing_waveforms_reloaded = load_passing_waveforms(output_dir='passing_waveforms_data_mldd_2022_2026_axas1')
...
for i, batch in enumerate(batched_waveforms):
    ...
    base_name = f'splitting_results_mldd_2022_2026_axas1_batch_{batch_num}.csv'
...
save_results_csv(final_results,
                 file_name=os.path.join(results_dir, 'splitting_results_mldd_2022_2026_axas1_all_batches.csv'))
