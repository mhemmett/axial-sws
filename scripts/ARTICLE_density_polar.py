    ...
    for hyp_dist in hypdist_inters: 
        ...
        for kk,NCat in enumerate(NCats):   
            ...
            ax_polar=NCat.plot_polar_fastlag(colorbar=False,y_width=8,x_over=0.95,y_over=0.95,
                                             ...
                                             flag_resample=True,flag_filter=True,label_mode='azimuth',ax=ax_polar,plot_mode='contourf')
            
            ax_polar.grid(True,color='k',alpha=1,lw=0.2)
            ax_polar.tick_params(axis='y', colors='k')
...
#                
        ...
        figname_png='ARTICLE_density_polar_ms_%s_%s.png'%(station_name,z_suffix)
        figname_pdf='ARTICLE_density_polar_ms_%s_%s.pdf'%(station_name,z_suffix)
