...
Event: AXEC2, event_id 16_3842 (batch 16), origin 2015-01-31T22:12:19.124Z.
...
low-dt and high-dt companion events) meeting the same QC gate used throughout this repo
(e.g. lqt_pykonal_percent_anisotropy_quality_pages.py):
    quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    phi_error = 5 deg, dt_error = 0.010 s (both well inside phi_error<20 deg / dt_error<0.04 s)
    dt = 0.16 s (mid-range, between the 0.02 s low-dt and 0.29 s high-dt companion events)
    chosen filter band (try_filters): 5-10 Hz (chosen_filter_dom_period_samples=32.0, i.e.
        dominant frequency ~6.25 Hz at 200 Hz sampling)
    pre-eruption (< 2015-04-24 06:00 UTC)

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch16_event_...mseed)
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_3842_mid_dt_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch16_event_2015-01-31T22-12-19.124000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-01-31T22:12:19.124000Z')
S_ARRIVAL_TIME = 1.676   # seconds from origin
P_ARRIVAL_TIME = 0.876   # seconds from origin
BACK_AZIMUTH = 327.346230
INCIDENCE_PYKONAL_S = 15.212796   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 12.122836
RECTILINEARITY = 0.991184
