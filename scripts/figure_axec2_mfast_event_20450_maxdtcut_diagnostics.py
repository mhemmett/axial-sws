"""
figure_axec2_mfast_event_20450_maxdtcut_diagnostics.py

Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected to
test the alternative "dt < max_t_shift_s*0.8" (0.16s) QC cut explored this session, per explicit
user request: this event has dt=0.150s, which FAILS the standard cycle-skip-risk cut
(dt < T_dom/2 = 0.0691s, since T_dom=0.1382s here -- dt is ~2.17x over that threshold) but PASSES
the alternative fixed cut (0.150 < 0.16). The question being tested: is this event's large dt a
genuine measurement, or a cycle-skipped one that the T_dom-based cut correctly screens out but
the max_t_shift_s-based cut does not?

Event: AXEC2, event_id 82_20450 (batch 82), origin 2015-03-05T14:19:14.030Z (pre-eruption).

Selection: searched production_axec2_mfast_filters_maxdt02_lqt_pykonal_results/ for pre-eruption
events with quality>=0.5, 0.12<=dt<0.16, phi_error<10 deg, dt_error<0.05s (the alternative-cut
tier-5 criteria), rectilinearity>0.9, snr>10, incidence<20 deg, ranked by rectilinearity/snr
descending. This event tops that ranking and was re-verified on re-run with max_t_shift_s=0.2
explicitly set (matching production):
    Production: quality=0.937146, phi_error=7 deg, dt_error=0.010s, snr_horizontal=13.29,
                rectilinearity_jurkevics=0.9933, incidence_pykonal_s=18.22 deg, dt=0.15s,
                dominant_period=0.138182s (T_dom/2=0.0691s)
    RE-RUN (max_t_shift_s=0.2 explicitly set, matching production):
                phi=-22.0 deg, dt=0.150s, phi_error=7 deg, dt_error=0.010s, Q_w=0.937,
                T_dom=0.1382s (matches production exactly)
    chosen filter band (try_filters): 5-10 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch82_event_...mseed)
...
Pages: same 9-page structure as figure_axec2_mfast_event_73170_15hz_diagnostics.py (ZNE, LQT,
particle motion before/after, phi-dt parameter space [PAGE 5 -- inspect for cycle-skipping
signature: a secondary/competing minimum near dt~T_dom/2 that might be the TRUE non-cycle-skipped
solution, vs. the selected dt=0.15s minimum], DBSCAN clustering, ZNE composite summary,
cluster-stability + Q/T overlay, fast/slow alignment). Also prints the winning window's length
vs T_dom and max_t_shift_s, as before.

Run with: python3 figure_axec2_mfast_event_20450_maxdtcut_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_20450_maxdtcut_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch82_event_2015-03-05T14-19-14.030000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-03-05T14:19:14.030000Z')
S_ARRIVAL_TIME = 1.39   # seconds from origin
P_ARRIVAL_TIME = 0.66   # seconds from origin
BACK_AZIMUTH = 315.4655615285915
INCIDENCE_PYKONAL_S = 18.222744432426225   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 13.291478772608658
RECTILINEARITY = 0.9932664002405543
...
def main():
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='dt=0.15s Cycle-Skip Check',
        )
