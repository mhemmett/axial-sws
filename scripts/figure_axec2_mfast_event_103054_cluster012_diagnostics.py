...
Event: AXEC2, event_id 413_103054 (batch 413), origin 2019-03-08T15:35:48.330000.

One of a CLUSTER OF 4 EVENTS all sharing dt=0.12 s in the strictest cumulative-filtered AXEC2
mfast dataset (Q_w>=0.5, dt<=T_dom/2, phi_error<20 deg, dt_error<0.04 s -- N=12,550, see
rose_axec2_mfast_final_cumulative_filters.pdf) -- the next tier down from the single 0.22 s
outlier (figure_axec2_mfast_event_75564_outlier_dt_diagnostics.py), well above the population
median of 0.03 s.

Production metadata (raw_axec2_all_batches_mfast_filters_data metadata + splitting results):
    quality (Q_w, Wustefeld 2010) = 0.618301  (> 0.7 threshold: NO -- above the 0.5 filter floor but below the 0.7 true-split threshold)
    phi_error = 12 deg, dt_error = 0.025 s (both inside phi_error<20 deg / dt_error<0.04 s)
    dt = 0.12 s
    depth = 0.568 km, SNR = 8.36, rectilinearity = 0.919
    chosen filter band (try_filters): 3-5 Hz, dominant period T_dom = 51.2 samples
        @ 200 Hz = 0.256 s (dt/T_dom = 0.469, i.e. dt sits at
        47% of T_dom -- well inside the T_dom/2 = 0.128 s
        cycle-skip-risk threshold)

The MFAST-2.2 analysis window is defined in units of T_dom/2 (FIRST_WINDOW_START=2,
LAST_WINDOW_START=1, FIRST_WINDOW_END=1.5, LAST_WINDOW_END=2.5 -- see
run_production_axec2_mfast_filters_all_batches.py), so for THIS event's T_dom=0.256 s
the grid-search window spans from -1.00*T_dom (=-0.256 s before S-pick) to
+1.25*T_dom (=0.320 s after S-pick), a total length of 2.25*T_dom =
0.576 s. The single best-solution window is one specific (start,end) pair
drawn from the n_win x n_win grid inside that range and DBSCAN-clustered (smallest-variance
cluster, min combined dt+phi error observation) -- recovered here via a monkey-patch of
SWSPy's _sws_win_clustering to capture opt_obs_global_idx (see main()), same approach as the
0.22 s outlier script.

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch413_event_2019-03-08T15-35-48.330000Z.mseed)
was already detrended/tapered/bandpass-filtered to this event's own 3-5 Hz optimal
band when the mfast build script saved it -- no additional filtering is applied here.
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_103054_cluster012_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch413_event_2019-03-08T15-35-48.330000Z.mseed')
ORIGIN_TIME = UTCDateTime('2019-03-08T15:35:48.330000Z')
S_ARRIVAL_TIME = 0.97   # seconds from origin
P_ARRIVAL_TIME = 0.47   # seconds from origin
BACK_AZIMUTH = 296.962453
INCIDENCE_PYKONAL_S = 16.762249   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 8.358177
RECTILINEARITY = 0.919362
DEPTH_KM = 0.568
CHOSEN_FILTER_BAND = (3.0, 5.0)   # try_filters optimal band for this event
...
def plot_lqt_page(pdf, st_lqt, splitting_obj, best_win_info):
    ...
    best_start_abs = (s_time - t0) + best_win_info['start_rel_spick']
    best_end_abs = (s_time - t0) + best_win_info['end_rel_spick']
    ...
    for i, (ax, comp) in enumerate(zip(axes, comp_order)):
        ...
        ax.axvspan(best_start_abs, best_end_abs, color='limegreen', alpha=0.2, zorder=1,
                   label='Best-solution window' if i == 0 else None)
        ax.set_ylabel(f'{tr.stats.channel}\namplitude (counts)')
        if i == 0:
    ...
    fig.suptitle(
        ...
        f'(back-azi={BACK_AZIMUTH:.1f} deg, inc={INCIDENCE_PYKONAL_S:.1f} deg, depth={DEPTH_KM:.3f} km), '
        r'$T_{dom}$=' + f'{t_dom*1000:.1f} ms\n'
        f'Best-solution window (green): {best_win_info["start_rel_spick"]:+.3f}s to '
        f'{best_win_info["end_rel_spick"]:+.3f}s rel. S-pick '
        f'({best_win_info["start_Tdom"]:+.2f} to {best_win_info["end_Tdom"]:+.2f} '
        r'$T_{dom}$' + f'), length={best_win_info["len_s"]:.3f}s ({best_win_info["len_Tdom"]:.2f} '
        r'$T_{dom}$)',
        fontsize=9.5
    )
...
def plot_multipanel_summary_page(pdf, st_lqt, splitting_obj, st_q_uncorr, st_t_uncorr, st_q_corr, st_t_corr,
                                  best_win_info, title_suffix='High dt'):
    ...
    t_dom = splitting_obj.Tmid
    fig.suptitle(
        f'AXEC2 (mfast) Shear-Wave Splitting Analysis: {title_suffix} Event\n'
        f'{ORIGIN_TIME.strftime("%Y-%m-%dT%H:%M:%S")}Z, depth={DEPTH_KM:.3f} km, '
        r'$T_{dom}$=' + f'{t_dom*1000:.1f} ms, best-solution window '
        f'{best_win_info["start_Tdom"]:+.2f} to {best_win_info["end_Tdom"]:+.2f} '
        r'$T_{dom}$' + f' rel. S-pick (length {best_win_info["len_Tdom"]:.2f} ' + r'$T_{dom}$' + ')',
...
def main():
    ...
    # Monkey-patch _sws_win_clustering to capture opt_obs_global_idx (the flat index,
    # among n_win*n_win windows, of the specific window whose single measurement was
    # picked as the "best solution" - grid_search_idx = n_win*a + b, see
    # swspy/swspy/splitting/split.py::_phi_dt_grid_search). SWSPy does not store this
    # index anywhere on the object itself (only the resulting phi_dt_grid_best_window
    # array), so we capture it directly from the return value here.
    captured = {}
    orig_cluster_fn = splitting_obj._sws_win_clustering

    def _wrapped_cluster_fn(*args, **kwargs):
        result = orig_cluster_fn(*args, **kwargs)
        captured['result'] = result
        return result
    splitting_obj._sws_win_clustering = _wrapped_cluster_fn
    ...
    # Recover the best-solution window's own start/end (samples -> seconds relative to
    # S-pick -> fraction of T_dom), from the captured opt_obs_global_idx.
    opt_obs_global_idx = captured['result'][-1]
    win_idxs = splitting_obj.event_station_win_idxs[STATION]
    win_start_idxs = np.asarray(win_idxs['win_start_idxs'])
    win_end_idxs = np.asarray(win_idxs['win_end_idxs'])
    a = int(opt_obs_global_idx) // N_WIN
    b = int(opt_obs_global_idx) % N_WIN
    best_start_idx = win_start_idxs[a]
    best_end_idx = win_end_idxs[b]
    fs_tmp = splitting_obj.fs
    t_dom = splitting_obj.Tmid
    best_start_rel_spick = best_start_idx / fs_tmp - splitting_obj.overall_win_start_pre_fast_S_pick
    best_end_rel_spick = best_end_idx / fs_tmp - splitting_obj.overall_win_start_pre_fast_S_pick
    best_win_len_s = best_end_rel_spick - best_start_rel_spick
    print(f"Best-solution window: start={best_start_rel_spick:+.3f}s ({best_start_rel_spick/t_dom:+.2f} T_dom) "
          f"end={best_end_rel_spick:+.3f}s ({best_end_rel_spick/t_dom:+.2f} T_dom) "
          f"length={best_win_len_s:.3f}s ({best_win_len_s/t_dom:.2f} T_dom), relative to S-pick")
    ...
    best_win_info = dict(
        start_rel_spick=best_start_rel_spick, end_rel_spick=best_end_rel_spick,
        start_Tdom=best_start_rel_spick / t_dom, end_Tdom=best_end_rel_spick / t_dom,
        len_s=best_win_len_s, len_Tdom=best_win_len_s / t_dom,
    )

    with PdfPages(OUT_PDF) as pdf:
        plot_zne_page(pdf, st_zne)
        plot_lqt_page(pdf, st_lqt, splitting_obj, best_win_info)
        ...
        plot_multipanel_summary_page(
            ...
            best_win_info, title_suffix='0.12s Cluster',
        )
