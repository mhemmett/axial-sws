#!/usr/bin/env python3
"""
axec1_uplift_phi_cosine_vs_time.py

AXEC1 counterpart of axec2_uplift_phi_cosine_vs_time.py -- same structure, methodology, AND
BPR data source (Eastern Caldera BOTPT, RS03ECAL-MJ03E-06-BOTPTA302, via bpr_inflation_
periods.py -- AXEC1 and AXEC2 are both Eastern Caldera stations, so they share this same
...
etc.), not repeated in full here. Differences from the AXEC2 version:
  ...
  - Data: AXEC1 windowcheck grade-3 events (not AXEC2's), same bpr_inflation_periods module.
  - No "seismometer potentially unleveled" window is assumed here -- that flag was specific to
    ...
    AXEC1 shows a comparable anomaly has NOT been established, so this script does not shade or
    flag any window.
  - The cube-root fit's x-shift (page 2) is NOT hardcoded to 0.55 m (that value was specific to
    ...
    fit via scipy.optimize.curve_fit, same as page 3, so it's determined from AXEC1's own data.
...
Produces (NEW file):
    axec1_uplift_phi_cosine_vs_time.pdf (4 pages)

Run with:
    python3 axec1_uplift_phi_cosine_vs_time.py
"""
...
OUT_PDF = os.path.join(HERE, 'axec1_uplift_phi_cosine_vs_time.pdf')
...
# AXEC1 has a genuine ~58-day raw-data gap (no events at all -- confirmed by inspecting the
...
def make_uplift_vs_cosine_page(roll, inflation_roll):
    ...
    since no user-dictated shift value exists for AXEC1 -- see module docstring)."""
