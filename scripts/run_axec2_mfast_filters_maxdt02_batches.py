"""
Production splitting run for AXEC2, mfast-like per-event bandpass variant, with the SWSPy
grid-search maximum delay time constrained to 0.2s instead of the default 0.3s.

Identical to run_production_axec2_mfast_filters_all_batches.py (same raw mfast-filtered
waveform data, same LQT frame + PyKonal-FMM incidence 35 deg cut, same window/n_win/
s_pick_uncertainty), except max_t_shift_s=0.2 is passed through to swspy's splitting object
(splitting_obj.max_t_shift_s), constraining the phi/dt grid search itself to a physically
motivated maximum delay time (~20% shear-wave anisotropy over the typical S-wave travel
time through the well-sampled part of the Baillard velocity model, consistent with the
0.2-0.4s range used in other local volcano SWS studies) rather than relying on a post-hoc
dt cut applied after the unconstrained (0.3s) grid search.

All output paths are new (production_axec2_mfast_filters_maxdt02_lqt_pykonal_results/), so
nothing in production_axec2_mfast_filters_lqt_pykonal_results/ (the existing max_dt=0.3 run)
is overwritten. Resumable: skips batches whose results CSV already exists.

Run with:
    python3 run_axec2_mfast_filters_maxdt02_batches.py [--start N] [--end M]
"""
...
def main():
    ...
    print(f"max_t_shift_s override: {MAX_T_SHIFT_S}s (swspy default is 0.30s)")

    t_start = time.time()
    total_events = 0
    for batch_num in range(args.start, args.end + 1):
        out_csv = os.path.join(
            RESULTS_DIR, f'splitting_results_mldd_2015_2021_axec2_mfast_filters_maxdt02_batch_{batch_num}.csv')
