    ...
    for nlloc_file in nlloc_list:
            ...
            #for i_station in [3]:
                ...
                try:
                    ...
                    if os.path.exists(pickle_fullname):
                        print('Following file already exists, skip.\n %s'%(pickle_fullname))
                        continue
        ...
        #            
                    ...
                    if s_snr<=s_snr_thres:
                        print('s_snr is %f < %f threshold, skip observation'%(s_snr,s_snr_thres))
                        continue
...
results = pool.apply(run, args=([1,5],)) 
results = pool.apply(run, args=([6,10],)) 
results = pool.apply(run, args=([11,15],)) 
results = pool.apply(run, args=([16,19],)) 
results = pool.apply(run, args=([20,22],)) 
