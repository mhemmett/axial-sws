"""
Full-catalog raw QC rebuild for AXEC2, 2015-2021, mfast-like per-event bandpass variant.

Same pipeline as build_raw_axec2_all_batches.py (organize waveforms, compute SNR/
rectilinearity/back-azimuth/incidence, apply SNR+rectilinearity QC, cache passing
waveforms + metadata per batch), EXCEPT: instead of a single fixed 5-40 Hz bandpass
applied to every event, each event's raw (unfiltered) stream is run through
mfast_filters_functions.try_filters(), which two-pole-Butterworth-bandpasses a copy
of the stream once per candidate band (FILTER_BANDS in mfast_filters_functions.py),
scores each with the same Baillard S-wave SNR windowing logic already used in
production, and keeps only the highest-SNR filtered version.

That one chosen filtering is done exactly once per event and is what gets stored to
disk and carried through every later QC/metadata step (SNR, rectilinearity, back-
azimuth, P-Jurkevics incidence, PyKonal-FMM incidence) - none of those steps filter
internally (verified against splitting_functions.py), so the selected band is never
re-filtered or overwritten downstream. Unlike the standard build script, this script
does NOT call calculate_snr_for_organized_waveforms (which re-filters to a hardcoded
5-40 Hz internally) - snr_horizontal here IS the try_filters SNR for the chosen band.

All outputs use new, separate names/directories from the original 5-40 Hz production
build (raw_axec2_all_batches_data/, production_axec2_lqt_pykonal_results/) so nothing
existing is overwritten.

Run with:
    python3 build_raw_axec2_all_batches_mfast_filters.py [--start N] [--end M]
"""
...
from splitting_functions import (
    ...
    calculate_back_azimuth_for_organized_waveforms,
...
from mfast_filters_functions import FILTER_BANDS, try_filters
...
OUT_DIR = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data')
OUT_WAVEFORMS_DIR = os.path.join(OUT_DIR, 'waveforms')
OUT_METADATA_CSV = os.path.join(OUT_DIR, 'raw_axec2_all_batches_mfast_filters_metadata.csv')
...
def process_batch(batch_num, mseed_path, catalog, tracer):
    ...
    for eid in organized_waveforms.keys():
        ...
        organized_waveforms[eid]['s_arrival_time'] = s_arrival_time
        organized_waveforms[eid]['p_arrival_time'] = p_arrival_time

        event_row = {
            'datetime': organized_waveforms[eid]['datetime'],
            'p_arrival_time': p_arrival_time,
            's_arrival_time': s_arrival_time,
        }
        best_band, best_stream, best_snr, best_dom_period, _ = try_filters(st, event_row, FILTER_BANDS)
        if best_band is None:
            events_to_remove.append(eid)
            continue

        organized_waveforms[eid]['traces'] = best_stream
        organized_waveforms[eid]['snr_horizontal'] = best_snr
        organized_waveforms[eid]['chosen_filter_min'] = best_band[0]
        organized_waveforms[eid]['chosen_filter_max'] = best_band[1]
        organized_waveforms[eid]['chosen_filter_dom_period_samples'] = best_dom_period
    ...
    # NOTE: no calculate_snr_for_organized_waveforms call here (unlike the standard
    # build) - it re-filters to a hardcoded 5-40 Hz internally, which would discard
    # the per-event chosen filter above. snr_horizontal is already set from try_filters.
    ...
    for eid, ed in qc_passed.items():
        ...
        rows.append({
            ...
            'chosen_filter_min': ed.get('chosen_filter_min'), 'chosen_filter_max': ed.get('chosen_filter_max'),
