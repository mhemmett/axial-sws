#!/usr/bin/env python3
"""
axas2_uplift_phi_cosine_vs_time.py

AXAS2 / ASHES vent field BOTPT (RS03ASHS-MJ03B-09-BOTPTA304, Western Caldera) counterpart of
...
x-range handling, etc.), not repeated in full here. Differences from the AXEC2 version:
  ...
  - Data: AXAS2 windowcheck grade-3 events + bpr_inflation_periods_ashes (ASHES vent field
  ...
  - No "seismometer potentially unleveled" window is assumed here -- that flag was specific to
    ...
    AXAS2 shows a comparable anomaly has NOT been established, so this script does not shade or
    flag any window.
  - The cube-root fit's x-shift on page 2 is FIXED at 0 (per explicit user request); page 3's
    is a free parameter.
  - NO page corresponding to AXCC1/AXEC2's "uplift relative to the PRE-ERUPTION MAXIMUM uplift"
    page: that requires BPR data from before the 2015 eruption, which this instrument doesn't
    have at all (record starts 2017-08-15, over 2 years after the eruption -- see
    bpr_inflation_periods_ashes.py's docstring). That reference point cannot be computed here.

Three pages:
  1. Shared-axes time series: de-tided uplift, average fast direction, cosine similarity (vs.
     pre-eruption baseline, computed from the SPLITTING CATALOG's own pre-eruption AXAS2
...
Produces (NEW file):
    axas2_uplift_phi_cosine_vs_time.pdf (3 pages)

Run with:
    python3 axas2_uplift_phi_cosine_vs_time.py
"""
...
OUT_PDF = os.path.join(HERE, 'axas2_uplift_phi_cosine_vs_time.pdf')
...
# AXAS2 has a genuine ~58-day raw-data gap (no events at all -- confirmed by inspecting the
