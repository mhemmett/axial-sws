"""
figure_axec2_mfast_event_2_302_qwflip_windowcheck_diagnostics.py

Diagnostic multi-page PDF for AXEC2 event_id 2_302 (batch 2), run through
swspy.splitting.split_windowcheck (the 2026-07-24 windowing fix) instead of the production
swspy.splitting.split module. Companion to
figure_axec2_mfast_event_2_302_qwflip_production_diagnostics.py, which runs the SAME event
through unpatched production split.py.

Event selected from the AXEC2 10-batch test (batches 1-10,
production_axec2_mfast_filters_maxdt02_windowcheck_lqt_pykonal_results/) as a "high-quality,
affected-by-the-fix" example: quality/success/tier-5 QC all pass under BOTH production and the
window-check fix, and the reported best-fit phi/dt are IDENTICAL between the two runs (same
grid-search optimum found), but the reported Q_w quality metric is NOT:
    Production   (split.py):            phi=-82.36 deg, dt=0.020 s, phi_err=3 deg, dt_err=0.005 s,
                                         Q_w = -0.953 (fails a Q_w>=0.5 QC gate)
    Window-check (split_windowcheck.py): phi=-82.36 deg, dt=0.020 s, phi_err=3 deg, dt_err=0.005 s,
                                         Q_w = +1.000 (passes cleanly)
...
from -- silently flipping a genuinely well-constrained measurement from "fails QC" to "passes
QC" or vice versa, with no crash and no visible difference in phi/dt alone. Under the fix, every
window is well-formed from the start (end-window range widened so the shortest window is exactly
max_t_shift_s, per split_windowcheck.py's __init__), so clustering is stable and Q_w=1.000.

Selection: searched production_axec2_mfast_filters_maxdt02_windowcheck_lqt_pykonal_results/
batches 1-10 for tier-5-passing events (quality>=0.9, dt<T_dom/2, phi_error<10 deg,
...
to both T_dom and max_t_shift_s, for direct comparison against the production twin script.

Run with: python3 figure_axec2_mfast_event_2_302_qwflip_windowcheck_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_2_302_qwflip_windowcheck_diagnostics.pdf')
...
# ── Production splitting parameters (run_axec2_mfast_filters_maxdt02_windowcheck_batches.py) ─
...
def main():
    ...
    splitting_obj = swc.create_splitting_object(
        ...
        # attribute override, as the production diagnostics twin script does) -- the
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='Q_w Stability Check (Window-Check Fix)',
        )
