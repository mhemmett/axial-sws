...
Event: AXEC2, event_id 360_89944 (batch 360), origin 2017-03-30T16:09:42.588Z.

Selected from production_axec2_mfast_filters_lqt_pykonal_results/ (the new mfast try_filters
AXEC2 run) as a high-quality, high-delay-time event where the AD HOC RE-RUN
(create_splitting_analysis + perform_sws_analysis, same as this script's own pipeline) itself
yields Q_w >= 0.7 -- not just the stored production CSV's Q_w, which can differ from a re-run's
Q_w due to small waveform-trimming differences (see figure_axec1_event_23919 docstring). Found
via find_mfast_diagnostic_events.py, which re-runs each QC-passing candidate and keeps the
first whose RE-RUN also clears the gate:
    RE-RUN: quality (Q_w, Wustefeld 2010) = 1.000  (> 0.7 threshold)
    RE-RUN: phi_error = 2 deg, dt_error = 0.005 s (both well inside phi_error<20 deg / dt_error<0.04 s)
    RE-RUN: dt = 0.25 s (> 0.24 s -- the dt threshold used to separate "high dt" from "low dt" events)
    chosen filter band (try_filters): 5-10 Hz
    NOTE: this event is post-eruption (2017-03-30), not pre-eruption like the other companion
    events -- it was the first high-dt candidate whose re-run cleared Q_w>=0.7; no pre-eruption
    high-dt candidate in the searched pool did.

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch360_event_...mseed)
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_89944_high_dt_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch360_event_2017-03-30T16-09-42.588000Z.mseed')
ORIGIN_TIME = UTCDateTime('2017-03-30T16:09:42.588000Z')
S_ARRIVAL_TIME = 1.092   # seconds from origin
P_ARRIVAL_TIME = 0.562   # seconds from origin
BACK_AZIMUTH = 221.70370642183863
INCIDENCE_PYKONAL_S = 17.09233549514825   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 5.489195613553946
RECTILINEARITY = 0.8161513107125064
