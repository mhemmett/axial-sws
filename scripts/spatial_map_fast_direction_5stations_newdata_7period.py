...
Extends spatial_map_fast_direction_axcc1_axec1_axec2_newdata_7period.py (kept as-is, not
overwritten) with the two stations whose new-mfast max_dt=0.2s reruns completed 2026-07-23:
AXAS1 and AXEC3. Spatial map of fast-polarization direction (phi), 7-period columns, using
the new mfast max_dt=0.2s data for AXAS1/AXCC1/AXEC1/AXEC2/AXEC3 (AXAS2 excluded, no new-data
rerun yet) -- same data sources, QC, voxel grid (X:4-12, Y:0-12 km, 0.25 km cells),
bathymetry/fault/Kidiwela overlays, and yellow/white station-marker convention as
traveltime_anisotropy_7period_5stations_newdata.py, but:
...
Produces: spatial_map_fast_direction_5stations_newdata_7period.pdf (1 page, 4 depth rows x 7 period columns)

Run with:
    python3 spatial_map_fast_direction_5stations_newdata_7period.py
"""
...
OUT_PDF = os.path.join(HERE, 'spatial_map_fast_direction_5stations_newdata_7period.pdf')
...
def main():
    ...
    title = (f'AXAS1/AXCC1/AXEC1/AXEC2/AXEC3 (new mfast max_dt=0.2s data) — fast direction by epicenter '
