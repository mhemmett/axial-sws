#!/usr/bin/env python3
"""
axec3_uplift_phi_cosine_vs_time.py

AXEC3 counterpart of axec2_uplift_phi_cosine_vs_time.py -- same structure, methodology, AND
BPR data source (Eastern Caldera BOTPT, RS03ECAL-MJ03E-06-BOTPTA302, via bpr_inflation_
periods.py -- AXEC3 and AXEC2 are both Eastern Caldera stations, so they share this same
processed record, unlike the AXCC1/AXAS1/AXAS2 scripts which each use a different BPR pair).
See axec2_uplift_phi_cosine_vs_time.py's docstring for the full per-element rationale
(rolling-window centering fix, seismicity-bar percentile capping, common x-range handling,
etc.), not repeated in full here. Differences from the AXEC2 version:

  - Data: AXEC3 windowcheck grade-3 events (not AXEC2's), same bpr_inflation_periods module.
  - No "seismometer potentially unleveled" window is assumed here -- that flag was specific to
    ...
    AXEC3 shows a comparable anomaly has NOT been established, so this script does not shade or
    flag any window.
  - The cube-root fit's x-shift (page 2) is NOT hardcoded to 0.55 m (that value was specific to
    where AXEC2's own uplift-vs-cosine-similarity transition sits) -- it is a FREE parameter
    fit via scipy.optimize.curve_fit, same as page 3, so it's determined from AXEC3's own data.

Four pages:
  1. Shared-axes time series: de-tided uplift, average fast direction, cosine similarity (vs.
     pre-eruption baseline), and monthly seismicity rate (bottom bar strip).
  2. Post-eruption rolling-window scatter: uplift vs. cosine similarity, cube-root fit (free
     shift).
  3. Same scatter, x-axis = uplift minus the pre-eruption maximum uplift, cube-root fit (free
     shift).
  4. Same scatter, y-axis = signed angular difference from the pre-eruption baseline fast
     direction (degrees) instead of cosine similarity, cube-root fit (free shift).

Produces (NEW file):
    axec3_uplift_phi_cosine_vs_time.pdf (4 pages)

Run with:
    python3 axec3_uplift_phi_cosine_vs_time.py
"""
...
OUT_PDF = os.path.join(HERE, 'axec3_uplift_phi_cosine_vs_time.pdf')
...
# AXEC3 has a genuine ~58-day raw-data gap (no events at all -- confirmed by inspecting the
...
def make_uplift_vs_cosine_page(roll, inflation_roll):
    """Post-eruption rolling-window uplift vs. cosine similarity + cube-root fit (free shift,
    since no user-dictated shift value exists for AXEC3 -- see module docstring)."""
    ...
    ax.plot(x_line, y_line, color='black', lw=1.5, linestyle='--',
           label=f'Cube-root fit (free shift): cos = {a:.2f}$\\cdot$(uplift {c_sign} {abs(c):.2f})$^{{1/3}}$ + {b:.2f}\n'
                 f'(r = {r:.2f}, N = {len(x)})')
...
X_SHIFT_DIFF = -1.4   # fixed per explicit user request (was free-fit before)


def make_uplift_diff_vs_cosine_page(roll, inflation_roll):
    """Page 3: same scatter, x-axis = uplift - pre-eruption maximum uplift, cube-root fit
    with a FIXED shift (X_SHIFT_DIFF = -1.4 m, per explicit user request -- previously a
    free-fit parameter)."""
    ...
    c_sign = '-' if c >= 0 else '+'
    ax.plot(x_line, y_line, color='black', lw=1.5, linestyle='--',
           label=f'Cube-root fit (shift={c:.2f}): cos = {a:.2f}$\\cdot$(diff {c_sign} {abs(c):.2f})$^{{1/3}}$ + {b:.2f}\n'
                 f'(r = {r:.2f}, N = {len(x)})')
    ...
    ax.set_ylabel('Cosine similarity (vs. pre-eruption)')
    ax.set_title(f'{STATION}: Uplift Relative to Pre-Eruption Maximum vs. Cosine Similarity\n'
                 '(post-eruption rolling windows)', fontsize=11, fontweight='bold')
    ax.legend(loc='lower right', fontsize=8, framealpha=0.9)
    ax.grid(alpha=0.3)
    fig.tight_layout()
...
def make_uplift_vs_phidiff_page(roll, inflation_roll):
    ...
    uplift vs. y), except y = the SIGNED angular difference from the pre-eruption baseline
    fast direction (degrees, _axial_phi_diff_deg) instead of cosine similarity."""
    valid = roll.dropna(subset=['mean_phi', 'cos_sim']).copy()
    valid = valid[valid['t'] >= ERUPTION_END]
    ...
    c_sign = '-' if c >= 0 else '+'
    ax.plot(x_line, y_line, color='black', lw=1.5, linestyle='--',
           label=f'Cube-root fit (free shift): $\\Delta\\phi$ = {a:.2f}$\\cdot$(uplift {c_sign} {abs(c):.2f})$^{{1/3}}$ + {b:.2f}\n'
                 f'(r = {r:.2f}, N = {len(x)})')
...
def main():
    ...
    # Same BPR record as AXEC2 (both Eastern Caldera stations), so the same uplift range.
    ...
    print('\nBuilding page 2: uplift vs. cosine-similarity scatter + cube-root fit '
          '(free shift, post-eruption windows only)...')
