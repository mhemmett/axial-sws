#!/usr/bin/env python3
"""
traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff_syneruption_ref_1row.py

Syn-eruption-reference variant of
...
that script's docstring, not repeated here). Still all 7 original eruption-cycle panels
(Pre-eruption, Syn-eruption, 5 equal-count post-eruption bins), and Pre-eruption is back to
being a SINGLE panel (not split into thirds, per explicit user request -- undoing the earlier
"_preeruption3split" variant's pre-eruption breakdown for THIS figure).

The key change from the original figure: instead of Pre-eruption being the absolute/reference
(Blues) panel and everything else diffed against it, SYN-ERUPTION is now the absolute/reference
panel (plotted first, in Blues, with its own true-max colorbar), and ALL 6 remaining panels
(Pre-eruption and the 5 post-eruption bins) are plotted as DIFFERENCES from Syn-eruption
(RdBu_r, true-max symmetric colorbar), positioned to the right of the Syn-eruption colorbar --
per explicit user request. This is a NEW, distinctly-named script/output so the existing
Pre-eruption-referenced figure is not overwritten.

Produces:
    traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff_syneruption_ref_1row.pdf
(1 page, 1 row, 7 panels) -- a NEW file, does not touch any existing output.

Run with:
    python3 traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff_syneruption_ref_1row.py
"""
...
def make_diff_page(results, period_labels, title, depth_rows, fields):
    ...
    if last_h_abs:
        ...
        cb.set_label('Percent Anisotropy Syn-Eruption', fontsize=7.5)
    ...
    if last_h_diff:
        ...
        cb2 = plt.colorbar(last_h_diff, cax=cax2, ticks=np.linspace(-d_vmax, d_vmax, 5),
                           format=FormatStrFormatter('%.1f'))
        cb2.set_label('Change in Percent Anisotropy from Syn-Eruption Period', fontsize=7.5)
...
def main():
    ...
    periods_orig = T.build_7_periods(pd.DataFrame({'t': all_t}))
    # periods_orig = [Pre-eruption, Syn-eruption, post1..post5]. Reorder so Syn-eruption is
    # panel 0 (the absolute/reference panel) -- Pre-eruption moves to panel 1 (now a diff panel,
    # kept as a single panel, not split), the 5 post-eruption bins keep their original order.
    reorder = [1, 0, 2, 3, 4, 5, 6]
    periods = [periods_orig[i] for i in reorder]
    period_labels = [periods_orig[i][0] for i in reorder]
    ...
    title = 'Percent Anisotropy Across an Eruption Cycle (Referenced to Syn-Eruption)'
