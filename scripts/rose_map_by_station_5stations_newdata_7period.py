#!/usr/bin/env python3
"""
rose_map_by_station_5stations_newdata_7period.py

Extends rose_map_by_station_axcc1_axec1_axec2_newdata_7period.py (kept as-is, not overwritten)
with the two stations whose new-mfast max_dt=0.2s reruns completed 2026-07-23: AXAS1 and
AXEC3. 7-period spatial map where, instead of tick marks or a voxel field, each panel shows
the FULL ROSE DIAGRAM (doubled-angle fast-direction histogram, same drawing convention as
rose_7period_5stations_newdata.py's _draw_rose) for each station, drawn as a polar inset
positioned AT that station's actual (x,y) location on top of the grayscale bathymetry map --
using sws_methods.get_ax_inset (the same inset-axes helper documented for this exact purpose
in sws_methods.py). Ring faults and the two Kidiwela sources are also drawn, matching the
sibling scripts' style (traveltime_anisotropy_7period_5stations_newdata.py /
spatial_map_fast_direction_5stations_newdata_7period.py).

Data: same new mfast max_dt=0.2s data + QC as the other new-data scripts this session
(AXAS1/AXCC1/AXEC1/AXEC2/AXEC3; AXAS2 excluded, no new-data rerun yet; success, dt>0,
quality>=0.5, dt<T_dom/2, phi_error<20 deg, dt_error<0.05s). 7 eruption-relative periods
(verbatim scheme).

Produces: rose_map_by_station_5stations_newdata_7period.pdf (1 page, 7 panels)

Run with:
    python3 rose_map_by_station_5stations_newdata_7period.py
"""
...
OUT_PDF = os.path.join(HERE, 'rose_map_by_station_5stations_newdata_7period.pdf')
...
ROSE_WIDTH_KM = 0.65   # inset polar-plot diameter, in map data (km) units -- shrunk from the
                       # 3-station sibling's 1.3 km since AXEC1/AXEC2/AXEC3 sit only
                       # ~0.5-1.2 km apart and would otherwise overlap
...
def main():
    ...
    title = ('AXAS1/AXCC1/AXEC1/AXEC2/AXEC3 (new mfast max_dt=0.2s data) — per-station rose diagrams '
