#!/usr/bin/env python3
"""
dt_vs_dominant_period_axec1_windowcheck_partial_grade3_vs_dt016.py

Delay time (dt, seconds) vs. per-event dominant period (T_dom, seconds), 2D histogram
(pcolormesh, LogNorm count scale) -- same style/structure as
dt_vs_depth_axec1_windowcheck_partial_grade3_vs_dt016.py, just T_dom instead of depth on the
x-axis -- for the NEW partial AXEC1 window-check rerun only
...
NOTE on the Grade-3 panel: dt<=T_dom/2 is a hard constraint of that filter itself, so ANY
positive dt-vs-T_dom correlation there is at least partly a built-in ceiling effect (dt is
mechanically bounded above by T_dom/2), not necessarily a physical relationship -- interpret
that panel's regression with that in mind. The Grade-3 (dt<=0.16s) panel has no such built-in
coupling to T_dom, so a correlation (or lack of one) there is more directly interpretable.

Page 1: all depths. Page 2 (same depth cut used in the dt-vs-depth companion script): the SAME
two filters, restricted to events with source depth <= 1.5 km only.

For every page/filter combination, prints a linear regression of dt on T_dom (Pearson r, R^2,
slope, intercept).

Produces:
    dt_vs_dominant_period_axec1_windowcheck_partial_grade3_vs_dt016.pdf
(2 pages, 2 panels each) -- a NEW file, does not touch any existing output.

Run with:
    python3 dt_vs_dominant_period_axec1_windowcheck_partial_grade3_vs_dt016.py
"""
...
OUT_PDF = os.path.join(HERE, 'dt_vs_dominant_period_axec1_windowcheck_partial_grade3_vs_dt016.pdf')
...
DEPTH_MAX_SUBSET = 1.5   # km -- page 2, same cut as the dt-vs-depth companion script
...
def print_regression(subs, page_label):
    print(f'\n  Linear regression of dt on T_dom -- {page_label}:')
    for sub, filt in zip(subs, FILTERS):
        if len(sub) < 3:
            print(f'    {filt["key"]}: N={len(sub)} too few points for regression')
            continue
        r, p = stats.pearsonr(sub['dominant_period'], sub['dt'])
        slope, intercept, r_lr, p_lr, se = stats.linregress(sub['dominant_period'], sub['dt'])
        print(f'    {filt["key"]}: N={len(sub):,}  Pearson r={r:.4f} (p={p:.2e})  '
              f'slope={slope:.5f} s/s  intercept={intercept:.4f}s  R^2={r_lr**2:.4f}')


def make_figure(subs, title_suffix=''):
    tdom_min = min(s['dominant_period'].min() for s in subs)
    tdom_max = max(s['dominant_period'].max() for s in subs)
    dt_max = max(s['dt'].max() for s in subs)
    tdom_edges = np.linspace(tdom_min, tdom_max, N_TDOM_BINS + 1)
    ...
    for ax, sub, filt in zip(axes, subs, FILTERS):
        ...
        dt = sub['dt'].values
        counts, xe, ye = np.histogram2d(tdom, dt, bins=[tdom_edges, dt_edges])
        im = ax.pcolormesh(xe, ye, counts.T, cmap='viridis',
                           norm=mcolors.LogNorm(vmin=1, vmax=max(counts.max(), 2)))
        fig.colorbar(im, ax=ax, label='Count')
        ax.set_xlabel('Dominant period $T_{dom}$ (s)', fontsize=10, fontweight='bold')
    ...
    title = 'AXEC1 (window-check partial rerun): delay time vs. dominant period'
    if title_suffix:
