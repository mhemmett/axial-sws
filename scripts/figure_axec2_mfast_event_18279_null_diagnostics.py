"""
figure_axec2_mfast_event_18279_null_diagnostics.py

Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected as
a CLEARLY NULL measurement: Q_w = -0.984287 on re-run (the discretized floor value, i.e. the
algorithm's most confident "no measurable splitting" outcome -- same convention as the older
figure_axec2_event_diagnostics.py's event 35382, "qnull"), with a small, well-constrained dt
(not itself in cycle-skip territory) and low phi_error, so the null classification isn't
confounded by a noisy/ambiguous measurement.

Event: AXEC2, event_id 74_18279 (batch 74), origin 2015-03-02T00:01:37.340Z (pre-eruption).
...
production quality < -0.9 (confident null), dt < T_dom/2 (not a cycle-skip artifact),
phi_error < 15 deg, incidence < 20 deg, snr > 8, rectilinearity > 0.9, ranked by phi_error
ascending. This event tops that ranking:
    Production: quality=-0.984287, phi_error=2 deg, dt_error=0.005s, snr_horizontal=10.71,
                rectilinearity_jurkevics=0.9713, incidence_pykonal_s=16.10 deg, dt=0.01s
    RE-RUN (create_splitting_analysis + perform_sws_analysis, same pipeline as this script):
                phi=-44.0 deg, dt=0.010s, phi_error=2 deg, dt_error=0.005s, Q_w=-0.984
                (re-run reproduces the production quality almost exactly -- a clean, confident
                null, not an artifact of re-run trimming drift)
    chosen filter band (try_filters): 10-20 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch74_event_...mseed)
...
Pages:
  ...
  4. Splitting-corrected LQT Q-T particle motion, same window (expect this to look ALREADY
     near-linear even before correction, and barely change after -- the hallmark of a null:
     little/no splitting to remove)
  ...
  8. Cluster stability (phi and dt vs. window number, with Silver & Chan 1991 error bars, for
     the winning DBSCAN cluster) + Q and T wavelets, initial vs. corrected overlaid on shared
  ...
  9. Fast and slow wavelets overlaid on one seismogram (fast = solid black, slow = dashed red),
     ...
     +dt/2 / -dt/2 shift remove_splitting() applies internally) -- expect fast/slow to already
     look similar even before correction, consistent with a null

Run with: python3 figure_axec2_mfast_event_18279_null_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_18279_null_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch74_event_2015-03-02T00-01-37.340000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-03-02T00:01:37.340000Z')
S_ARRIVAL_TIME = 0.77   # seconds from origin
P_ARRIVAL_TIME = 0.38   # seconds from origin
BACK_AZIMUTH = 227.93936188937673
INCIDENCE_PYKONAL_S = 16.10464903548693   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 10.707697839445387
RECTILINEARITY = 0.971342652585496
