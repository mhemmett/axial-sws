...
anisotropy strength m [s/km]. "Ultra strict" = phi_error<10 & dt_error<0.01 cuts.
...
Outputs:
  sws_tomography_dt_7period_phi20_dt004_fractional_anisotropy.pdf
  sws_tomography_dt_annual_phi20_dt004_fractional_anisotropy.pdf
"""
...
# COUNT_MIN_HOURLY: minimum total rays in the 1-hour bin before attempting
# the inversion at all. MIN_PATH_KM_HOURLY: ~0, i.e. include ANY voxel
# touched by at least some ray path ("any data"), not a minimum-count cut.
# HOURLY_STD_MIN: minimum rays per voxel needed to compute a circular std-dev
# (need >=2 observations for a variance).
...
def voxel_circ_std(A_sub, phi_sub, count_min=COUNT_MIN):
    ...
    that touch that voxel (A_sub sparsity pattern), independent of the smooth
    inversion theta field. NaN where fewer than count_min rays touch the voxel."""
    A_csc=A_sub.tocsc()
...
def make_page(results,title,count_min=COUNT_MIN,merge_depth=False):
    ...
    for ri,(z0,zlbl) in enumerate(depth_rows):
        if z0 is None:
            iz0,iz1=0,NZ   # merge ALL depth slices into one row
        else:
...
def make_phi_stickvar_page(results,title,std_vmax=None,merge_depth=False):
    cmap_phi=plt.colormaps['hsv_r']; cmap_std=plt.colormaps['viridis']
    ...
    for ri,(z0,zlbl) in enumerate(depth_rows):
        ...
        for ci,res in enumerate(results):
            ax=fig.add_subplot(gs[ri,ci]); _bathy(ax)
            if z0 is None:
                # circular mean over ALL depth slices (vector-averaged, not arithmetic)
...
out1=os.path.join(BASE,'sws_tomography_dt_7period_phi20_dt004_fractional_anisotropy_CORRECTED.pdf')
...
out2=os.path.join(BASE,'sws_tomography_dt_annual_phi20_dt004_fractional_anisotropy_CORRECTED.pdf')
...
# ── New φ contourf pages (item 1) ──────────────────────────────────────────────
out3=os.path.join(BASE,'sws_tomography_phi_7period_dt004_fractional_anisotropy_CORRECTED_contourf.pdf')
paginate_and_save(results_7,make_phi_contourf_page,
                   f'SWS tomography — fast direction φ contourf (7 periods)  {tag}',out3,chunk=7)

out4=os.path.join(BASE,'sws_tomography_phi_annual_dt004_fractional_anisotropy_CORRECTED_contourf.pdf')
paginate_and_save(results_a,make_phi_contourf_page,
                   f'SWS tomography — fast direction φ contourf (annual)  {tag}',out4,chunk=7)

# ── New φ stick maps colored by circular std-dev (item 2) ─────────────────────
...
out5=os.path.join(BASE,'sws_tomography_phi_7period_dt004_fractional_anisotropy_CORRECTED_stickvar.pdf')
paginate_and_save(results_7,make_phi_stickvar_page,
                   f'SWS tomography — φ sticks colored by circular std-dev (7 periods)  {tag}',
                   out5,chunk=7,std_vmax=std_vmax_static)

out6=os.path.join(BASE,'sws_tomography_phi_annual_dt004_fractional_anisotropy_CORRECTED_stickvar.pdf')
paginate_and_save(results_a,make_phi_stickvar_page,
                   f'SWS tomography — φ sticks colored by circular std-dev (annual)  {tag}',
                   out6,chunk=7,std_vmax=std_vmax_static)

# ── Hourly window: 24h pre-eruption + first 24h syn-eruption (item 4) ─────────
# Same grid layout as the 7-period/annual pages (columns = time periods,
# paginated in chunks), but each panel merges ALL depth slices into one row
# (merge_depth=True) and uses the looser hourly coverage thresholds so any
# voxel with data is included.
out7=os.path.join(BASE,'sws_tomography_dt_hourly48_dt004_fractional_anisotropy_CORRECTED.pdf')
paginate_and_save(results_h,make_page,
                   f'SWS tomography — shear anisotropy ΔVs/Vs (hourly, eruption onset window, all depths)  {tag}',
                   out7,chunk=8,count_min=COUNT_MIN_HOURLY,merge_depth=True)

out8=os.path.join(BASE,'sws_tomography_phi_hourly48_dt004_fractional_anisotropy_CORRECTED_stickvar.pdf')
...
paginate_and_save(results_h,make_phi_stickvar_page,
                   f'SWS tomography — φ sticks colored by circular std-dev (hourly, eruption onset window, all depths)  {tag}',
