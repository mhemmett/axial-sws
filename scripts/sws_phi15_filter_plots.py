#!/usr/bin/env python3
"""
sws_phi15_filter_plots.py

Applies phi_error ≤ 15° filter (no dt_error cut) and generates 4 PDFs:

  sws_temporal_combined_phi_annual_kidiwela_source_phi15.pdf
  sws_temporal_combined_dt_annual_kidiwela_source_phi15.pdf
  sws_temporal_combined_phi_kidiwela_source_phi15.pdf        (7 periods)
  sws_temporal_combined_dt_kidiwela_source_phi15.pdf         (7 periods)

7-period boundaries (pre, syn, + 5 post-eruption quintiles) are computed
from the ALL-stations combined dataset and applied consistently to every page.
"""
...
# Four equal depth slices spanning 0–2.5 km
Z_HALF_WIDTH  = 0.3125
...
def build_7_periods(all_df):
    """5 post-eruption quintiles from ALL-stations combined data."""
...
def xyzd2mesh(x, y, z, d_phi, d_dt, z0):
    ...
    return Xg,Yg,gaussian_filter(PHI,GAUSS_SIGMA),gaussian_filter(DT,GAUSS_SIGMA),CNT

# ── Page builders ─────────────────────────────────────────────────────────────
def make_page(param, df_or_dict, periods, highlight, title, dt_vmax=None):
    cmap_phi = plt.colormaps['hsv_r']
    cmap_dt  = plt.colormaps['Blues']
    vmin_phi,vmax_phi = 0,180
    vmin_dt, vmax_dt  = 0, dt_vmax or 0.15
        ...
        for ci,((lbl,_,__),sub,neq) in enumerate(zip(periods,subs,n_eqs)):
            ...
            if len(sub) >= COUNT_MIN:
                ...
                if param == 'phi':
                    ...
                    last_h = ScalarMappable(cmap=cmap_phi,
                                            norm=Normalize(vmin_phi,vmax_phi))
                    last_h.set_array([])
                else:
                    DT_plot = np.where(mask, DT, np.nan)
                    ax.contourf(Xg,Yg,DT_plot,
                                levels=np.linspace(vmin_dt,vmax_dt,15),
                                cmap=cmap_dt,vmin=vmin_dt,vmax=vmax_dt,
                                extend='neither')
                    last_h = ScalarMappable(cmap=cmap_dt,
                                            norm=Normalize(vmin_dt,vmax_dt))
                    last_h.set_array([])
            ...
            # Count events in this depth slice for this period
            n_slice = int(np.sum(np.abs(sub['z'].values - z0) <= Z_HALF_WIDTH)) \
                      if len(sub) > 0 else 0
            if ri==0: ax.set_title(f'{lbl}\nN={neq:,}',fontsize=7.5,
                                    fontweight='bold',pad=1)
            if ci==0: ax.set_ylabel(
                f'{DEPTH_SLICE_LABELS[ri]}\n(n={n_slice:,})',fontsize=7)
    ...
    if last_h:
        ...
        if param == 'phi':
            ...
            ticks = np.linspace(vmin_dt,vmax_dt,5)
            cb = plt.colorbar(last_h,cax=cax,ticks=ticks,
                              format=FormatStrFormatter('%.2f'))
            cb.set_label('δt [s]',fontsize=9)
        cb.ax.tick_params(labelsize=7)
    fig.suptitle(title,fontsize=11,fontweight='bold',y=0.97)
    return fig

# ── Load data ──────────────────────────────────────────────────────────────────
print(f'Loading data (phi_error ≤ {PHI_ERR_MAX}°, no dt_error cut)...')
...
# ALL-stations combined — used to set consistent period boundaries
all_df = pd.concat(dfs.values(), ignore_index=True)
print(f'  Total: {len(all_df):,}')

# Build periods from ALL data (consistent across every page)
periods_7      = build_7_periods(all_df)
periods_annual = build_annual_periods()

# dt vmax from 99th percentile of all data
dt_vmax_all = float(np.percentile(all_df['dt'].values, 99))
label = f'(φ_error ≤ {PHI_ERR_MAX}°)'

# ── Generate 4 PDFs ────────────────────────────────────────────────────────────
pdfs_to_make = [
    ('phi',    periods_annual, 'annual',
     'sws_temporal_combined_phi_annual_kidiwela_source_phi15.pdf'),
    ('dt',     periods_annual, 'annual',
     'sws_temporal_combined_dt_annual_kidiwela_source_phi15.pdf'),
    ('phi',    periods_7,     '7-period',
     'sws_temporal_combined_phi_kidiwela_source_phi15.pdf'),
    ('dt',     periods_7,     '7-period',
     'sws_temporal_combined_dt_kidiwela_source_phi15.pdf'),
]

for param, periods, period_tag, fname in pdfs_to_make:
    out = os.path.join(OUT_DIR, fname)
    print(f'\nWriting {fname}...')
    with PdfPages(out) as pdf:
        for sta in STATIONS:
            vmax = (float(np.percentile(dfs[sta]['dt'].values,99))
                    if param=='dt' else None)
            fig = make_page(param, dfs[sta], periods,
                            highlight=sta,
                            title=f'{sta} — {"φ principal stress" if param=="phi" else "δt"} '
                                  f'{period_tag} {label}',
                            dt_vmax=vmax)
        ...
        fig = make_page(param, dfs, periods,
                        highlight=None,
                        title=f'All stations — {"φ principal stress" if param=="phi" else "δt"} '
                              f'{period_tag} {label}',
