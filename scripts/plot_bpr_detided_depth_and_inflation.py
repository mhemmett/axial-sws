#!/usr/bin/env python3
"""
plot_bpr_detided_depth_and_inflation.py

Two-panel figure from the de-tided Central Caldera BOTPT seafloor-depth record
(process_bpr_detided_depth.py's output), per explicit user request:
  ...
  B. "m of uplift" (inflation), referenced to zero at the lowest point observed right after the
     2015 eruption (2015-05-17, via a 5-day-smoothed search -- see bpr_inflation_periods.py's
     docstring for why raw daily data isn't used to locate this point), showing the raw daily
     inflation AND a 30-day rolling mean overlay (per explicit user follow-up: "plot a rolling
     mean line"). The 2015 eruption window is marked on both panels.

The rolling-mean inflation curve (not the raw daily one) is what bpr_inflation_periods.py uses
to define the 5 equal-inflation-amount post-eruption bins for the companion AXEC2 rose plot --
this figure's panel B boundary markers are exactly those bin edges, so the two figures agree.

Produces (NEW file):
    bpr_detided_depth_and_inflation.pdf

Run with:
    python3 plot_bpr_detided_depth_and_inflation.py
"""
...
OUT_PDF = os.path.join(HERE, 'bpr_detided_depth_and_inflation.pdf')
...
def main():
    ...
    ax_top.set_title('Central Caldera BOTPT (RS03ECAL-MJ03E-06-BOTPTA302): De-tided Seafloor '
