...
def _find_multi_layer_splitting_pairs_for_direct_method(phi_app, dt_app, rotate_step_deg, max_t_shift_s, fs, dom_freq_Hz):
    ...
    # Define max t shift from MFAST-like windowing
    # Earliest window start = 
    #self.overall_win_start_pre_S_pick
    # Latest window end = 
    #self.overall_win_end_post_S_pick
    # Therefore max t shift = (latest window end - earliest window start) * fs
...
class create_splitting_object:
    ...
    def __init__(self, st, nonlinloc_event_path=None, event_uid=None, stations_in=[], S_phase_arrival_times=[], origin_times = [], back_azis_all_stations=[], receiver_inc_angles_all_stations=[], first_window_start=2, last_window_start=1, first_window_end=1.5, last_window_end=2.5, n_win=10, s_pick_uncertainty=0.0509, max_t_shift_s=0.30):
        ...
        # Define attributes:
        s_time = S_phase_arrival_times[0]

        origin_time = origin_times[0]

        self.s_time_relative = s_time - origin_time

        print(f"S-pick time relative to start of trace: {self.s_time_relative}")

        # Calc dominant period Tmid based on initial frequency analysis window
        freq_win = [0.1, 0.3]
        start_time = UTCDateTime(s_time - freq_win[0])
        end_time = UTCDateTime(s_time + freq_win[1])
        st_freq_win = st.copy().trim(start_time, end_time)
        fs = st_freq_win[0].stats.sampling_rate
        data_arr_E = st_freq_win.select(component="E")[0].data
        data_arr_N = st_freq_win.select(component="N")[0].data

        #n = len(data_arr_E)
        #freqs = np.fft.rfftfreq(n, d=1/fs)
        #psd_E = np.abs(np.fft.rfft(data_arr_E))**2
        #psd_N = np.abs(np.fft.rfft(data_arr_N))**2
        #psd_sum = psd_E + psd_N

        # Exclude DC component (index 0) to avoid division by zero
        # Also optionally clamp to a physically reasonable frequency range
        #valid_mask = freqs > 0  # exclude DC bin
        # Optionally also apply a frequency band, e.g. 1-50 Hz:
        #valid_mask = (freqs > 4.0) & (freqs <= 20.0) # The dominant frequency should fall into our bandpass range - also avoid 0 freq for np.inf error.

        #if not np.any(valid_mask):
        #    T_mid = 0.125  # fallback: 8 Hz, the mean dominant frequency for the dataset
        #else:
        #    valid_freqs = freqs[valid_mask]
        #    valid_psd = psd_sum[valid_mask]
        #    max_power_idx = np.argmax(valid_psd)
        #    f_mid = valid_freqs[max_power_idx]
        #    T_mid = 1.0 / f_mid

        (dom_period_E, dom_freq_E) = get_dominant_period_baillard(data_arr_E, fs=fs)
        (dom_period_N, dom_freq_N) = get_dominant_period_baillard(data_arr_N, fs=fs)

        print(f"Dominant period E-comp: {dom_period_E}")
        print(f"Dominant period N-comp: {dom_period_N}")

        T_mid = np.mean([dom_period_E,dom_period_N]) / 200 # Take the mean dominant period, and convert it back to seconds for consistency

        print(f"Dominant period: {T_mid}")

        # Old definition: no masking
        # And calculate Tmid as the period corresponding to the frequency with maximum power:
        #max_power_idx = np.argmax(psd_sum)
        #f_mid = freqs[max_power_idx]
        #T_mid = 1 / f_mid

        # Dominant period - controls window end times
        self.Tmid = T_mid

        # Define S-pick uncertainty
        #self.Terr = 0.0509
        self.Terr = s_pick_uncertainty

        # Check: make sure the first window starts before the last window (to avoid negative indexing errors!)
        # Ex: 2σ > 1σ (In this case greater magnitude means earlier time, since this is defined as time BEFORE the S-pick)
        if first_window_start > last_window_start:
            
            # First window start time is a whole integer of the S-pick uncertainty, before the S-pick (i.e, 2 or 3 sigma before S-pick)
            #self.overall_win_start_pre_fast_S_pick = 2 * self.Terr # Start 2 sigma pick uncertainty before the S-pick
            self.overall_win_start_pre_fast_S_pick = first_window_start * self.Terr

            print(f"First window start time before S-pick, in seconds: {self.overall_win_start_pre_fast_S_pick}")

            # Last window start time is a whole integer of the S-pick uncertainty, before or up to the S-pick (i.e, 1 or 0)
            #self.win_S_pick_tolerance = 0. # Start windows right up until the S-pick
            self.win_S_pick_tolerance = last_window_start * self.Terr

            print(f"Last window start time before S-pick, in seconds: {self.win_S_pick_tolerance}")

        else:
            raise ValueError("First window start time must be greater than last window start time!")

        # Define maximum time shift allowed... should be based on volcanic setting! Set as 0.25 seconds as a maximum ceiling, based on Baillard's results
        # (Moved up from below the window-end-time block so it is available for the undersized-
        # window check immediately below. Now a constructor param -- the production pipeline's
        # override, e.g. 0.2s, should be passed in here directly, not set as a post-construction
        # attribute -- so that the window layout itself is derived from the CORRECT value.)
        self.max_t_shift_s = max_t_shift_s # Baillard allows 60 samples maximum at 200 Hz, which is 0.3 seconds. Default here matches that (0.30s).

        # Check: make sure the first window starts before the last window (to avoid negative indexing errors!)
        # Ex: 2.5 > 1.5 (In this case greater magnitude means later time, since it is defined AFTER the S-pick)
        if last_window_end > first_window_end:

            # First window end time is related to the number of dominant period cycles (i.e, 1, 1.5, or 2)
            #self.overall_win_start_post_fast_S_pick = 1.5 * self.Tmid # 1.5 * avg Tmid for the dataset, first window end time
            self.overall_win_start_post_fast_S_pick = first_window_end * self.Tmid
            print(f"First window end time after S-pick, in seconds: {self.overall_win_start_post_fast_S_pick}")

            # Last window end time is related to the number of dominant period cycles (i.e, 2, 2.5, or 3)
            #self.overall_win_end_post_fast_S_pick = 2.5 * self.Tmid # 2.5 * avg Tmid for the dataset, last window end time
            self.overall_win_end_post_fast_S_pick = last_window_end * self.Tmid
            print(f"Last window end time after S-pick, in seconds: {self.overall_win_end_post_fast_S_pick}")

            # If the shortest candidate window (from the LATEST start window to the EARLIEST end
            # window) would be shorter than max_t_shift_s, the grid search's lag-shift is applied
            # via a cyclic np.roll of the window's own data slice (see _phi_dt_grid_search), so
            # once the tested shift approaches/exceeds the window's own length the window wraps
            # around onto itself, corrupting the result. Rather than discard windows after the
            # fact, anchor the end-window range so the shortest window is exactly max_t_shift_s:
            # shift the first end window out to (max_t_shift_s - win_S_pick_tolerance) after the
            # S-pick, and keep the last end window exactly one dominant period (Tmid) after that
            # -- preserving the end-window range's span (with the repo's standard first/last_
            ...
            if shortest_window_s < self.max_t_shift_s:
                print(f"Warning: shortest candidate window ({shortest_window_s:.4f}s) is shorter "
                      f"than max_t_shift_s ({self.max_t_shift_s:.4f}s). Adjusting end-window range "
                      f"so the shortest window is exactly max_t_shift_s, keeping the end-window "
                      f"range's span fixed at one dominant period (Tmid).")
                self.overall_win_start_post_fast_S_pick = self.max_t_shift_s - self.win_S_pick_tolerance
                self.overall_win_end_post_fast_S_pick = self.overall_win_start_post_fast_S_pick + self.Tmid
                print(f"Adjusted first window end time after S-pick, in seconds: {self.overall_win_start_post_fast_S_pick}")
                print(f"Adjusted last window end time after S-pick, in seconds: {self.overall_win_end_post_fast_S_pick}")

        else:
            raise ValueError("Last window end time must be greater than first window end time!")

        # Default value from SWSPy - we could increase for greater granularity, but this seems to be reasonable for viewing changes across large spatial and temporal scale
    ...
    def _select_windows_orig(self):
        ...
        #end_idxs_range = int((self.overall_win_start_post_fast_S_pick - self.overall_win_end_post_fast_S_pick) * self.fs)
        end_idxs_range = int((self.overall_win_end_post_fast_S_pick - self.overall_win_start_post_fast_S_pick) * self.fs)
        ...
        # Don't multiply this by the sampling rate: it's already in samples relative to the start! But might still need to consider the S-pick time here.
        end_idxs_start = int((self.overall_win_start_post_fast_S_pick))

        win_end_idxs = np.linspace(end_idxs_start, self.overall_win_end_post_fast_S_pick - end_idxs_start, num=self.n_win, dtype=int) #np.random.randint(end_idxs_start, end_idxs_start + start_idxs_range, size=self.n_win)
        
        print(f"First start window: {win_start_idxs[0]}")
        print(f"Last start window: {win_start_idxs[-1]}")
        print(f"Full start window range: {win_start_idxs}")

        print(f"First end window: {win_end_idxs[0]}")
        print(f"Last end window: {win_end_idxs[-1]}")
        print(f"Full end window range: {win_end_idxs}")
    ...
    def _select_windows(self):
        ...
        # Define s_time as the absolute number of samples in the trace where the S-pick occurs
        # We need to define the first start window at zero, and shift everything around that
        s_time_relative = self.overall_win_start_pre_fast_S_pick

        s_time_samples = int(s_time_relative * self.fs)
        print(f"S-pick time in samples, relative to the trace start (where trace starts at the earliest window start time): {s_time_samples}")


        # The start window index range should be last window start - first window start. In this case it is reversed because positive samples is defined as before the S-pick
        # Ex: 2-1σ. (2* 0.0509 - 1*0.0509) * 200) = 10 samples, approximately. We'll start windows every sample from 20 samples before the S-pick to 10 samples before the S-pick.
        start_idxs_range = int((self.overall_win_start_pre_fast_S_pick - self.win_S_pick_tolerance) * self.fs) 

        first_window_start_samples = int(self.overall_win_start_pre_fast_S_pick * self.fs)

        print(f"Start window index range in samples: {start_idxs_range}")

        # The end window index range should be the last window end - first window start.
        end_idxs_range = int((self.overall_win_end_post_fast_S_pick - self.overall_win_start_post_fast_S_pick) * self.fs)

        print(f"End window index range in samples: {end_idxs_range}")

        # The range of windows will start at the number of samples equal to S_pick time - earliest window start time and end at S_pick time - latest window start time, which is captured in start_idxs_range
        #win_start_idxs = np.linspace(s_time_samples - first_window_start_samples, s_time_samples - first_window_start_samples + start_idxs_range, num=self.n_win, dtype=int) #np.random.randint(start_idxs_range, size=self.n_win)
        win_start_idxs = np.linspace(0, start_idxs_range, num=self.n_win, dtype=int)

        # Don't multiply this by the sampling rate: it's already in samples relative to the start!
        # How to make this relative to the s-pick time? 
        # Ex: With Tmid = 18 samples, and starting at 1.5 * Tmid, that's 27 samples
        end_idxs_start = int((self.overall_win_start_post_fast_S_pick) * self.fs)

        # The end window indexes should start at S_pick time +  earliest window end time and go to S_pick time + latest window end time, divided up into 10 equal parts by the number n_win. In this case, 10.
        # The last window end time is 2.5Tmid, Ex: Tmid = 18 samples; 2.5* 18 = 45 samples. 45 - 27 = 18 / 10 = 1.8. Range from 27, 28.8, 30.6, ... 45. 
        win_end_idxs = np.linspace(s_time_samples + end_idxs_start, s_time_samples + end_idxs_start + end_idxs_range, num=self.n_win, dtype=int) #np.random.randint(end_idxs_start, end_idxs_start + start_idxs_range, size=self.n_win)
    ...
    def _sws_win_clustering(self, lags, phis, lag_errs, phi_errs, min_eig_ratios=None, method="dbscan", return_clusters_data=False,
        ...
        # Calculate standard deviation of samples in new coordinate system, to use as clustering distance threshold:
        #samples_std = np.std(samples_new_coords, axis=0)
        # And set clustering distance threshold as 1 times the standard deviation of the samples in the new coordinate system
        #threshold = np.linalg.norm(samples_std)

        # Calc percent of samples w/in 1 std, to use as a minimum percentage of samples to form a cluster:
        #dist_from_mean = np.linalg.norm(samples_new_coords - np.mean(samples_new_coords, axis=0), axis=1)
        #percent_within_threshold = np.sum(dist_from_mean <= threshold) / len(dist_from_mean)
        #min_samples = int(percent_within_threshold * len(lags))
        #min_samples = max(min_samples, 10) # Set a minimum of 10 samples

        #print(f"Clustering threshold: {threshold:.3f}, min_samples: {min_samples} (percent within threshold: {percent_within_threshold:.2%})")
        ...
        if n_clusters > 0:
            ...
            for i in range(n_clusters):
                ...
                clusters_dict[str(i)]['sample_idxs'] = curr_cluster_idxs
            ...
            opt_obs_global_idx = int(smallest_var_cluster['sample_idxs'][opt_obs_idx])
            ...
            if return_clusters_data:
                if not min_eig_ratios is None:
                    return opt_phi, opt_lag, opt_phi_err, opt_lag_err, opt_eig_ratio, clusters_dict, min_var_idx, opt_obs_global_idx
                else:
                    return opt_phi, opt_lag, opt_phi_err, opt_lag_err, clusters_dict, min_var_idx, opt_obs_global_idx

            else:
                if not min_eig_ratios is None:
                    return opt_phi, opt_lag, opt_phi_err, opt_lag_err, opt_eig_ratio, opt_obs_global_idx
                else:
                    return opt_phi, opt_lag, opt_phi_err, opt_lag_err, opt_obs_global_idx
    ...
    def _calc_Q_w(self, opt_phi_EV, opt_lag_EV, grid_search_results_all_win_XC, tr_for_dof, method="dbscan"):
        ...
        opt_phi_XC, opt_lag_XC, opt_phi_err_XC, opt_lag_err_XC, _ = self._sws_win_clustering(lags_tmp, phis_tmp, lag_errs_tmp, phi_errs_tmp, method=method)
    ...
    def _get_uncorr_and_corr_waveforms(self, station):
        ...
        st_ZNE_curr.trim(starttime=arrival_time_curr - self.overall_win_start_pre_fast_S_pick,
                           # endtime=arrival_time_curr + self.overall_win_start_post_fast_S_pick + self.max_t_shift_s) original definition
                           # Updated definition where latest end time equals latest end window time plus a buffer of the max_t_shift_s in case the window is small
    ...
    def _get_uncorr_and_corr_waveforms_multi_layer(self, station):
        ...
        st_ZNE_curr.trim(starttime=arrival_time_curr - self.overall_win_start_pre_fast_S_pick,
                            ...
                            # Redefine in terms of latest end window time, as we redefined max_t_shift_s to be longest window duration:
                            endtime=arrival_time_curr + self.overall_win_end_post_fast_S_pick + self.max_t_shift_s)
    ...
    def perform_sws_analysis(self, coord_system="ZNE", sws_method="EV", return_clusters_data=True, num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS,
        ...
        for station in stations_list:
            ...
            self.phi_dt_grid_best_window[station] = grid_search_results_all_win_EV[int(opt_obs_global_idx), :, :]
    ...
    def perform_sws_analysis_multi_layer(self, coord_system="ZNE", multi_layer_method="explicit", num_threads=numba.config.NUMBA_DEFAULT_NUM_THREADS):
        ...
        for station in stations_list:
            ...
            if multi_layer_method == "explicit":
                ...
                opt_phi_win1, opt_lag_win1, opt_phi_err_win1, opt_lag_err_win1, opt_eig_ratio1, clusters_dict1, min_var_idx1, opt_obs_global_idx1 = self._sws_win_clustering(lags, phis, lag_errs, phi_errs, 
                ...
                opt_phi_win2, opt_lag_win2, opt_phi_err_win2, opt_lag_err_win2, opt_eig_ratio2, clusters_dict2, min_var_idx2, opt_obs_global_idx2 = self._sws_win_clustering(lags, phis, lag_errs, phi_errs, 
                                                                                                                        min_eig_ratios=min_eig_ratios, method="dbscan", return_clusters_data=True)
                if not opt_phi_win2:
                    continue # If didn't cluster, skip station
                ...
                if best_win_idx == 0:
                    ...
                    self.clustering_info[station]['layer2']['opt_obs_global_idx'] = opt_obs_global_idx1
                elif best_win_idx == 1:
                    ...
                    self.clustering_info[station]['layer2']['opt_obs_global_idx'] = opt_obs_global_idx2
    ...
    def plot(self, outdir=None, suppress_direct_plotting=False):
        ...
        for station in self.stations_list:
            ...
            # Plot data:
            # Setup figure:
            fig = plt.figure(constrained_layout=True, figsize=(20*6,16*6))
            if suppress_direct_plotting:
                plt.ion()
            gs = fig.add_gridspec(6*6, 6*6, wspace=0.3, hspace=0.3) # no. rows, no. cols
            ...
            # added by me
            phi_dt_scatter_ax = fig.add_subplot(gs[4:6, 0:2])
            phi_dt_scatter_ax.get_xaxis().set_visible(True)
            #phi_dt_scatter_ax.set_xlabel("Phi (degrees)", fontsize=250)
            phi_dt_scatter_ax.get_yaxis().set_visible(True)
            #phi_dt_scatter_ax.set_ylabel("Delay Time (s)", fontsize=250)
            phi_dt_scatter_ax.tick_params(axis='both')
            ...
            # ...existing code...
            # phi - dt space:
            # Plot for single layer, with best window method:
            if self.sws_multi_layer_result_df is None:
                if station in getattr(self, "phi_dt_grid_best_window", {}):
                    Z = self.phi_dt_grid_best_window[station]   # <-- best window
                    print("Plotting best window result for station:", station)
                else:
                    Z = self.phi_dt_grid_average[station]       # fallback
                    print("Best window result not found for station:", station, "Plotting average result instead.")

                if len(self.lags_labels) == Z.shape[0]:
                    Y, X = np.meshgrid(self.phis_labels, self.lags_labels)
                else:
                    lag_labels_tmp = np.arange(Z.shape[0]) / self.fs
                    Y, X = np.meshgrid(self.phis_labels, lag_labels_tmp)

                CS = phi_dt_ax.contourf(X, Y, Z, levels=20, cmap="magma")
                CS2 = phi_dt_ax.contour(CS, levels=CS.levels[1::2], colors='w', alpha=0.25)
                phi_dt_ax.errorbar(dt_curr , phi_curr, xerr=dt_err_curr, yerr=phi_err_curr, c='g', capsize=20)
                phi_dt_ax.tickparams(fontsize=160)
            else:
                # Setup new axes:
                ...
                phi_dt_ax_layer1.errorbar(dt_layer1 , phi_layer1, xerr=dt_err_layer1, yerr=phi_err_layer1, c='g', capsize=20)  
                ...
                phi_dt_ax_layer2.errorbar(dt_layer2 , phi_layer2, xerr=dt_err_layer2, yerr=phi_err_layer2, c='g', capsize=20)
            ...
            if self.clustering_info:
                ...
                # Or plot single clustering, if not explicit multi-layer result:
                    ...
                    clustered_count = sum(
                                len(self.clustering_info[station]['clusters_dict'][c]['phis'])
                                for c in clust_idxs
                            )
                    total_count = self.n_win ** 2
                    noise_count = total_count - clustered_count

                    print(f"Total windows: {total_count}")
                    print(f"Clustered points plotted: {clustered_count}")
                    print(f"DBSCAN noise (not plotted): {noise_count}")
            ...
            # added by me - whole section here
            # Plot phi-dt scatter distribution
            if self.clustering_info:
                # Collect all phi and dt measurements from all windows
                all_phis = []
                all_dts = []
                all_phi_errs = []
                all_dt_errs = []
                
                # Check if multi-layer (has 'layer1' key) or single layer
                if "layer1" in self.clustering_info[station]:
                    # For multi-layer, plot both layers with different colors
                    for layer_id, color, label in [('layer1', '#1E69A9', 'Layer 1'), 
                                                    ('layer2', '#D73215', 'Layer 2')]:
                        layer_phis = []
                        layer_dts = []
                        layer_phi_errs = []
                        layer_dt_errs = []
                        
                        clust_idxs = list(self.clustering_info[station][layer_id]['clusters_dict'].keys())
                        for clust_idx in clust_idxs:
                            layer_phis.extend(self.clustering_info[station][layer_id]['clusters_dict'][clust_idx]['phis'])
                            layer_dts.extend(self.clustering_info[station][layer_id]['clusters_dict'][clust_idx]['lags'])
                            layer_phi_errs.extend(self.clustering_info[station][layer_id]['clusters_dict'][clust_idx]['phi_errs'])
                            layer_dt_errs.extend(self.clustering_info[station][layer_id]['clusters_dict'][clust_idx]['lag_errs'])
                        
                        #phi_dt_scatter_ax.errorbar(layer_dts, layer_phis, 
                        #                        xerr=layer_dt_errs, yerr=layer_phi_errs,
                        #                        fmt='o', markersize=3, alpha=0.6, 
                        #                        label=label, color=color, capsize=2)
                        
                    # Plot optimal values for both layers
                    phi_dt_scatter_ax.plot(dt_layer1, phi_layer1, 'g*', markersize=12, 
                                        markeredgecolor='k', markeredgewidth=0.5, 
                                        label='Optimal Layer 1', zorder=10)
                    phi_dt_scatter_ax.plot(dt_layer2, phi_layer2, 'y*', markersize=12,
                                        markeredgecolor='k', markeredgewidth=0.5,
                                        label='Optimal Layer 2', zorder=10)
                else:
                    # For single layer
                    clust_idxs = list(self.clustering_info[station]['clusters_dict'].keys())
                    colors = plt.cm.tab10(np.linspace(0, 1, len(clust_idxs)))
                    
                    for idx, clust_idx in enumerate(clust_idxs):
                        phis = self.clustering_info[station]['clusters_dict'][clust_idx]['phis']
                        dts = self.clustering_info[station]['clusters_dict'][clust_idx]['lags']
                        phi_errs = self.clustering_info[station]['clusters_dict'][clust_idx]['phi_errs']
                        dt_errs = self.clustering_info[station]['clusters_dict'][clust_idx]['lag_errs']
                        
                        #phi_dt_scatter_ax.errorbar(dts, phis, xerr=dt_errs, yerr=phi_errs,
                        #                        fmt='o', markersize=3, alpha=0.6,
                        #                        color=colors[idx], capsize=2,
                        #                        label=f'Cluster {clust_idx}')
                    
                        phi_dt_scatter_ax.plot(
                                            dts, phis,
                                            marker='o', linestyle='None',
                                            markersize=4000, alpha=0.6,
                                            color=colors[idx],
                                            label=f'Cluster {clust_idx}'
)
                        
                    # Plot optimal value
                    phi_dt_scatter_ax.plot(dt_curr, phi_curr, 'g*', markersize=5000,
                                        markeredgecolor='k', markeredgewidth=5,
                                        label='Optimal', zorder=10)

                    print("Best solution - Phi:", phi_curr, "Dt:", dt_curr)
                
                # Set labels and limits
                phi_dt_scatter_ax.set_xlabel(r'$\delta t$ (s)')
                phi_dt_scatter_ax.set_ylabel(r'$\phi$ from Q ($^o$)')
                phi_dt_scatter_ax.set_xlim(0, np.max(self.lags_labels))
                phi_dt_scatter_ax.set_ylim(-90, 90)
                phi_dt_scatter_ax.legend(loc='best')
                phi_dt_scatter_ax.grid(True, alpha=0.3)
                phi_dt_scatter_ax.set_title('Clustered Solutions for All Windows')


            # Add text:
            text_ax.text(0,0,"Event origin time : \n"+self.origin_time.strftime("%Y-%m-%dT%H:%M:%SZ"), fontsize='small')
            text_ax.text(0,-1,"Station : "+station, fontsize=180)
            # Plot intermediate post layer 2 correction (multi-layer splitting):
            if self.sws_multi_layer_result_df is not None:
            ...
            if self.sws_multi_layer_result_df is None:
                ...
                phi_dt_ax.tick_params(axis='both')
                phi_dt_ax.set_title('Parameter Space for Best Solution')
            else:
    ...
    def plot_orig(self, outdir=None, suppress_direct_plotting=False):
        """Function to perform plotting...
        """
        # Loop over stations, plotting:
        for station in self.stations_list:
...
class create_splitting_object_orig:
    """
    Class to create splitting object to perform shear wave splitting on.
    ...
    def __init__(self, st, nonlinloc_event_path=None, event_uid=None, stations_in=[], S_phase_arrival_times=[], back_azis_all_stations=[], receiver_inc_angles_all_stations=[], first_window_start=2, last_window_start=0, first_window_end=1.5, last_window_end=2.5, n_win=10, s_pick_uncertainty=0.0509):
        """Initiate the class object.
        ...
        valid_mask = freqs > 0  # exclude DC bin
        # Optionally also apply a frequency band, e.g. 1-50 Hz:
        valid_mask = (freqs >= 4.0) & (freqs <= 50.0)

        if not np.any(valid_mask):
            T_mid = 1 / 8.0  # fallback: 8 Hz average
        else:
            valid_freqs = freqs[valid_mask]
            valid_psd = psd_sum[valid_mask]
            max_power_idx = np.argmax(valid_psd)
        ...
        # Sanity clamp: guard against unrealistically long or short periods
        T_mid = np.clip(T_mid, 0.01, 5.0)  # 0.2–100 Hz range
        ...
        if last_window_end > first_window_end:
            ...
            # First window end time is related to the number of dominant period cycles (i.e, 1, 1.5, or 2)
            #self.overall_win_start_post_fast_S_pick = 1.5 * self.Tmid # 1.5 * avg Tmid for the dataset, first window end time
            self.overall_win_start_post_fast_S_pick = first_window_end * self.Tmid  #Tmid is already in samples!
        ...
        # Define maximum time shift allowed... should be based on volcanic setting! Set as 0.25 seconds as a maximum ceiling, based on Baillard's results
        self.max_t_shift_s = 0.30 # Baillard allows 60 samples maximum at 200 Hz, which is 0.3 seconds. We will do the same here

        #if self.max_t_shift_s < (self.overall_win_end_post_fast_S_pick - self.overall_win_start_pre_fast_S_pick):
        #    print(f"Warning: max_t_shift_s ({self.max_t_shift_s}) is less than the window duration ({self.overall_win_end_post_fast_S_pick - self.overall_win_start_pre_fast_S_pick}).")
        #    print("Setting max_t_shift_s to window duration.")
        #self.max_t_shift_s = self.overall_win_end_post_fast_S_pick - self.overall_win_start_pre_fast_S_pick
    ...
    def _select_windows(self):
        ...
        win_start_idxs = np.linspace(0, start_idxs_range, num=self.n_win, dtype=int) #np.random.randint(start_idxs_range, size=self.n_win)
        end_idxs_start = int((self.overall_win_start_pre_fast_S_pick + self.overall_win_start_post_fast_S_pick) * self.fs) 
        win_end_idxs = np.linspace(end_idxs_start, end_idxs_start + end_idxs_range, num=self.n_win, dtype=int) #np.random.randint(end_idxs_start, end_idxs_start + start_idxs_range, size=self.n_win)
        return win_start_idxs, win_end_idxs
