#!/usr/bin/env python3
"""
traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff.py

Δ-from-pre-eruption companion to
traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax.pdf:
same data/filter/cached ray tracing, COUNT_MIN=10, Gaussian smoothing, original 0-3.0 km /
0.75 km-per-row depth scheme, and true-max colorbars (see that script's docstring for the
COUNT_MIN=10 rationale) -- but plots column 1 (Pre-eruption) as the absolute smoothed field
and columns 2-7 as the per-voxel difference from pre-eruption at that same voxel (diverging
RdBu_r), same convention as traveltime_anisotropy_7period_6stations_newdata_diff.py.

BOTH colorbars (absolute Blues scale and diverging RdBu_r diff scale) use the TRUE MAX of
their own actually-plotted (smoothed, masked) values -- not a percentile, and not the raw
pre-smoothing data -- consistent with the "colorbar must reflect what's on the page" fix
...
Run with:
    python3 traveltime_anisotropy_7period_6stations_newdata_snr2_q75_countmin10_smooth_truemax_diff.py
"""
...
# Original 0-3.0 km / 0.75 km-per-row scheme, expressed as (iz, weight) lists for a shared
# weighted-slice helper (all weights = 1.0 here -- no fractional voxel splitting needed).
DEPTH_ROWS_3KM = [
    ([(0, 1.0), (1, 1.0), (2, 1.0)], '0.0–0.75 km', 0.375),
    ([(3, 1.0), (4, 1.0), (5, 1.0)], '0.75–1.5 km', 1.125),
    ([(6, 1.0), (7, 1.0), (8, 1.0)], '1.5–2.25 km', 1.875),
    ([(9, 1.0), (10, 1.0), (11, 1.0)], '2.25–3.0 km', 2.625),
]
...
def _smoothed_masked_slices(results, depth_rows, count_min_display):
    """Returns smoothed+masked 2D field per (period_idx, depth_row_idx), all pre-computed
    once so both the absolute pre-eruption panel and the diff panels reuse the same smoothed
    arrays (smoothing is not re-applied to a difference -- it's applied per-period first, then
    differenced, matching the sibling _diff script's convention)."""
    fields = {}
...
def make_diff_page(results, title, depth_rows, fields):
    ...
    # Column 0 = pre-eruption absolute; columns 1..n_per-1 = diff vs pre-eruption.
    abs_vals = []
    diff_vals = []
    diffs = {}   # (period_idx>=1, depth_row_idx) -> masked diff array
    pre_masked = {}   # depth_row_idx -> masked pre-eruption array
...
def main():
    ...
    print('Computing smoothed+masked fields (0-3.0km, 4 equal 0.75km rows) for diff...')
    fields = _smoothed_masked_slices(results, DEPTH_ROWS_3KM, count_min_display=T.COUNT_MIN)
    ...
    fig = make_diff_page(results, title, DEPTH_ROWS_3KM, fields)
