"""
figure_axec2_mfast_event_59346_cycleskip_diagnostics.py

Diagnostic multi-page PDF for a single AXEC2 mfast try_filters PRE-ERUPTION event, selected as
an INDETERMINATE-QUALITY event with dt > T_dom/2 -- i.e. a likely CYCLE-SKIPPING candidate,
illustrating exactly why the "dt < T_dom/2" QC cut (used throughout this session's rose/
anisotropy figures) is needed: the measured dt exceeds half the dominant period by a factor of
~4.9, so the true delay time has likely wrapped past a half-cycle ambiguity in the grid search.

Event: AXEC2, event_id 238_59346 (batch 238), origin 2015-04-06T13:01:55.600Z (pre-eruption).

Selection: searched production_axec2_mfast_filters_maxdt02_lqt_pykonal_results/ (the COMPLETED
maxdt02 AXEC2 pre-eruption rerun, all 497 batches), restricted to pre-eruption events with
|production quality| < 0.35 ("indeterminate" -- Q_w near the algorithm's ambiguous midpoint,
neither confidently split nor confidently null, per Wustefeld et al. 2010 Eq. 4) AND
dt > T_dom/2, snr > 5, rectilinearity > 0.8:
    Production: quality=0.000, phi_error=5 deg, dt_error=0.005s, snr_horizontal=7.04,
                rectilinearity_jurkevics=0.8809, incidence_pykonal_s=18.82 deg, dt=0.16s,
                dominant_period=0.0691s (T_dom/2=0.0345s -- dt is ~4.6x over the cycle-skip
                threshold)
    RE-RUN (create_splitting_analysis + perform_sws_analysis, same pipeline as this script):
                phi=74.0 deg, dt=0.170s, phi_error=51 deg, dt_error=0.030s, Q_w=0.000,
                n_clusters=2 (multiple DBSCAN clusters in phi-dt space)
                (re-run's phi_error jumps to 51 deg from the production CSV's stored 5 deg --
                a large swing consistent with a genuinely unstable/ambiguous measurement, not
                just ordinary re-run trimming drift -- see figure_axec1_event_23919's docstring
                for the trimming-drift caveat in general; here the instability is real and is
                exactly the point of this example)
    chosen filter band (try_filters): 10-20 Hz

The waveform file (raw_axec2_all_batches_mfast_filters_data/waveforms/batch238_event_...mseed)
...
Pages:
  ...
  5. phi-dt parameter space (grid-search contour + optimal solution) -- watch for a broad or
     multi-lobed minimum, consistent with the ambiguity flagged above
  6. DBSCAN clustering result in phi-dt space -- expect >1 cluster (see re-run note above)
  ...
  8. Cluster stability (phi and dt vs. window number, with Silver & Chan 1991 error bars, for
     the WINNING (smallest-variance) cluster only -- the other cluster(s) are not plotted here,
     see page 6 for all clusters) + Q and T wavelets, initial vs. corrected overlaid on shared
     axes (same y-scale for Q and T)
...
Run with: python3 figure_axec2_mfast_event_59346_cycleskip_diagnostics.py
"""
...
OUT_PDF = os.path.join(HERE, 'figure_axec2_mfast_event_59346_cycleskip_diagnostics.pdf')
...
WAVEFORM_PATH = os.path.join(HERE, 'raw_axec2_all_batches_mfast_filters_data', 'waveforms',
                              'batch238_event_2015-04-06T13-01-55.600000Z.mseed')
ORIGIN_TIME = UTCDateTime('2015-04-06T13:01:55.600000Z')
S_ARRIVAL_TIME = 1.08   # seconds from origin
P_ARRIVAL_TIME = 0.53   # seconds from origin
BACK_AZIMUTH = 300.5932257547573
INCIDENCE_PYKONAL_S = 18.82305626011668   # drives LQT rotation + QC in production
SNR_HORIZONTAL = 7.037716449599679
RECTILINEARITY = 0.8808886317080581
...
def main():
    ...
    with PdfPages(OUT_PDF) as pdf:
        ...
        plot_multipanel_summary_page(
            ...
            title_suffix='Cycle-Skip Candidate',
        )
